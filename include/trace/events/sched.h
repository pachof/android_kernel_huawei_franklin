...
DECLARE_EVENT_CLASS(sched_wakeup_template,
	...
	TP_STRUCT__entry(
		...
		__field(long, state)
#endif
#ifdef CONFIG_HW_MTK_RTG_SCHED_TRACE
	...
	TP_printk(
#ifdef CONFIG_HW_MTK_RTG_SCHED_TRACE
	"comm=%s pid=%d prio=%d success=%d target_cpu=%03d state=%s rtg_depth=%d",
#else
...
TRACE_EVENT(sched_find_vip_cpu_each,
	...
	TP_fast_assign(
		...
		__entry->task_vip_prio	= task->static_vip;
		__entry->cpu		= cpu;
...
TRACE_EVENT(sched_find_vip_cpu,
	...
	TP_ARGS(task, favor_larger_capacity, target_cpu),

	TP_STRUCT__entry(
		__array(char, comm, TASK_COMM_LEN)
		__field(pid_t, pid)
		__field(unsigned int, static_vip)
		__field(int, target_cpu)
	...
	TP_fast_assign(
		...
		__entry->pid		= task->pid;
		__entry->static_vip	= task->static_vip;
		__entry->target_cpu	= target_cpu;
		__entry->favor_larger_capacity	= favor_larger_capacity;
	),

	TP_printk("pid=%d comm=%s prio=%u favor_larger=%d target=%d",
		__entry->pid, __entry->comm, __entry->static_vip,
		__entry->favor_larger_capacity, __entry->target_cpu)
);
...
TRACE_EVENT(task_min_util,
	...
	TP_fast_assign(
		...
		__entry->pid		= task->pid;
		__entry->static_vip	= task->static_vip;
		__entry->min_util	= task->cpu_cap_req.min_util;
	),

	TP_printk("pid=%d comm=%s prio=%u min_util=%d",
		__entry->pid, __entry->comm, __entry->static_vip,
		__entry->min_util)
);
...
DECLARE_EVENT_CLASS(sched_rtg_template,
	...
	TP_ARGS(__perf_task(p), __perf_task(from), type, op),
...
#endif
		__entry->min_util	= p->cpu_cap_req.min_util;
...
#endif
		__entry->from_min_util	= (from == NULL) ? 0 : from->cpu_cap_req.min_util;
...
#ifdef CONFIG_HW_MTK_RTG_SCHED_TRACE
	"prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%s%s ==> next_comm=%s next_pid=%d next_prio=%d%s%s prev->cgrp=%d next->cgrp=%d prev->st=%d next->st=%d next->rtg_depth=%d",
#else
...
#else
#ifdef CONFIG_HW_MTK_RTG_SCHED_TRACE
	"prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%s%s ==> next_comm=%s next_pid=%d next_prio=%d%s%s next->rtg_depth=%d",
#else
