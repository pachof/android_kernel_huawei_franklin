...
static uint32_t touch_driver_83102_check_crc(
	...
	for (i = 0; i < retry; i++)
	{
		...
		if ((tmp_data[3] == (uint8_t)hx_id_name[0]) &&
			...
			if((tmp_data[3] == HX_83102_ID_PART_1) &&
				...
				IC_NC_TYPE = HX_83102E_SERIES_PWON;
				TS_LOG_INFO("Himax IC pkg HX83102E series\n");
			} else {
...
void touch_driver_83102_read_tp_info(void)
{
	...
	touch_driver_83102_sense_on(SENSE_ON_0);
	while (1) {
...
void touch_driver_83102_sense_on(uint8_t flashmode)
{
...
static int touch_driver_83102_projectid_read(uint8_t *buffer)
{
	uint32_t len;
	uint8_t type;
	int j;
...
static int touch_driver_83102_tpcolor_read(uint8_t *buffer)
{
	uint32_t len;
	uint8_t type;
	int j;
...
int touch_driver_read_panel_info(void)
{
	...
	for (i = 0; i < INFO_SECTION_NUM; i++) {
		...
		if (touch_driver_83102_check_crc(tmp_addr,
			INFO_PAGE_LENGTH) != 0) {
		...
		if (title[DATA_0] == DATA_0 &&
			...
			touch_driver_83102_projectid_read(buffer);
		} else if (title[DATA_0] == DATA_0 &&
			title[DATA_1] == DATA_0 && title[DATA_2] == DATA_1) {
			touch_driver_83102_tpcolor_read(buffer);
		} else {
	...
	if (buffer != NULL)
		kfree(buffer);
	touch_driver_83102_sense_on(SENSE_ON_1);
	return NO_ERR;
}
...
static uint32_t touch_driver_83102_check_crc(uint8_t *start_addr,
		int reload_length)
{
	uint32_t result;
	uint8_t tmp_addr[DATA_4] = {0};
...
uint8_t himax_nc_calculateChecksum(bool change_iref)
{
	...
	if (IC_NC_TYPE == HX_83102E_SERIES_PWON) {
		...
		CRC_result = touch_driver_83102_check_crc(tmp_data,
			FW_SIZE_128k);
...
int hx_nc_fts_ctpm_fw_upgrade_with_fs(unsigned char *fw, int len, bool change_iref)
{
	...
	if (IC_NC_TYPE == HX_83102E_SERIES_PWON) {
		...
		himax_rw_reg_reformat(DATA_INIT, tmp_data);
		if (touch_driver_83102_check_crc(tmp_data, len) == DATA_0)
