...
static int g_soc;
static BLOCKING_NOTIFIER_HEAD(g_soc_decimal_nh);
...
{
	if (!l_dev->info.start)
		return power_platform_get_filter_soc() + l_dev->ui_offset;
	else if (l_dev->info.level == SOC_DECIMAL_DEFAULT_LEVEL)
		return power_platform_get_filter_soc() + l_dev->ui_offset;
	else
		return l_dev->info.soc + l_dev->ui_offset;
}
...
static void soc_decimal_init_fifo(struct soc_decimal_dev *l_dev)
{
	...
	curr_soc = power_platform_get_filter_soc();
	curr_soc /= 10; // 10 is decimal base
	curr_soc *= 10;
...
void huawei_set_ui_soc(int soc)
{
	g_soc = soc;
}
...
static u32 soc_decimal_calculate_soc(struct soc_decimal_dev *l_dev)
{
	...
	if (curr_samples < samples) {
		...
		return power_platform_get_filter_soc();
	}
	...
	hwlog_info("samples[%d]: curr_soc=%d rep_soc=%d round=%d remain=%d ui_soc=%d\n",
		l_dev->info.fifo.curr_samples,
		curr_soc, rep_soc, round_soc, remain_soc, g_soc);

	power_platform_sync_filter_soc(rep_soc, round_soc);
	return rep_soc;
}
...
static int soc_decimal_notifier_chain_register(struct notifier_block *nb)
{
	if (!nb) {
	...
	return blocking_notifier_chain_register(&g_soc_decimal_nh, nb);
}

static int soc_decimal_notifier_chain_unregister(struct notifier_block *nb)
{
	if (!nb) {
		hwlog_err("nb is null\n");
		return NOTIFY_OK;
	...
	return blocking_notifier_chain_unregister(&g_soc_decimal_nh, nb);
}

void soc_decimal_event_notify(unsigned long event, void *data)
{
	blocking_notifier_call_chain(&g_soc_decimal_nh, event, data);
}
...
static void soc_decimal_sysfs_init_attrs(void)
{
	int s;

	for (s = 0; s < SOC_DECIMAL_SYSFS_ATTRS_SIZE; s++)
		soc_decimal_sysfs_attrs[s] = &soc_decimal_sysfs_field_tbl[s].attr.attr;

	soc_decimal_sysfs_attrs[s] = NULL;
}

static struct power_sysfs_attr_info *soc_decimal_sysfs_field_lookup(
	const char *name)
{
	int s;

	for (s = 0; s < SOC_DECIMAL_SYSFS_ATTRS_SIZE; s++) {
		if (!strncmp(name, soc_decimal_sysfs_field_tbl[s].attr.attr.name,
			strlen(name)))
			break;
	}

	if (s >= SOC_DECIMAL_SYSFS_ATTRS_SIZE)
		return NULL;

	return &soc_decimal_sysfs_field_tbl[s];
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	info = soc_decimal_sysfs_field_lookup(attr->attr.name);
	if (!info) {
	...
	case SOC_DECIMAL_SYSFS_PARA:
		for (i = 0; i < l_dev->para_level; i++) {
			(void)memset_s(rd_buf, SOC_DECIMAL_RD_BUF_SIZE,
				0, SOC_DECIMAL_RD_BUF_SIZE);
	...
	const char *buf)
{
	...
	hwlog_err("soc_decimal_sysfs_store_start=%s\n", buf);
	...
	hwlog_err("soc_decimal_sysfs_store_0\n");

	if (soc_decimal_get_op_user(user) < 0)
		return -EINVAL;
	hwlog_err("soc_decimal_sysfs_store_1\n");
	...
	hwlog_err("soc_decimal_sysfs_store l_dev->info.start=%d\n", l_dev->info.start);

	if (!l_dev->info.start)
		return 0;
	hwlog_err("soc_decimal_sysfs_store l_dev->info.level =%d\n", l_dev->info.level);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	hwlog_err("soc_decimal_sysfs_store=%d\n", info->name);
	if (l_dev->info.level == SOC_DECIMAL_DEFAULT_LEVEL)
		msleep(10); /* wait 10ms to update info.level */
	switch (info->name) {
	...
	struct soc_decimal_dev *l_dev)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ui_offset", &l_dev->ui_offset, UI_OFFSET);
...
static int soc_decimal_probe(struct platform_device *pdev)
{
	...
	hwlog_err("soc_decimal_probe start \n");
	...
	ret = soc_decimal_notifier_chain_register(&l_dev->nb);
	if (ret)
		goto fail_free_mem;
	...
	hwlog_err("soc_decimal_probe end \n");
	return 0;
...
static int soc_decimal_remove(struct platform_device *pdev)
{
	...
	(void)soc_decimal_notifier_chain_unregister(&l_dev->nb);
