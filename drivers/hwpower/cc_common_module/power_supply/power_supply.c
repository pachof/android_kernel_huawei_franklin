...
{
	int ret;
	struct power_supply *phy = NULL;

	if (!name || !val)
		return -1;

	phy = power_supply_get_by_name(name);
	if (!phy) {
		hwlog_err("power supply %s not exist\n", name);
		return -1;
	}

	ret = power_supply_get_property(phy, psp, val);
	if (ret) {
		hwlog_err("get %u property of %s fail return %d\n",
			psp, name, ret);
		return ret;
	}
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	ret = power_supply_set_property(phy, psp, val);
	if (ret) {
		hwlog_err("set %u property of %s fail return %d\n",
			psp, name, ret);
		return ret;
	}
	...
	unsigned long event, void *data)
{
	...
	switch (event) {
	case POWER_EVENT_NE_PSY_UPDATE_BATTERY:
		if (l_dev->support_bat)
			power_supply_changed(l_dev->bat);
		break;
	case POWER_EVENT_NE_PSY_UPDATE_RAW_BATTERY:
		if (l_dev->support_raw_bat)
			power_supply_changed(l_dev->raw_bat);
		break;
	case POWER_EVENT_NE_PSY_UPDATE_ASSITST_BATTERY:
		if (l_dev->support_assist_bat)
			power_supply_changed(l_dev->assist_bat);
		break;
	case POWER_EVENT_NE_PSY_UPDATE_MAINS:
		if (l_dev->support_mains)
			power_supply_changed(l_dev->mains);
		break;
	case POWER_EVENT_NE_PSY_UPDATE_USB:
		if (l_dev->support_usb)
			power_supply_changed(l_dev->usb);
		break;
	case POWER_EVENT_NE_PSY_UPDATE_WIRELESS:
...
static enum power_supply_property power_supply_battery_props[] = {
 ...
 #ifdef CONFIG_DIRECT_CHARGER
 extern int huawei_battery_get_scp_status(void);
 extern int huawei_battery_get_fcp_status(void);
 #endif
...
static int power_supply_probe(struct platform_device *pdev)
{
	...
	power_event_notifier_chain_register(&l_dev->nb);
