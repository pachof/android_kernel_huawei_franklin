...
void write_conf_to_tee(void)
{
	...
	uint8_t ese_type = 2; // ese type 2:ST
...
static int check_sim_status(struct i2c_client *client, struct  st21nfc_dev *pdev)
{
    ...
    if (pdev->platform_data.reset_gpio != 0) {
        /* pulse low for 20 millisecs */
        gpio_set_value(pdev->platform_data.reset_gpio,
                0);
        pr_info("%s,reset_gpio=%d\n", __func__,gpio_get_value(pdev->platform_data.reset_gpio));
        msleep(20);
        gpio_set_value(pdev->platform_data.reset_gpio,
				 ...
				 size_t count, loff_t *offset)
{
	...
	for (j = 0; j < send_times; j++) {
		...
		if (is_active_cmd) {
			...
			hci_byte = (unsigned char)data_buf[INDEX_3_HCI];
			if (j != send_times - 1) { // not last packet
				data_len = DATA_LENGTH;
				hci_byte &= ~(1 << 7);  // clear bit 7
				short_buf[INDEX_3_HCI] = (char)hci_byte;
			} else {
				data_len = (remainder == 0) ?
					DATA_LENGTH : remainder;
			}
			short_buf[INDEX_2_LEN] = HCI_HEADER + data_len;
			err_ret= memcpy_s(&short_buf[HEADER_LEN], DATA_LENGTH,
				&data_buf[HEADER_LEN + DATA_LENGTH * j],
				data_len);
			if (err_ret != EOK)
				hwlog_err("%s : memcpy_s fail, err_ret = %d\n",
					__func__, err_ret);
		...
		for(retry = 0; retry < NFC_TRY_NUM; retry++) {
			...
			// 0~4:printf head 5 bytes, send_buf length is larger than 5
...
static int st21nfc_dev_open(struct inode *inode, struct file *filp)
{
    ...
    // signal PMU that NFC clock may be requested
	//if (g_nfc_clk_src != NFC_CLK_SRC_XTAL)
...
static int st21nfc_release(struct inode *inode, struct file *file)
{
    ...
    // signal PMU that NFC clock won t be requested
	//if (g_nfc_clk_src != NFC_CLK_SRC_XTAL)
...
//
	...
	st21nfc_dev->pctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR(st21nfc_dev->pctrl)) {
		st21nfc_dev->pctrl = NULL;
		pr_err("%s : failed to get clk pin\n", __func__);
	} else {
		st21nfc_dev->pins_default = pinctrl_lookup_state(st21nfc_dev->pctrl, "srclkenai");
		if (IS_ERR_OR_NULL(st21nfc_dev->pins_default)) {
			hwlog_err("can not lookup gpio_sleep_low pinstate\n");
		} else {
		    ret = pinctrl_select_state(st21nfc_dev->pctrl, st21nfc_dev->pins_default);
		    if (ret < 0)
			    pr_err("%s: unapply new state!\n", __func__);
		    else
			    pr_info("%s : clk pin set success\n", __func__);
		}
	}
	...
	if (IS_ERR(st21nfc_dev->pctrl)) {
		...
		st21nfc_dev->pins_idle = pinctrl_lookup_state(st21nfc_dev->pctrl, "eseclkreq");
		if (IS_ERR_OR_NULL(st21nfc_dev->pins_idle)) {
			hwlog_err("can not lookup gpio_sleep_low pinstate\n");
		} else {
		    ret = pinctrl_select_state(st21nfc_dev->pctrl, st21nfc_dev->pins_idle);
		    if (ret < 0)
			    pr_err("%s: unapply new state!\n", __func__);
		    else
			    pr_info("%s : clk pin set success\n", __func__);
