...
static int stwlc88_tx_set_fod_coef(u32 pl_th, u8 pl_cnt)
{
	...
	pl_th /= ST88_TX_PLOSS_TH_STEP;
	/* tx ploss threshold 0:disabled */
	ret = stwlc88_write_byte(ST88_TX_PLOSS_TH_ADDR, (u8)pl_th);
	/* tx ploss fod debounce count 0:no debounce */
	ret += stwlc88_write_byte(ST88_TX_PLOSS_CNT_ADDR, pl_cnt);
...
static int stwlc88_tx_set_init_para(struct stwlc88_dev_info *di)
{
	...
	ret += stwlc88_tx_set_fod_coef(ST88_TX_PLOSS_TH_VAL,
