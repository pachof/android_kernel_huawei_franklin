...
#define TORCH_MODE (1)
#define MMI_SUB_REARFLASH (9)
#define MAX_FLASH_ERR_INFO_LEN 60
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((flash_id == SUB_REARFLASH) || (mode == MMI_SUB_REARFLASH)) {
		ct_id = SUB_FLASH_CT;
		pr_info("%s factory mode ct_id=%d\n", __func__, ct_id);
	}

	if ((last_time_mode == MMI_SUB_REARFLASH) && (mode == STANDBY_MODE)) {
		ct_id = SUB_FLASH_CT;
		pr_info("%s standby mode ct_id=%d\n", __func__, ct_id);
	}
	last_time_mode = mode;

	pr_info("%s mtk_flash_id=%d,ct_id=%d,last_time_mode=%d\n", __func__, mtk_flash_id, ct_id, last_time_mode);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (length != 1) {
		pr_err("%s failed to check para", __func__);
		return -1;
	}
	mutex_lock(&fl_mutex);
...
static void flash_hiview_handle(int error_no,
	struct flash_name_info_t *name_info, u8 addr, s32 data)
{
	errno_t ret;
	char error_info[MAX_FLASH_ERR_INFO_LEN] = {0};
	struct hwcam_hievent_info flash_info;

	ret = memset_s(&flash_info, sizeof(flash_info), 0, sizeof(flash_info));
	if (ret != EOK) {
		pr_err("flash_info memset_s fail, ret = %d\n", ret);
		return;
	}

	if (snprintf_s(error_info, sizeof(error_info), sizeof(error_info) - 1,
		"(addr[0x%x], value[0x%x])", addr, data) < 0)
		pr_err("error_info snprintf_s fail\n");

	flash_info.error_no = error_no;
	hwcam_hiview_get_ic_name(name_info->ic_name, &flash_info);
	hwcam_hiview_get_module_name(name_info->module_name, &flash_info);
	hwcam_hiview_get_content(error_no, error_info, &flash_info);
	hwcam_hiview_report(&flash_info);
}

static u8 get_flag_value_by_mask(flag_mask_mode_t mask_mode,
	u8 error_mask, u8 reg_val)
{
	u8 val;

	switch (mask_mode) {
	case REG_VAL_OR_MASK:
		val = reg_val | error_mask;
		break;
	case REG_VAL_AND_MASK:
		val = reg_val & error_mask;
		break;
	default:
		val = reg_val;
...
static void flash_error_handle(u8 reg_val, u8 reg_addr,
	struct flash_error_table_t *error_table, unsigned int size,
	struct flash_name_info_t *name_info)
{
	unsigned int i;
	u8 val;

	for (i = 0; i < size; ++i) {
		val = get_flag_value_by_mask(error_table[i].mask_mode,
			error_table[i].error_mask, reg_val);
		if (val == 0)
			continue;
		flash_hiview_handle(error_table[i].error_no,
			name_info, reg_addr, reg_val);
		pr_err("%s\n", error_table[i].error_info);
	}
}

/*
 * FunctionName: flash_clear_err_and_unlock;
 * Description : clear the error and unlock the IC;
 * NOTE: this funtion must be called before register is read and write
 */
int flash_clear_error_and_unlock(struct i2c_client *client,
	struct flash_error_info_t *error_info, unsigned int size,
	struct flash_name_info_t *name_info)
{
	s32 reg_val;
	unsigned int i;

	if (!client || !error_info || !name_info) {
		pr_err("params is NULL\n");
		return -1;
	}

	for (i = 0; i < size; ++i) {
		reg_val = i2c_smbus_read_byte_data(client, error_info[i].reg_addr);
		if (reg_val < 0) {
			flash_hiview_handle(DSM_FLASH_I2C_ERROR_NO,
				name_info, error_info[i].reg_addr, reg_val);
			pr_err("flash i2c fail\n");
			return -1;
		}
		flash_error_handle((u8)reg_val, error_info[i].reg_addr,
			error_info[i].error_table, error_info[i].size, name_info);
	}
