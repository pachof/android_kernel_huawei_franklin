...
#define BUF_LEN               40
static const u8 g_mt6360_reg_addr[] = {
	...
	MT6360_PMU_CHG_IRQ5,
	MT6360_PMU_CHG_IRQ6,
	MT6360_PMU_BUCK1_IRQ,
	MT6360_PMU_BUCK2_IRQ,
	MT6360_PMU_LDO_IRQ1,
	MT6360_PMU_LDO_IRQ2,
	MT6360_PMU_CHG_STAT1,
	MT6360_PMU_CHG_STAT2,
	MT6360_PMU_CHG_STAT3,
	MT6360_PMU_CHG_STAT4,
	MT6360_PMU_CHG_STAT5,
	MT6360_PMU_CHG_STAT6,
	MT6360_PMU_DPDM_STAT,
	MT6360_PMU_CHRDET_STAT,
};
...
static int mt6360_chgdet_post_process(struct mt6360_pmu_chg_info *mpci)
{
	...
	/* BC12 retry 3 times(NONSTD) */
	if (!(get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT ||
		get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT) &&
		atomic_read(&mpci->bc12_retry_cnt) < 3 &&
		mpci->chg_type == NONSTANDARD_CHARGER) {
...
static int mt6360_enable_eoc(struct charger_device *chg_dev, bool en)
{
	...
	mpci = charger_get_data(chg_dev);
	if (!mpci)
		return -EINVAL;
	dev_info(mpci->dev, "%s: en = %d\n", __func__, en);

	return mt6360_pmu_reg_update_bits(mpci->mpi, MT6360_PMU_CHG_CTRL9,
...
static int mt6360_enable_force_sleep(struct charger_device *chg_dev, bool en)
{
	struct mt6360_pmu_chg_info *mpci = charger_get_data(chg_dev);
	...
	return (en ? mt6360_pmu_reg_set_bits : mt6360_pmu_reg_clr_bits)
		(mpci->mpi, MT6360_PMU_CHG_CTRL1, MT6360_MASK_FORCE_SLEEP);
}
...
static int mt6360_get_ts(struct charger_device *chg_dev, u32 *ts_volt)
{
	struct mt6360_pmu_chg_info *mpci = charger_get_data(chg_dev);

	mt_dbg(mpci->dev, "%s\n", __func__);
	return mt6360_get_adc(chg_dev, ADC_CHANNEL_TS, ts_volt, ts_volt);
}
...
static int mt6360_get_usb_id(struct charger_device *chg_dev, u32 *id)
{
	struct mt6360_pmu_chg_info *mpci = charger_get_data(chg_dev);

	mt_dbg(mpci->dev, "%s\n", __func__);
	return mt6360_get_adc(chg_dev, ADC_CHANNEL_USBID, id, id);
}

int mt6360_get_batt_by_usb_id(void)
{
	int id = 0;
	int ret;
	if (!g_mpci || !g_mpci->chg_dev)
		return 0;

	mt_dbg(g_mpci->dev, "%s\n", __func__);
	ret = mt6360_get_adc(g_mpci->chg_dev, ADC_CHANNEL_USBID, &id, &id);
	if (ret < 0)
		return 0;
	return id;
}
...
{
	struct mt6360_pmu_chg_info *mpci = charger_get_data(chg_dev);
	struct mt6360_pmu_core_info *core_mpci = get_core_mpci();
	int i, ret = 0;
	...
	ret = mt6360_pmu_reg_read(core_mpci->mpi, 0x01);
	dev_info(core_mpci->dev, "%s: MT6360_PMU_CORE_CTRL1: 0x%02X\n", __func__, ret);

	return 0;
}

static int mt6360_get_register_head(struct charger_device *chg_dev,
	char *reg_head, int size)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_mt6360_reg_addr); i++) {
		len = snprintf(reg_head + offset, size - offset, "R[0x%02X] ",
			g_mt6360_reg_addr[i]);
		if (len <= 0)
			break;
		else
...
static int mt6360_hw_dump_register(struct charger_device *chg_dev,
	char *reg_value, int size)
{
	...
	if (!chg_dev || !reg_value)
		return -EINVAL;
	chip = charger_get_data(chg_dev);
	if (!chip)
		return -EINVAL;
	memset(reg_value, 0, size);
	for (i = 0; i < ARRAY_SIZE(g_mt6360_reg_addr); i++) {
		ret = mt6360_pmu_reg_block_read(chip->mpi, g_mt6360_reg_addr[i], 1, &regval);
		if (ret < 0) {
			pr_err("dump_register read Reg[%02X] fail\n", i);
			regval = 0;
		}

		len = snprintf(reg_value + offset, size - offset, "0x%-6.2x", regval);
		if (len <= 0)
			break;
		else
			offset += len;
...
static const struct charger_ops mt6360_chg_ops = {
	...
	.get_ts_adc = mt6360_get_ts,
	.get_usbid_adc = mt6360_get_usb_id,
	/* kick wdt */
	...
	.dump_reg = mt6360_hw_dump_register,
	.get_reg_head  = mt6360_get_register_head,
	/* event */
	...
	.enable_force_sleep = mt6360_enable_force_sleep,
};
...
static struct charge_extra_ops mt6360_charge_extra_ops = {
        .get_batt_by_usb_id = mt6360_get_batt_by_usb_id,
};
...
&& !defined(CONFIG_TCPC_CLASS)
	...
	/* charge get usb id ops register */
	ret = charge_extra_ops_register(&mt6360_charge_extra_ops);
	if (ret != 0)
                dev_err(mpci->dev, "charge extra ops register fail\n");
