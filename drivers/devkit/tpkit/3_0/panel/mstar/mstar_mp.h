...
#define UNIF_ABS(x,y) ((x-y) > 0 ? (x-y) : (y-x))
...
#define INI_PATH "/odm/etc/firmware/ts/autoSettings.ini"


#define INI_FILE_NAME_LENS                  64
#define INI_FILE_PATH_LENS                  128

#define INI_PATH_PERF_ODM                   "/odm/etc/firmware/ts/"
...
typedef struct
{
    ...
    u16 *nNormalTestResultCheck;
    u16 *normalTestFail_check_Deltac;
    u16 *normalTestFail_check_Ratio;
    int *ratio_border;
    int *ratio_move;
    int *ratio;

    /* global vars for short */
    s8 normalTestFail[8];//0:golden    1:ratio
    u16 ** normalTestFail_check;//6:max subframe    14:max afe
...
extern int mstar_mp_stop_mcu(void);
extern int mstar_mp_start_mcu(void);
extern int mstar_dbbus_exit(void);
extern int mstar_dbbus_enter(void);

extern MutualMpTestResult_t * mstar_get_mp_test_info(void);
