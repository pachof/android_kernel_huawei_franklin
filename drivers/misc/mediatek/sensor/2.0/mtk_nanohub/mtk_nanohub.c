...
#define SENSOR_TYPE_GAIN 1000000
...
struct mtk_nanohub_device {
	...
	int32_t light_config_data[6];
	int32_t proximity_config_data[2];
	int32_t pedo_config_data[2];
	int32_t sar_config_data[4];
	int32_t ois_config_data[2];
	int32_t hw_motion_config_data[2];
	int32_t finger_sense_data[2];
	int32_t aod_config_data[3];
	int32_t rpc_motion_data[2];
};
...
static void mtk_nanohub_init_sensor_info(void)
{
	...
	p = &sensor_state[SENSOR_TYPE_HW_MOTION];
	p->sensorType = SENSOR_TYPE_HW_MOTION;
	p->gain = 1000000;
	strlcpy(p->name, "mo_tion", sizeof(p->name));
	strlcpy(p->vendor, "mtk", sizeof(p->vendor));

	p = &sensor_state[SENSOR_TYPE_FINGER_SENSE];
	p->sensorType = SENSOR_TYPE_FINGER_SENSE;
	p->gain = 1000000;
	strlcpy(p->name, "finger_sen", sizeof(p->name));
	strlcpy(p->vendor, "mtk", sizeof(p->vendor));
	...
	p = &sensor_state[SENSOR_TYPE_RPC_MOTION];
	p->sensorType = SENSOR_TYPE_RPC_MOTION;
	p->gain = SENSOR_TYPE_GAIN;
	strlcpy(p->name, "rpc_motion", sizeof(p->name));
	strlcpy(p->vendor, "mtk", sizeof(p->vendor));
}
		...
		int sensor_type, int32_t *data)
{
	...
	case ID_STEP_COUNTER:
		length = sizeof(device->pedo_config_data);
		spin_lock(&config_data_lock);
		memcpy(device->pedo_config_data, data, length);
		spin_unlock(&config_data_lock);
		break;
	case ID_SAR:
		length = sizeof(device->sar_config_data);
	...
	case ID_OIS:
		...
		break;
	case ID_HW_MOTION:
		length = sizeof(device->hw_motion_config_data);
		spin_lock(&config_data_lock);
		memcpy(device->hw_motion_config_data, data, length);
		spin_unlock(&config_data_lock);
		break;
	case ID_FINGER_SENSE:
		length = sizeof(device->finger_sense_data);
		spin_lock(&config_data_lock);
		memcpy(device->finger_sense_data, data, length);
		spin_unlock(&config_data_lock);
		break;
	case ID_AOD:
		length = sizeof(device->aod_config_data);
		spin_lock(&config_data_lock);
		memcpy(device->aod_config_data, data, length);
		spin_unlock(&config_data_lock);
		break;
	case ID_RPC_MOTION:
		length = sizeof(device->rpc_motion_data);
		spin_lock(&config_data_lock);
		memcpy(device->rpc_motion_data, data, length);
		spin_unlock(&config_data_lock);
		break;
...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		...
		case ID_PROXIMITY:
			...
			event.word[1] = data->data[1];
			event.word[2] = data->data[2];
			pr_info("kernel get ps state = %d, pdata = %d, thre_l = %d\n",
		...
		case ID_STEP_COUNTER:
			...
			pr_info("kernel get pedo = %d, gap = %lu\n", event.word[0],
		...
		case ID_HW_MOTION:
			...
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->hw_motion_event.data[0];
			event.word[1] = data->hw_motion_event.data[1];
			event.word[2] = data->hw_motion_event.data[2];
			break;
		case ID_FINGER_SENSE:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->fg_sense_event.fg_data_ready;
			event.word[1] = data->fg_sense_event.fg_status;
			event.word[2] = data->fg_sense_event.fg_temp;
			break;
		case ID_AOD:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->aod_motion_event.data[0];
			event.word[1] = data->aod_motion_event.data[1];
			event.word[2] = data->aod_motion_event.data[2];
			break;
		case ID_RPC_MOTION:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->rpc_motion_event.data[0];
			event.word[1] = data->rpc_motion_event.data[1];
			event.word[2] = data->rpc_motion_event.data[2];
			break;
		default:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
		...
		pr_info("kernel get sensor(%u) cali result : accurancy = %d "
			"bias = %d %d %d %d %d %d\n", event.sensor_type,
			event.accurancy, event.word[0], event.word[1], event.word[2],
		...
		pr_info("kernel get sensor(%u) selftest result : %d\n",
			event.sensor_type, event.word[0]);
	} else {
		event.timestamp = data->time_stamp;
		event.sensor_type = id_to_type(data->sensor_type);
		event.action = data->flush_action;
			...
			void *unused)
{
	...
	case PM_POST_SUSPEND:
		pr_info("resume ap boottime=%lld\n", ktime_get_boot_ns());
		WRITE_ONCE(rtc_compensation_suspend, false);
		mtk_nanohub_send_timestamp_to_hub();
		return NOTIFY_DONE;
	case PM_SUSPEND_PREPARE:
		pr_info("suspend ap boottime=%lld\n", ktime_get_boot_ns());
