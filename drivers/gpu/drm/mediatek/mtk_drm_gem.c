...
{
	void *va = NULL;
	unsigned long value = (uintptr_t)dma_handle;
	...
	DRM_MMP_EVENT_END(dma_alloc, value,
			(unsigned long)size);
	return va;
}
...
{
	struct drm_gem_object *obj;
	struct mtk_drm_gem_obj *mtk_gem = NULL;// = to_mtk_gem_obj(obj);
#if defined(CONFIG_MTK_IOMMU_V2)
	struct mtk_drm_private *priv = dev->dev_private;
	struct ion_client *client;
	struct ion_handle *handle;
			 ...
			 struct drm_file *file_priv)
{
	...
	struct mtk_fence_buf_info *buf = NULL, *buf2 = NULL;
