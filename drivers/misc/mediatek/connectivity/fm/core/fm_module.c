...
static unsigned int huawei_fm_info = 0;
...
Called By:      audio driver side accdet.c send_accdet_status_event()
...
void setHeadsetStatus(bool status)
{
	...
	WCN_DBG(FM_NTC | MAIN, "%s: [fm build-in] is_headset_on=%d\n", __func__, is_headset_on);

	if (fm_node && is_headset_on && gpio_is_valid(huawei_fm_info)) {
		WCN_DBG(FM_NTC | MAIN, "%s: [fm build-in] Headset is plug-in, pull down gpio\n", __func__);
		gpio_set_value_cansleep(huawei_fm_info, GPIO_PULL_DOWN);
	}
	else {
		WCN_DBG(FM_NTC | MAIN, "%s: [fm build-in]  get headset status is fail\n", __func__);
	}
	return;
}
EXPORT_SYMBOL(setHeadsetStatus);

static bool get_fm_dtsi(void)
{
	fm_node = of_find_compatible_node(NULL,NULL,"huawei,huawei_fm_info");

	if (fm_node) {
		WCN_DBG(FM_NTC | MAIN,"%s: [fm build-in] find node is ok\n", __func__);
		return true;
	}

	WCN_DBG(FM_ERR | MAIN,"%s: [fm build-in] find node is fail\n", __func__);
	return false;
}

static bool init_dtsi_lna(void)
{
	int ret = 0;
	huawei_fm_info = of_get_named_gpio_flags(fm_node, "fm,fm_elna_pins", 0,NULL);

	if (huawei_fm_info < 0){
		WCN_DBG(FM_NTC | MAIN,"%s: [fm build-in] no provide the gpio pin,gpio=%d\n", __func__, huawei_fm_info);
		goto err;
	}

	ret = gpio_request(huawei_fm_info, "fm_elna_pins");
	WCN_DBG(FM_NTC | MAIN,"%s: [fm build-in] request gpio: ret=%d,gpio=%d\n", __func__, ret, huawei_fm_info);

	if (ret) {
		WCN_DBG(FM_ERR | MAIN,"%s: [fm build-in] request gpio failed\n", __func__);
		goto err;
	}

	if (gpio_direction_output(huawei_fm_info, GPIO_PULL_DOWN)) {
		WCN_DBG(FM_ERR | MAIN,"%s: [fm build-in] gpio set output failed!\n", __func__);
		gpio_free(huawei_fm_info);
		goto err;
	}
	return true;

err:
	WCN_DBG(FM_ERR | MAIN,"%s: [fm build-in] go to error!\n", __func__);
	return false;
}
...
static long fm_ops_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FM_IOCTL_POWERUP:{
			...
			if (fm_node) {
				if ((!is_headset_on) && gpio_is_valid(huawei_fm_info)) {
					WCN_DBG(FM_NTC | MAIN, "pull up gpio\n");
					gpio_set_value_cansleep(huawei_fm_info, GPIO_PULL_UP);
				}
				else {
					WCN_DBG(FM_NTC | MAIN, "headset in or gpio is unvalid\n");
				}
			}
			else {
				WCN_DBG(FM_ERR | MAIN, "find node is fail\n");
			}
	...
	case FM_IOCTL_POWERDOWN:{
			...
			if (fm_node) {
				if (gpio_is_valid(huawei_fm_info)) {
					WCN_DBG(FM_NTC | MAIN, "pull down gpio\n");
					gpio_set_value_cansleep(huawei_fm_info, GPIO_PULL_DOWN);
				}
				else {
					WCN_DBG(FM_ERR | MAIN, "gpio is unvalid\n");
				}
			}
			else {
				WCN_DBG(FM_ERR | MAIN, "find node is fail\n");
...
static signed int mt_fm_probe(struct platform_device *pdev)
{
	...
	if (get_fm_dtsi()) {
		if (true == init_dtsi_lna()) {
			WCN_DBG(FM_NTC | MAIN, "init dtsi lna success\n");
		}
		else {
			WCN_DBG(FM_NTC | MAIN, "init dtsi lna failed\n");
		}
	}
