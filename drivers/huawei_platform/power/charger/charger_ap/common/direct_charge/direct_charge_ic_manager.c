...
static inline struct direct_charge_batinfo_ops *dc_get_battinfo_ops(int mode,
	unsigned int type)
{
	if (type >= CHARGE_IC_TYPE_MAX)
		return NULL;
...
int dc_set_ic_buck_enable(int mode, int type, int enable)
{
	...
	hwlog_info("ic buck enable type %d, enable %d\n", type, enable);
	return ret;
}
...
int dc_ic_reg_reset_and_init(int mode, int type)
{
	...
	hwlog_info("ic reg reset and init, type: %d\n", type);
...
{
	int vbat_main = dc_get_bat_btb_voltage_with_comp(mode, CHARGE_IC_MAIN, vbat_comp);
	int vbat_aux = dc_get_bat_btb_voltage_with_comp(mode, CHARGE_IC_AUX, vbat_comp);
