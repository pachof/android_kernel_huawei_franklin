...
static void himos_log_detect_result(struct himos_aweme_detect_info *info)
{
	...
	pr_debug("print detect result-------begin");
	pr_debug("valid_num=%d, cur_index=%d, cur_detect_index=%d",
		info->valid_num, info->cur_index, info->cur_detect_index);
	for (i = 0; i < info->valid_num; ++i) {
		pr_debug("index=%d, video=%s, preload=%lu, total=%lu, sport=%d", i, info->stalls[i].key,
			info->stalls[i].preload, info->stalls[i].total, info->stalls[i].sport);
	}
	pr_debug("print detect result-------end");
}
...
static struct himos_aweme_detect_info *himos_alloc_aweme_detect_info(int flags)
{
	...
	if (info == NULL) {
		pr_warning("there are no enough memory space for himos_alloc_aweme_detect_info");
		return NULL;
	}
...
int himos_start_aweme_detect(struct genl_info *info)
{
	...
	if (NULL == stats_info) {
		stats_info = himos_alloc_aweme_detect_info(GFP_ATOMIC);
		if (stats_info == NULL) {
			pr_warning("there is no memory when alloc himos_aweme_detect_info");
			ret = -ENOMEM;
			goto out;
		}
...
int himos_stop_aweme_detect(struct genl_info *info)
{
	...
	stats_info = (struct himos_aweme_detect_info *)himos_get_stats_info_by_uid(uid);
	if (NULL == stats_info) {
		pr_warning("stop a invalid uid:%d", uid);
		ret = -ENOENT;
		goto out;
	}
...
int himos_keepalive_aweme_detect(struct genl_info *info)
{
	...
	stats_info = (struct himos_aweme_detect_info *)himos_get_stats_info_by_uid(uid);
	if (NULL == stats_info) {
		pr_warning("keepalive a invalid uid:%d", uid);
		ret = -ENOENT;
		goto out;
	}
...
int himos_update_aweme_stall_info(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info == NULL || info->attrs == NULL ||
		...
		pr_warning("In command UPDATE, hope the UID and type attr, but they are null");
		return -ENOENT;
	}
	...
	pr_debug("update command uid=%d, type=%d, detecting=%d", uid, type, detecting);
	...
	pr_debug("cur_detect_index=%d, stats_info->cur_detect_index=%d",
...
static void himos_aweme_report_cb(unsigned long arg)
{
	...
	if (info->keepalive > KEEPALIVE_MAX) {
		pr_warning("aweme exceed the keepalive");
	...
	struct stall_info *stall, const char *msg, int len)
{
	...
	if ((loc = strnstr(msg, info->keys.ul_key6, temp))) {
		...
		if (likely(i > 0)) {
			stall->key[i] = '\0';
			ret = 0;
			pr_debug("detect the key: %s", stall->key);
		}
	}
...
static void himos_set_sport_key_dict(struct himos_aweme_detect_info *info, int req_index)
{
	...
	if (idx < info->dict_num) {
		...
		memcpy(&info->sport_key_dict[sport_index].key,
				info->stalls[req_index].key,
				STALL_KEY_MAX);
		pr_debug("update dict:(%d, %s), index:%d, req_index:%d",
				info->sport_key_dict[sport_index].sport,
	...
	memcpy(&info->sport_key_dict[info->cur_dict_index].key,
			info->stalls[req_index].key,
			STALL_KEY_MAX);
	pr_debug("add new dict: sport=%d, key=%s, cur_idx:%d, req_idx:%d",
			info->sport_key_dict[info->cur_dict_index].sport,
	...
	unsigned short sport, int *dict_index)
{
	...
	for (idx = 0; idx < info->valid_num; idx++) {
		...
		if (sport_index < 0)
			sport_index = AWEME_STALL_WINDOW - 1;
		pr_debug("before find in dict, idx:%d, INPUT spot:%d, sport:%d, key:%s",
		...
		if (info->sport_key_dict[sport_index].sport == sport ||
			...
			pr_debug("find in dict, idx:%d, info->stalls.sport:%d, info->stalls.key:%s",
	...
	struct sock *sk, const char *msg, int len)
{
	...
	case HIMOS_STATS_TYPE_AWEME:
		if (himos_paser_proxy_header(info, &stall, msg, len) < 0) {
			pr_debug("paser the aweme proxy header failed, ignore this request");
			return;
		}
		break;
	case HIMOS_STATS_TYPE_KWAI:
		if (himos_paser_kwai_proxy_header(info, &stall, msg, len) < 0) {
			pr_debug("paser the proxy kwai header failed, ignore this request");
			return;
		}
		break;
	default:
	...
	if (unlikely(i >= info->valid_num)) {
		pr_debug("NOTE: we find the proxy request, but without exist key");
		return;
	}

	pr_debug("proxy request and find the exist information(index:%d, proxy preload=%lu, preload=%lu)",
		i, stall.preload, info->stalls[i].preload);
	info->stalls[i].sport = stall.sport;
	himos_set_sport_key_dict(info, i);

	pr_debug("sport=%d, snum=%d", info->stalls[i].sport, sk->sk_num);
	...
	struct sock *sk, const char *msg, int len)
{
	...
	for (i = 0; i < info->valid_num; ++i) {
		if (!strncmp(info->stalls[i].key, key, STALL_KEY_MAX)) {
			pr_debug("found key = %s. index = %d\n", key, i);
			break;
		}
	}

	if (i >= info->valid_num) {
		pr_debug("a new local GET request, cur_index = %d", info->cur_index);
	...
	pr_debug("local request over, info->valid_num = %d, sk.sport = %d, info->cur_detect_index = %d, key = %s ",
...
#define		KEY_SHIFT	12
	...
	if (!loc) {
		pr_debug("himos_get_duration: loc is NULL");
		return;
	}
	loc += strlen(info->keys.dl_key6);
	len -= (loc - msg + KEY_SHIFT + 8);
	pr_debug("len = %d, the druation=%d, timescale=%d", len, stall->duration, stall->timescale);
	...
	stall->duration = (loc[4] << 24) |
		...
		(loc[6] << 8) |
		(loc[7] << 0);
	pr_debug("the druation=%d, timescale=%d", stall->duration, stall->timescale);
#undef KEY_SHIFT
}
...
void himos_aweme_tcp_stats(struct sock *sk, struct msghdr *old, struct msghdr *new, int inbytes, int outbytes)
{
	...
	if (inbytes > 0) {
		...
		if (!info->local_inbytes && info->local_sport == htons(sk->sk_num)) {
			...
			pr_debug("add inbytes=%u for local request(sport=%d)", inbytes, info->local_sport);
		}

		// find the index of video key in stalls array
		if (himos_is_sport_in_dict(info, htons(sk->sk_num), &sport_index)) {
			pr_debug("inbytes > 0, uid = %d, sport_index = %d, cur_detect_index = %d\n",
			...
			pr_debug("uid = %d,add inbytes = %u,total"
