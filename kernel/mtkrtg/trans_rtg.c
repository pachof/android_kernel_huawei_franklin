	...
	struct task_struct *from)
{
	...
	task->restore_params.min_util = task->cpu_cap_req.min_util;
	if (from->cpu_cap_req.min_util > task->cpu_cap_req.min_util)
		set_task_min_util(task, from->cpu_cap_req.min_util);
}
	...
	struct task_struct *from, unsigned int type)
{
	...
	if (ret < 0) {
		pr_err("[AWARE_RTG] %s set_rtg_sched task=%d grpid=%d failed\n",
			__func__, task->pid, grpid);
		task->rtg_depth = 0;
...
void trans_rtg_sched_dequeue(struct task_struct *task, unsigned int type)
{
	...
	if (ret < 0) {
		pr_err("[AWARE_RTG] %s reset_rtg_sched task=%d grpid=%d failed\n",
			__func__, task->pid, grpid);
