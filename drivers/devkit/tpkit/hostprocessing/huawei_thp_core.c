...
#if defined (CONFIG_LCD_KIT_DRIVER)
static void pm_type_switch(struct thp_core_data *cd,
	enum lcd_kit_ts_pm_type pm_type)
{
	...
	case TS_RESUME_DEVICE:
		...
		if (cd->use_ap_gesture) {
			...
			THP_LOG_ERR("resume send FINGER_UP to fp\n", __func__);
...
#endif
	pm_type_switch(cd, pm_type);
	return 0;
}
#endif

#ifdef CONFIG_HUAWEI_THP_MTK
static void thp_suspend_thread(struct thp_cmd_node *in_cmd)
{
	struct thp_core_data *cd = thp_get_core_data();
...
static void thp_resume_thread(struct thp_cmd_node *in_cmd)
{
	struct thp_core_data *cd = thp_get_core_data();
...
#ifdef CONFIG_HUAWEI_THP_MTK
		case TS_SUSPEND:
			thp_suspend_thread(proc_cmd);
			break;
		case TS_RESUME:
			thp_resume_thread(proc_cmd);
			break;
#endif
		default:
			break;
