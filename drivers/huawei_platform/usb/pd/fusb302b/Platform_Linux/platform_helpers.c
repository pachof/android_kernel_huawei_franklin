...
//extern void custom_set_vbus(int is_on);
//extern void fusb302b_call_otg(int openorclose);
//extern void control_charger_type(int chg_type, bool chg_online);
...
#define FUSB_VOLTAGE_UNIT               50
#define FUSB_CURRENT_UNIT               10
		...
		void *usr_ctx, void *app_ctx)
{
	...
	pr_info("FUSB %s - Notice, event=0x%x\n", __func__, event);
	switch (event) {
	case CC1_ORIENT:
	case CC2_ORIENT:
		...
		chip->tcpc->typec_polarity = (event == CC1_ORIENT) ? true : false;
	...
	case PD_NEW_CONTRACT:
		...
		pr_info("FUSB %s:PD_NEW_CONTRACT=0x%x, PE_ST=%d\n",
			__func__, event, chip->port.PolicyState);
		tcpci_sink_vbus(chip->tcpc, TCP_VBUS_CTRL_PD, set_voltage * FUSB_VOLTAGE_UNIT,
			op_current * FUSB_CURRENT_UNIT);
	...
	case POWER_ROLE:
		...
		pr_info("FUSB - %s:POWER_ROLE start_power_swap=%d", __func__, start_power_swap);

		if (start_power_swap == FALSE) {
			start_power_swap = true;
			type=1;
			//control_charger_type(type,1);

		} else {
			start_power_swap = false;
			type=0;//CHARGER_UNKNOWN;
			//control_charger_type(type,0);
		}

		dual_role_instance_changed(chip->dual_role);
		tcpci_notify_role_swap(chip->tcpc, TCP_NOTIFY_PR_SWAP, !(chip->port.sourceOrSink));
		break;
	case AUDIO_ACC:
		pr_info("FUSB %s:AUDIO_ACC=0x%x\n", __func__, event);
		...
		break;
	case CUSTOM_SRC:
		pr_info("FUSB %s:CUSTOM_SRC=0x%x\n", __func__, event);
		...
		break;
	case NORP_SRC_ATTACHED:
		pr_info("FUSB %s:NORP_SRC_ATTACHED=0x%x\n", __func__, event);
		tcpci_force_sink_vbus(chip->tcpc, true);
		if (chip->tcpc->typec_attach_old != TYPEC_ATTACHED_AUDIO) {
			chip->tcpc->typec_attach_new = TYPEC_ATTACHED_NORP_SRC;
			usb_state = 1;
			tcpci_notify_typec_state(chip->tcpc);
			chip->tcpc->typec_attach_old = TYPEC_ATTACHED_NORP_SRC;
			pr_info("FUSB %s:NORP_SRC_ATTACHED,new state to TYPEC_ATTACHED_NORP_SRC\n",
				__func__);
		}
		break;
	case NORP_SRC_UNATTACHED:
		pr_info("FUSB %s:NORP_SRC_UNATTACHED=0x%x\n", __func__, event);
		...
		if (chip->tcpc->typec_attach_old != TYPEC_ATTACHED_AUDIO) {
			...
			tcpci_notify_typec_state(chip->tcpc);
			chip->tcpc->typec_attach_old = TYPEC_UNATTACHED;
			pr_info("FUSB %s:NORP_SRC_UNATTACHED,new state to TYPEC_UNATTACHED\n",
				__func__);
		}
		break;
	default:
