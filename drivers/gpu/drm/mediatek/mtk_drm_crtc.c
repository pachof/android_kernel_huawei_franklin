...
void mtk_drm_crtc_dump(struct drm_crtc *crtc)
{
	...
	const struct mtk_addon_path_data *addon_path = NULL;
...
{
	struct mtk_crtc_state *state = NULL, *old_state = NULL;
...
static void save_backlight(unsigned int level)
{
	last_BL_level = level;
	return;
}

unsigned int mtk_drm_get_last_backlight(void)
{
	return last_BL_level;
}
		...
		int need_lock)
{
	...
	if (crtc_state && crtc_state->prop_val[CRTC_PROP_DOZE_ACTIVE]) {
		DDPINFO("not support this way to setbacklight in AOD mode\n");
		mutex_unlock(&mtk_crtc->lock);
		CRTC_MMP_EVENT_END(index, backlight, 1, 1);
		return -EINVAL;
	}
...
static int mtk_drm_crtc_set_panel_hbm(struct drm_crtc *crtc, bool en)
{
	...
	if (en)
		mtk_disp_ccorr_clear(comp, cmdq_handle);
	else
		mtk_disp_ccorr_restore(comp, cmdq_handle);
...
#if defined (CONFIG_LCD_KIT_DRIVER) && defined (CONFIG_MACH_MT6853)
...
static void mtk_crtc_atmoic_ddp_config(struct drm_crtc *crtc,
				...
				struct cmdq_pkt *cmdq_handle)
{
	struct mtk_lye_ddp_state *lye_state = NULL, *old_lye_state = NULL;
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	struct mtk_drm_lyeblob_ids *lyeblob_ids = NULL, *next = NULL;
	struct mtk_drm_private *mtk_drm = crtc->dev->dev_private;
	unsigned int index = drm_crtc_index(crtc);
	unsigned int crtc_mask = 0x1 << index;
...
void mtk_crtc_connect_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	struct mtk_ddp_comp *comp = NULL;
	struct drm_crtc *crtc = &mtk_crtc->base;
	struct mtk_ddp_comp **ddp_comp = NULL;
...
void mtk_crtc_config_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j) {
		...
		if (!mtk_drm_helper_get_opt(
				    priv->helper_opt,
				    MTK_DRM_OPT_USE_PQ))
			mtk_ddp_comp_bypass(comp, cmdq_handle, true);
	}
...
void mtk_crtc_disconnect_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	static bool pq_bypass_status = false;

void mtk_crtc_bypass_pq_for_aod(struct mtk_drm_crtc *mtk_crtc,
	 struct cmdq_pkt *handle, bool bypass)
{
	...
	DDPINFO("%s, pq_bypass_status=%d, bypass=%d\n",
	  __func__, pq_bypass_status, bypass);

	if (pq_bypass_status == bypass)
		return;
	...
	for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j)
		mtk_ddp_comp_bypass(comp, cmdq_handle, bypass);
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc)
{
	...
	/* Enable DISP_PQ from Doze_supend to Normal */
	if (crtc_id == 0)
		mtk_crtc_bypass_pq_for_aod(mtk_crtc, NULL, false);
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	/* Bypass DISP_PQ in Doze mode */
	if (index == 0) {
		if ((bool)state->prop_val[CRTC_PROP_DOZE_ACTIVE])
			mtk_crtc_bypass_pq_for_aod(mtk_crtc,
				cmdq_handle, true);
		else
			mtk_crtc_bypass_pq_for_aod(mtk_crtc,
				cmdq_handle, false);
	...
	mtk_crtc->have_frame_update = true;
...
static void mtk_drm_crtc_attach_property(struct drm_crtc *crtc)
{
	...
	struct mtk_drm_property *crtc_prop = NULL;
	int index = drm_crtc_index(crtc);
	int i;
	static int num;
			...
			const struct mtk_crtc_path_data *path_data)
{
	...
	unsigned int zpos;
	unsigned int pipe = priv->num_pipes;
	int ret;
	unsigned int i, j, p_mode;
