 ...
 * frame_info.c
 *
 * Frame-based load tracking for rt_frame and mtk_rtg
 *
...
struct frame_info g_frame_info;
 ...
 */
static u64 calc_frame_exec(const struct frame_info *frame_info)
{
...
/*
 * real_util:
 * max(last_util, virtual_util, boost_util, phase_util)
 */
	...
	u64 timeline)
{
	return ((frame_info_rtg(frame_info)->util_invalid_interval <= timeline) &&
		(frame_info_rtg_load(frame_info)->curr_window_exec * FRAME_UTIL_INVALID_FACTOR <= timeline));
}
