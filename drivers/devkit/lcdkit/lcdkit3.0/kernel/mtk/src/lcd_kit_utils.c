...
int lcd_kit_alpm_setting(uint32_t mode)
{
	...
	case ALPM_SINGLE_CLOCK:
		for (i = 0; i < disp_info->alpm.off_cmds.cmd_cnt; i++)
			copy_dsi_cmd_desc(&(disp_info->alpm.off_cmds.cmds[i]),
				&(disp_info->alpm.two_bit_cmds.cmds[i]));
		ret = LCD_KIT_OK;
		break;
	case ALPM_DOUBLE_CLOCK:
		for (i = 0; i < disp_info->alpm.off_cmds.cmd_cnt; i++)
			copy_dsi_cmd_desc(&(disp_info->alpm.off_cmds.cmds[i]),
				&(disp_info->alpm.one_bit_cmds.cmds[i]));
		ret = LCD_KIT_OK;
		break;
	default:
		break;
...
void lcd_kit_parse_alpm(struct device_node *np)
{
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-aod-low-brightness-cmds",
		...
		&disp_info->alpm.low_light_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-aod-on-one-bit-cmds",
		"lcd-kit,panel-aod-on-one-bit-cmds-state",
		&disp_info->alpm.one_bit_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-aod-on-two-bit-cmds",
		"lcd-kit,panel-aod-on-two-bit-cmds-state",
		&disp_info->alpm.two_bit_cmds);
