...
unsigned int min_util[NR_CPUS];
bool util_changed = false;
#endif
...
#ifdef CONFIG_HW_MTK_RTG_SCHED
		if (j < NR_CPUS && max_min_util < min_util[j])
			max_min_util = min_util[j];
#endif
		sugov_iowait_boost(j_sg_cpu, &util, &max);
	}

#ifdef CONFIG_HW_MTK_RTG_SCHED
	util = max_t(unsigned long, util, max_min_util);
#endif
...
void sugov_mark_util_change(int cpu, unsigned int flags)
{
	...
	if (flags & (RQ_SET_MIN_UTIL | RQ_ENQUEUE_MIN_UTIL | FORCE_UPDATE)) {
		if (cpu < NR_CPUS) {
			util_changed = true;
			min_util[cpu] = get_min_util(rq);
		}
	}
}
