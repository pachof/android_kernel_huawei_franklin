...
static void send_accdet_status_event(u32 cable_type, u32 status)
{
#ifdef CONFIG_HUAWEI_HEADSET_DEBUG
	if (status == 0)
		headset_debug_input_set_state(HEADSET_DEBUG_JACK_BIT_NONE,
			false);
#endif
	switch (cable_type) {
	case HEADSET_NO_MIC:
		...
		if (status == 0)
			...
			headset_debug_input_set_state(
				HEADSET_DEBUG_JACK_BIT_HEADPHONE, false);
#endif
		input_sync(accdet_input_dev);
	...
	case HEADSET_MIC:
		...
		if (status == 0)
			...
			headset_debug_input_set_state(
				HEADSET_DEBUG_JACK_BIT_HEADSET, false);
#endif
	...
	case LINE_OUT_DEVICE:
		...
		if(status)
			headset_debug_input_set_state(
				HEADSET_DEBUG_JACK_BIT_LINEOUT, false);
#endif
		pr_info("%s LineOut %s\n", __func__,
			status ? "PlugIn" : "PlugOut");
		break;
...
#ifdef CONFIG_ANA_HS
	if (!ana_hs_support_usb_sw() && (eintID == M_PLUG_OUT)) {
#else
...
cur_AB = pmic_read(PMIC_ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET;
	...
	if (cable_type == HEADSET_MIC) {
		...
		pr_info("%s more than 6s, cur_AB:0x%x c_type:0x%x, do nothing\n",
				__func__, cur_AB, cable_type);
}
#endif /* end of #if PMIC_ACCDET_KERNEL */
...
static inline u32 read_ab_reg_from_pmic(void)
{
	u32 cur_ab_val;

	cur_ab_val = pmic_read(PMIC_ACCDET_MEM_IN_ADDR);
	cur_ab_val >>= ACCDET_STATE_MEM_IN_OFFSET;
	cur_ab_val &= ACCDET_STATE_AB_MASK;

	return cur_ab_val;
}

#ifdef CONFIG_ANA_HS
static inline u32 get_ab_reg_with_recheck(void)
{
	u32 cur_ab_val;

	cur_ab_val = read_ab_reg_from_pmic();
	if (!ana_hs_support_usb_sw() || (accdet_status != PLUG_OUT))
		return cur_ab_val;
	if (cur_ab_val == ACCDET_STATE_AB_00) {
		ana_hs_mic_gnd_swap();
		pr_notice("accdet %s(), swap mic_gnd to recheck\n", __func__);
		/* delay in case of unstable state */
		mdelay(10);
		cur_ab_val = read_ab_reg_from_pmic();
	}
	return cur_ab_val;
...
#ifdef CONFIG_ANA_HS
	cur_AB = get_ab_reg_with_recheck();
#else
	cur_AB = read_ab_reg_from_pmic();
#endif
...
static void config_eint_init_by_mode(void)
{
	...
	if ((accdet_dts.eint_detect_mode == 0x1) ||
		...
		pr_info("%s eint_detect_mode = 4\n", __func__);
		pmic_write_set(PMIC_RG_EINT0HIRENB_ADDR,
			PMIC_RG_EINT0HIRENB_SHIFT); // 500k
		pmic_write_mset(PMIC_RG_EINTCOMPVTH_ADDR,
			PMIC_RG_EINTCOMPVTH_SHIFT, PMIC_RG_EINTCOMPVTH_MASK,
			0x2);  // 2V
...
static void ana_hs_plug_in_detect(void *priv)
{
	...
	/* if moisture_detect_mode is 0x5, set a timer to disable micbias */
	if (accdet_dts.moisture_detect_mode != 0x5)
...
#ifdef CONFIG_ANA_HS
	ret = ana_hs_codec_dev_register(&ana_hs_dev, dev);
	if (ret)
		pr_notice("ana_hs_codec_dev_register fail, ignore, ret %d\n", ret);
#endif
