...
static int g_rme_deadline_margin[MAX_FRAME_RATE_NUM] = {-3, -7}; // -3 and -7 is the default value of deadline margin
static int g_rme_input_margin[MAX_FRAME_RATE_NUM] = {-12, -12}; // -12 is the default value of input margin
static int g_rme_animation_margin[MAX_FRAME_RATE_NUM] = {-10, -10}; // -10 is the default value of animation margin
static int g_rme_traversal_margin[MAX_FRAME_RATE_NUM] = {-10, -10}; // -10 is the default value of traversal margin
static int g_rme_input_period[MAX_FRAME_RATE_NUM] = {3, 3}; // 3 is the default value of input timeout period
static int g_rme_animation_period[MAX_FRAME_RATE_NUM] = {4, 4}; // 4 is the default value of animation timeout period
static int g_rme_traversal_period[MAX_FRAME_RATE_NUM] = {4, 4}; // 4 is the default value of traversal timeout period
static int g_rme_default_util[MAX_FRAME_RATE_NUM] = {600, 600}; // 600 is the default value of min util
static int g_rme_frame_rate_idx = 0; // 0 represents low fps, 1 represents high fps
...
static long ctrl_frame_state(unsigned long arg, bool is_enter)
{
	...
	// if rme set frame_freq_type, translated magin to freq_type
	if (state_data.frame_freq_type == FRAME_BUFFER_3)
		freq_type = 0 - g_rme_input_margin[g_rme_frame_rate_idx];
	else if (state_data.frame_freq_type == FRAME_BUFFER_4)
		freq_type = 0 - g_rme_animation_margin[g_rme_frame_rate_idx];
	else if (state_data.frame_freq_type == FRAME_BUFFER_5)
		freq_type = 0 - g_rme_traversal_margin[g_rme_frame_rate_idx];
...
static long ctrl_set_min_util_and_margin(unsigned long arg)
{
	...
	return set_min_util_and_margin(&(state_data.head),
		util, state_data.margin + g_rme_deadline_margin[g_rme_frame_rate_idx]);
}
...
static long ctrl_set_rme_margin(unsigned long arg)
{
	...
	if ((fps_data.deadline_margin > max_margin || fps_data.deadline_margin < min_margin) ||
		(fps_data.input_margin > max_margin || fps_data.input_margin < min_margin) ||
		(fps_data.animation_margin > max_margin || fps_data.animation_margin < min_margin) ||
		(fps_data.traversal_margin > max_margin || fps_data.traversal_margin < min_margin) ||
		(fps_data.input_period < min_period) ||
		(fps_data.animation_period < min_period) ||
		(fps_data.traversal_period < min_period) ||
		(fps_data.default_util >= max_util)) {
		g_rme_deadline_margin[idx] = -3; // -3 is the default value of deadline margin
		g_rme_input_margin[idx] = -12; // -12 is the default value of input margin
		g_rme_animation_margin[idx] = -10; // -10 is the default value of animation margin
		g_rme_traversal_margin[idx] = -10; // -10 is the default value of traversal margin
		g_rme_input_period[idx] = 3; // 3 is the default value of input timeout period
		g_rme_animation_period[idx] = 4; // 4 is the default value of animation timeout period
		g_rme_traversal_period[idx] = 4; // 4 is the default value of traversal timeout period
		g_rme_default_util[idx] = 600; // 600 is the default value of min util
	} else {
...
static long ctrl_get_rme_margin(unsigned long arg)
{
	...
	if (check_rme_head(&(margin_data.head)) == SUCC) {
		idx = margin_data.fps_data.frame_rate_idx % MAX_FRAME_RATE_NUM;
		g_rme_frame_rate_idx = idx;
		margin_data.head.pid = 0;
		margin_data.fps_data.deadline_margin = g_rme_deadline_margin[idx];
		margin_data.fps_data.input_margin = g_rme_input_margin[idx];
		margin_data.fps_data.animation_margin = g_rme_animation_margin[idx];
		margin_data.fps_data.traversal_margin = g_rme_traversal_margin[idx];
		margin_data.fps_data.input_period = g_rme_input_period[idx];
		margin_data.fps_data.animation_period = g_rme_animation_period[idx];
		margin_data.fps_data.traversal_period = g_rme_traversal_period[idx];
