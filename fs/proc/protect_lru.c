...
static int protect_lru_set(struct file *filp, unsigned long arg)
#endif
{
	struct inode *inode = NULL;
	int ret = 0;
...
#elif defined(CONFIG_MEMCG_PROTECT_LRU)
unsigned long protect_max_mbytes[PROTECT_LEVELS_MAX];
unsigned long protect_cur_mbytes[PROTECT_LEVELS_MAX];
#endif
