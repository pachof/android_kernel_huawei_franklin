...
static void mtk_unreference_work(struct work_struct *work)
{
	...
	unsigned long flags = 0;
	struct mtk_atomic_state *state = NULL;
	struct mtk_atomic_state *tmp = NULL;
					 ...
					 struct drm_atomic_state *old_state)
{
	...
	struct drm_connector_state *old_conn_state = NULL;
	int i = 0;
			    ...
			    struct drm_atomic_state *state)
{
	...
	struct mtk_crtc_state *old_state = NULL;
	struct mtk_crtc_state *new_state = NULL;
	int i = 0;
	int ret = 0;
...
{
	struct mtk_atomic_state *mtk_state = NULL;
...
void drm_trigger_repaint(enum DRM_REPAINT_TYPE type, struct drm_device *drm_dev)
{
	if (type > DRM_WAIT_FOR_REPAINT && type < DRM_REPAINT_TYPE_NUM) {
		...
		struct repaint_job_t *repaint_job = NULL;
			     ...
			     struct drm_mtk_session_info *info)
{
	...
	info->vsyncFPS = DEFAULT_60FPS; // lcm_fps_ctx_get(0);
	if (!info->vsyncFPS) {
...
static int mtk_drm_kms_init(struct drm_device *drm)
{
	...
	int ret = 0;
	struct task_struct *mtk_irq_log_task = NULL;
	...
	if (IS_ERR(mtk_irq_log_task))
		DDPPR_ERR("can not create mtk_irq_log_kthread");
...
#ifdef CONFIG_LCD_KIT_DRIVER
	mtk_disp_read_ddic_switch(true);
#endif
...
static const struct drm_ioctl_desc mtk_ioctls[] = {
	...
	DRM_IOCTL_DEF_DRV(MTK_SEC_HND_TO_GEM_HND, mtk_drm_sec_hnd_to_gem_hnd,
			  DRM_UNLOCKED | DRM_AUTH | DRM_RENDER_ALLOW),
#ifdef CONFIG_LCD_KIT_DRIVER
	DRM_IOCTL_DEF_DRV(RGBW_MODE_SET_PARAM, lcm_rgbw_mode_set_param,
			DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(RGBW_MODE_GET_PARAM, lcm_rgbw_mode_get_param,
			DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(DISPLAY_ENGINE_PANEL_INFO_GET,
			lcm_display_engine_get_panel_info, DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(DISPLAY_ENGINE_INIT, lcm_display_engine_init,
			DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(HBM_MODE_SET_PARAM, panel_drm_hbm_set,
			DRM_UNLOCKED),
#endif
};
