	...
	if (*change_counter >= change_counter_threshold) {
		...
		*change_counter = 0;
		...
		if (moisture_detection_by_cc_enable) {
			...
			power_event_notify(POWER_EVENT_NE_WD_REPORT_UEVENT, &flag);
		}
	}
...
void pd_dpm_report_pd_sink_vbus(struct pd_dpm_info *di, void *data)
{
	...
	extern void hw_usb_psy_force_change_type_to_ac(void);
...
static void pd_dpm_notify_ana_hs(int new_state, int *usb_event)
{
	...
	if (new_state == PD_DPM_TYPEC_UNATTACHED) {
		if (pd_dpm_analog_hs_state == ANA_HS_PLUG_IN) {
			...
			*usb_event = PD_DPM_USB_TYPEC_AUDIO_DETACHED;
			...
			hwlog_info("%s: USB UNATTACHED\n", __func__);
			*usb_event = PD_DPM_USB_TYPEC_DETACHED;
		}
	}
}

static void pd_dpm_dr_swap(int *usb_event, void *data)
{
	...
	case PD_DPM_PE_EVT_TYPEC_STATE:
		{
			...
			pd_dpm_notify_ana_hs(typec_state->new_state, &usb_event);
			pd_dpm_set_typec_state(usb_event);
		}
		break;
