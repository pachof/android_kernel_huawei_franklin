...
struct mtk_pinctrl_state {
	struct pinctrl_state *vm_high;
	struct pinctrl_state *step_high;
	struct pinctrl_state *step_low;
	struct pinctrl_state *enable_high;
	struct pinctrl_state *enable_low;
	struct pinctrl_state *enable_input;
	struct pinctrl_state *dir_high;
	struct pinctrl_state *dir_low;
	struct pinctrl_state *decay_high;
	struct pinctrl_state *decay_low;
	struct pinctrl_state *sleep_high;
	struct pinctrl_state *sleep_low;
	struct pinctrl_state *mode1_high;
	struct pinctrl_state *mode1_low;
	struct pinctrl_state *mode1_np;
	struct pinctrl_state *st_en_high;
	struct pinctrl_state *st_en_low;
	struct pinctrl_state *st_en_np;
	struct pinctrl_state *gpio_as_int;
	struct pinctrl_state *gpio_voltage_high;
	struct pinctrl_state *gpio_voltage_low;
};
...
struct pwm_spec_config pwm_config = {
	.pwm_no = 0,
	.mode = PWM_MODE_OLD,
	...
	.PWM_MODE_OLD_REGS.WAVE_NUM = 2000,
	.PWM_MODE_OLD_REGS.DATA_WIDTH = 676,
	.PWM_MODE_OLD_REGS.THRESH = 337,
};
...
static void step_motor_set_vdd_voltage(struct drv_data *di,
	unsigned int value);
static void step_motor_set_vdd_voltage_by_gpio(struct drv_data *di,
	unsigned int value);
...
static void speed_start_control(const struct drv_data *di, bool speed_control)
{
	s64 secs;
	unsigned long nsecs;
	if (!di) {
	...
	if (speed_control) {
		if (hrtimer_active(&di->speed_control))
			hrtimer_cancel((struct hrtimer *)&di->speed_control);

		secs = di->dur_per_step_ms / SEC_MS_RATIO;
		nsecs = (di->dur_per_step_ms % SEC_MS_RATIO) * MS_NSEC_RATIO;
		hrtimer_start((struct hrtimer *)&di->speed_control,
...
#ifdef CONFIG_PWM
static void pwm_step_control(const struct drv_data *di, bool enable)
{
	int ret;
	...
	if (enable) {
		// pwm start to work
		ret = pwm_set_spec_config(&pwm_config);
		if (ret < 0)
			hwlog_err("%s : pwm set config failed, error = %d\n",
				__func__, ret);
		hwlog_info("pwm_set_config_end");
	} else {
		// disable pwm after use it
		mt_pwm_disable(pwm_config.pwm_no, pwm_config.pmic_pad);
		hwlog_info("pwm_disable_end");
	}
}
#else
...
static void drv8846_power_pinctrl(const struct drv_data *di)
{
	int ret;

	ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.vm_high);
	if (ret < 0)
		hwlog_err("%s : select vm_high fail\n", __func__);
}
...
static enum hrtimer_restart drv_speed_ctrl_timer_func(struct hrtimer *timer)
{
	...
	if (di->cur_state == STATE_SPEED_UP) {
		mt_set_pwm_disable_hal(0);
		mt_set_pwm_wave_num_hal(0, 53705);  // wave_num
		mt_set_pwm_data_width_hal(0, 312);  // plus time
		mt_set_pwm_thresh_hal(0, 156);      // half plus time
		mt_set_pwm_enable_hal(0);
		...
		mt_set_pwm_disable_hal(0);
		mt_set_pwm_wave_num_hal(0, 1920);  // wave_num
		mt_set_pwm_data_width_hal(0, 676); // plus time
		mt_set_pwm_thresh_hal(0, 337);     // half plus time
		mt_set_pwm_enable_hal(0);
...
static enum hrtimer_restart drv8846_step_timer_func(struct hrtimer *timer)
{
	...
	if (di->cur_step_value == GPIO_VAL_LOW) {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.step_low);
		if (ret < 0)
			hwlog_err("%s : set step low failed\n", __func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.step_high);
		if (ret < 0)
			hwlog_err("%s : set step high failed\n", __func__);
	}
...
static void drv8846_step_start(struct drv_data *di)
{
	...
	if (pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.step_low))
		hwlog_err("%s : step init set gpio step failed\n", __func__);
...
static void drv8846_step_stop(const struct drv_data *di)
{
	...
	if (hrtimer_active(&di->step_timer))
		hrtimer_cancel((struct hrtimer *)&di->step_timer);
	if (hrtimer_active(&di->speed_control))
		hrtimer_cancel((struct hrtimer *)&di->speed_control);
}
 ...
 * @return void.
 */
static void drv8846_set_dir_in_pinctrl(const struct drv_data *di, int dir)
{
	int ret;

	if (dir == 0) {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.dir_low);
		if (ret < 0)
			hwlog_err("%s : select dir_low fail\n", __func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.dir_high);
		if (ret < 0)
			hwlog_err("%s : select dir_high fail\n", __func__);
	}
}
 ...
 * @return void.
 */
static void drv8846_set_dir_out_pinctrl(const struct drv_data *di, int dir)
{
	int ret;
 ...
 * @return void.
 */
static void drv8846_adec_pinctrl(const struct drv_data *di, int val)
{
	int ret;
	...
	if (val == GPIO_VAL_HIGH) {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.decay_high);
		if (ret < 0)
			hwlog_err("%s : select decay_high fail\n", __func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.decay_low);
		if (ret < 0)
			hwlog_err("%s : select decay_low fail\n", __func__);
	}
}
 ...
 * @return void.
 */
static void drv8846_sleep_pinctrl(const struct drv_data *di, bool enable)
{
	int ret;

	if (enable) {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.sleep_low);
		if (ret < 0)
			hwlog_err("%s : select sleep_low fail\n", __func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.sleep_high);
		if (ret < 0)
			hwlog_err("%s : select sleep_high fail\n", __func__);
	}
}
 ...
 * @return void.
 */
static void drv8846_ti_enable_pinctrl(const struct drv_data *di, bool enable)
{
	int ret;

	if (enable) {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.enable_low);
		if (ret < 0)
			hwlog_err("%s : select enable_low fail\n", __func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.enable_high);
		if (ret < 0)
			hwlog_err("%s : select enable_high fail\n", __func__);
	}
}

static void drv8846_st_enable_pinctrl(const struct drv_data *di, bool enable)
{
	int ret;

	if (enable) {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.st_en_high);
		if (ret < 0)
			hwlog_err("%s : select st_en_high fail\n", __func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.st_en_low);
		if (ret < 0)
			hwlog_err("%s : select st_en_low fail\n", __func__);
	}
}
...
static void drv8846_enable(const struct drv_data *di, bool enable)
{
	...
	if (di->ic_vendor == ST_VENDOR)
		drv8846_st_enable_pinctrl(di, enable);
	else if (di->ic_vendor == TI_VENDOR)
		drv8846_ti_enable_pinctrl(di, enable);
}
...
static void drv8846_st_reset(struct drv_data *di)
{
	...
	ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.mode1_low);
	ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.decay_low);
	ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.step_low);
	ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.dir_low);
	if (ret != 0)
...
static void drv8846_stop_immediately(struct drv_data *di)
{
	...
	drv8846_sleep_pinctrl(di, true);
	drv8846_power_pinctrl(di);
 ...
 * @return int SUCCESS or FAILURE.
 */
static void st_set_mode_pinctrl(struct drv_data *di, enum drv_step_mode step_mode)
{
	int ret = 0;
	...
	case FULL_STEP_RISING_ONLY:
		...
		ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.dir_low);
		if (ret < 0)
			hwlog_err("%s : set full-step fail\n", __func__);
		break;
	case THIRTY_TWO_STEP_RISING_ONLY:
		...
		ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.dir_low);
		if (ret < 0)
			hwlog_err("%s : set 32-step fail\n", __func__);
		break;
	default:
		hwlog_err("%s unknown step_mode:%d\n", __func__, step_mode);
		break;
	}
}

static void ti_set_mode_pinctrl(struct drv_data *di, enum drv_step_mode step_mode)
{
	int ret = 0;
	...
	if (di->support_ti_mode_hardware_set == 1) {
		...
		ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.mode1_np);
		ret |= pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.st_en_np);
		if (ret < 0)
			hwlog_err("%s : hardware set frequercy fail\n", __func__);
		return;
	}
	switch (step_mode) {
	case FULL_STEP_RISING_ONLY:
...
static void st_start_motor(struct drv_data *di)
{
	...
	drv8846_enable(di, false);
	drv8846_power_pinctrl(di);
	st_set_mode_pinctrl(di, g_step_work_mode);
	usleep_range(2, 3); // 2us
	drv8846_sleep_pinctrl(di, false);
	usleep_range(100, 200); // 100us
	if (di->cur_direction == DIR_ROTATE_IN)
		drv8846_set_dir_in_pinctrl(di, di->direction);
	else if (di->cur_direction == DIR_ROTATE_OUT)
		drv8846_set_dir_out_pinctrl(di, di->direction);
	else
...
static void ti_start_motor(const struct drv_data *di)
{
	...
	drv8846_enable(di, false);
	drv8846_power_pinctrl(di);
	ti_set_mode_pinctrl((struct drv_data *)di, g_step_work_mode);

	mdelay(di->power_on_delay_ms);
	drv8846_adec_pinctrl(di, GPIO_VAL_HIGH);
	if (di->cur_direction == DIR_ROTATE_IN)
		drv8846_set_dir_in_pinctrl(di, di->direction);
	else if (di->cur_direction == DIR_ROTATE_OUT)
		drv8846_set_dir_out_pinctrl(di, di->direction);
	...
	drv8846_enable(di, true);
	drv8846_step_start((struct drv_data *)di);
}
...
static int drv8846_get_pinctrl_gpio_config(struct drv_data *di)
{
	int ret = 0;

	// vm
	di->mtk_pinctrl.vm_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_vm_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.vm_high)) {
		hwlog_err("can not lookup gpio_vm_high pinstate\n");
		return -EINVAL;
	}
	drv8846_power_pinctrl(di);

	// mode1
	di->mtk_pinctrl.mode1_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_mode1_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.mode1_high)) {
		hwlog_err("can not lookup gpio_mode1_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.mode1_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_mode1_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.mode1_low)) {
		hwlog_err("can not lookup gpio_mode1_low pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.mode1_np =
		pinctrl_lookup_state(di->pinctrl, "gpio_mode1_np");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.mode1_np)) {
		hwlog_err("can not lookup gpio_mode1_np pinstate\n");
		return -EINVAL;
	}

	// ti--decay st -- mode2
	di->mtk_pinctrl.decay_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_decay_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.decay_high)) {
		hwlog_err("can not lookup gpio_decay_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.decay_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_decay_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.decay_low)) {
		hwlog_err("can not lookup gpio_decay_low pinstate\n");
		return -EINVAL;
	}

	// step
	di->mtk_pinctrl.step_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_step_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.step_high)) {
		hwlog_err("can not lookup step_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.step_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_step_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.step_low)) {
		hwlog_err("can not lookup gpio_step_low pinstate\n");
		return -EINVAL;
	}

	// dir
	di->mtk_pinctrl.dir_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_dir_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.dir_high)) {
		hwlog_err("can not lookup gpio_dir_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.dir_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_dir_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.dir_low)) {
		hwlog_err("can not lookup gpio_dir_low pinstate\n");
		return -EINVAL;
	}

	// ti--enable
	di->mtk_pinctrl.enable_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_enable_normal_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.enable_high)) {
		hwlog_err("can not lookup gpio_enable_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.enable_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_enable_normal_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.enable_low)) {
		hwlog_err("can not lookup gpio_enable_low pinstate\n");
		return -EINVAL;
	}

	// enable-input
	di->mtk_pinctrl.enable_input =
		pinctrl_lookup_state(di->pinctrl, "gpio_enable_input");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.enable_input)) {
		hwlog_err("can not lookup gpio_enable_input pinstate\n");
		return -EINVAL;
	}

	// st--enable
	di->mtk_pinctrl.st_en_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_st_en_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.st_en_high)) {
		hwlog_err("can not lookup gpio_st_en_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.st_en_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_st_en_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.st_en_low)) {
		hwlog_err("can not lookup gpio_st_en_low pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.st_en_np =
		pinctrl_lookup_state(di->pinctrl, "gpio_st_en_np");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.st_en_np)) {
		hwlog_err("can not lookup gpio_st_en_np pinstate\n");
		return -EINVAL;
	}

	ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.st_en_low);
	if (ret < 0)
		hwlog_err("gpio_fault init failed\n");

	// sleep
	di->mtk_pinctrl.sleep_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_sleep_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.sleep_high)) {
		hwlog_err("can not lookup gpio_sleep_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.sleep_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_sleep_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.sleep_low)) {
		hwlog_err("can not lookup gpio_sleep_low pinstate\n");
		return -EINVAL;
	}

	// gpio_fault_int
	di->mtk_pinctrl.gpio_as_int =
		pinctrl_lookup_state(di->pinctrl, "gpio_fault_int");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.gpio_as_int)) {
		hwlog_err("can not lookup gpio_fault_int pinstate\n");
		return -EINVAL;
	}
	ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.gpio_as_int);
	if (ret < 0)
		hwlog_err("gpio_fault init failed\n");

	// gpio_voltage
	di->mtk_pinctrl.gpio_voltage_high =
		pinctrl_lookup_state(di->pinctrl, "gpio_voltage_high");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.gpio_voltage_high)) {
		hwlog_err("can not lookup gpio_voltage_high pinstate\n");
		return -EINVAL;
	}
	di->mtk_pinctrl.gpio_voltage_low =
		pinctrl_lookup_state(di->pinctrl, "gpio_voltage_low");
	if (IS_ERR_OR_NULL(di->mtk_pinctrl.gpio_voltage_low)) {
		hwlog_err("can not lookup gpio_voltage_low pinstate\n");
		return -EINVAL;
	}
 ...
 */
static bool drv8846_handle_gpio_set(struct drv_data *di,
	const char *name, int val)
{
	...
	if (strcmp("gpio_enable", name) == 0) {
		...
		drv8846_sleep_pinctrl(di, value == GPIO_VAL_LOW);
		...
		drv8846_adec_pinctrl(di, value);
	} else if (strcmp("gpio_mode", name) == 0) {
		st_set_mode_pinctrl((struct drv_data *)di, g_step_work_mode);
	} else if (strcmp("gpio_dir", name) == 0) {
		drv8846_set_dir_in_pinctrl(di, value);
	} else if (strcmp("gpio_vm", name) == 0) {
		drv8846_power_pinctrl(di);
	} else if (strcmp("gpio_on", name) == 0) {
		if (value == 1)
			di->step_gen_manner = PWM_STEP;
		else
			di->step_gen_manner = GPIO_STEP;
	} else {
		handled = false;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strcmp("set_voltage", name) == 0) {
		...
		step_motor_set_vdd_voltage_by_gpio(di, val);
		mutex_unlock(&di->cmd_mutex);
		return count;
	}
...
static int find_vendor_by_gpio(struct drv_data *di)
{
	...
	ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.enable_input);
	if (ret < 0) {
...
static int drv8846_fault_init(struct drv_data *di)
{
	int ret;
	struct device_node *np = NULL;
	...
	if (drv_request_gpio(di, "gpio_enable", &di->gpio_enable) != SUCCESS)
		return FAILURE;
	if (drv_request_gpio(di, "gpio_voltage", &di->gpio_voltage) != SUCCESS)
		return FAILURE;

	di->irq_fault = irq_of_parse_and_map(np, 0); // get irq number
...
static void set_big_force_by_gpio(struct drv_data *di, unsigned int value)
{
	int ret;

	if (value == 1) {
		hwlog_info("%s current is big force, no need set\n", __func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl,
			di->mtk_pinctrl.gpio_voltage_high);
		if (ret < 0)
			hwlog_err("set small force fail\n");
		hwlog_info("set small force success\n");
	}
}

static void set_small_force_by_gpio(struct drv_data *di, unsigned int value)
{
	int ret;

	if (value == 0) {
		hwlog_info("%s current is small force, no need set\n",
			__func__);
	} else {
		ret = pinctrl_select_state(di->pinctrl,
			di->mtk_pinctrl.gpio_voltage_low);
		if (ret < 0)
			hwlog_err("set big force fail\n");
		hwlog_info("set big force success\n");
	}
}
...
static void step_motor_set_vdd_voltage_by_gpio(struct drv_data *di,
					       unsigned int value)
{
	int voltage_gpio_value;

#ifdef STEP_MOTOR_DEBUG
	hwlog_info("%s factory not support change voltage\n", __func__);
	return;
#endif
	...
	if (value >= VOLTAGE_TYPE_NUM) {
		hwlog_err("%s error, value = %u\n", __func__, value);
		return;
	}

	if (!(di->select_voltage_by_gpio)) {
		hwlog_info("not support select_voltage_by_gpio\n");
		return;
	}

	hwlog_info("%s voltage value = %u\n", __func__, value);
	// value : 0 -- small force, value : 1 -- big force
	// voltage_gpio_value : big force pull down, small force pull high
	voltage_gpio_value = gpio_get_value_cansleep(di->gpio_voltage);
	switch (voltage_gpio_value) {
	case GPIO_VAL_LOW : /* current is big force */
		set_big_force_by_gpio(di, value);
		break;
	case GPIO_VAL_HIGH : /* current is small force */
		set_small_force_by_gpio(di, value);
		break;
	default :
		hwlog_info("%s : voltage_gpio_value error\n", __func__);
		break;
	}
	hwlog_info("%s : success\n", __func__);
}
...
static void step_motor_get_vdd_by_gpio(struct drv_data *di)
{
	int ret;
	u32 tmp = 0;
	struct device_node *np = NULL;
	...
	np = di->pdev->of_node;
	ret = of_property_read_u32(np, "select_voltage_by_gpio", &tmp);
	if (ret != 0) {
		hwlog_err("read %s fail\n", __func__);
		di->select_voltage_by_gpio = false;
		return;
	}

	di->select_voltage_by_gpio = (bool)tmp;
	if (!(di->select_voltage_by_gpio)) {
		hwlog_info("not support select_voltage_by_gpio\n");
		return;
	}
	// default pull high is small force
	ret = pinctrl_select_state(di->pinctrl, di->mtk_pinctrl.gpio_voltage_high);
	if (ret < 0)
		hwlog_err("%s : set default small force fail\n", __func__);
...
static int drv8846_probe(struct platform_device *pdev)
{
	...
	if (drv8846_get_pinctrl_gpio_config(di) != SUCCESS)
		return -EFAULT;
	if (drv8846_fault_init(di) != SUCCESS)
		return -EFAULT;
...
static int drv8846_remove(struct platform_device *pdev)
{
	...
	devm_gpio_free(di->pdev, di->gpio_voltage);
