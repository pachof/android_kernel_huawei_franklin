...
#include "../inc/cust_mt6360_pmu_dsm.h"
...
/******************************************************************************
  check fled short
 *****************************************************************************/
static void cust_mt6360_fled_check_dsm(struct mt6360_pmu_fled_info *fi)
{
	int reg_read;
	if (fi == NULL) {
		pr_err("%s fi = NULL\n", __func__);
	...
	reg_read = mt6360_pmu_reg_read(fi->mpi, MT6360_PMU_FLED_IRQ1);
	if (reg_read < 0) {
		pr_err("%s:mt6360_pmu_reg_read MT6360_PMU_FLED_IRQ1 failed\n", __func__);
	} else {
		if (reg_read & FLED1_SHORT) {
			flash_report_dsm_err(DSM_FLASH_OPEN_SHOTR_ERROR_NO,
					     "mt6360 flash led1 short\n");
			pr_err("%s mt6360_pmu_fled1_short report dmd\n", __func__);
		}
		if (reg_read & FLED2_SHORT) {
			flash_report_dsm_err(DSM_FLASH_OPEN_SHOTR_ERROR_NO,
					     "mt6360 flash led2 short\n");
			pr_err("%s mt6360_pmu_fled2_short report dmd\n", __func__);
		}
	}
	return;
}
