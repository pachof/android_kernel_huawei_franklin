...
static void bat_fault_notify(enum coul_interface_event event)
{
	struct bat_fault_device *di = g_bat_fault_dev;
...
static void bat_fault_cutoff_vol_event_handle(struct bat_fault_device *di)
{
	...
	bat_fault_notify(COUL_EVENT_LOW_VOL);
	...
	unsigned long event, void *data)
{
	...
	hwlog_info("recive coul event=%lu\n", event);
	return NOTIFY_OK;
}
...
{
	struct bat_fault_device *di = (struct bat_fault_device *)dev_data;
...
static int bat_fault_probe(struct platform_device *pdev)
{
	...
	ret = coul_interface_event_notifier_register(&di->coul_event_nb);
	if (ret) {
		hwlog_err("coul_interface_event_notifier_register fail\n");
		goto fail_free_mem;
	}
...
static int bat_fault_remove(struct platform_device *pdev)
{
	...
	coul_interface_event_notifier_unregister(&di->coul_event_nb);
	kfree(di);
	g_bat_fault_dev = NULL;
	return 0;
}
