					...
					unsigned long event, void *data)
{
	...
	if (!g_psy)
		g_psy = power_supply_get_by_name("charger");
	if (!g_psy) {
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if ((pd_sink_voltage_new != pd_sink_voltage_old) ||
			...
			if (pd_sink_voltage_new) {
				...
				if (tcpm_inquire_typec_attach_state(tcpc_dev) ==
					TYPEC_ATTACHED_AUDIO) {
					propval.intval = true;
					ret = power_supply_set_property(g_psy,
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			if (tcpm_inquire_vbus_level(tcpc_dev, true) >=
				TCPC_VBUS_VALID) {
				propval.intval = true;
				ret = power_supply_set_property(g_psy,
					POWER_SUPPLY_PROP_ONLINE,
	...
	case TCP_NOTIFY_PLUG_OUT:
		...
		break;
	case TCP_NOTIFY_VBUS_CC_SHORT:
		mtk_set_cc1_cc2_status(noti->vbus_cc_short.cc1, noti->vbus_cc_short.cc2);
		pr_err("%s vbus cc short(cc1,cc2)=(%d, %d)\n", __func__,
			noti->vbus_cc_short.cc1, noti->vbus_cc_short.cc2);
		break;
	default:
		break;
	};
