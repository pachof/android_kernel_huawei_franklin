...
int coul_interface_get_battery_last_capacity(int type)
{
	...
	if (l_ops && l_ops->get_battery_last_capacity)
		return l_ops->get_battery_last_capacity();
...
int coul_interface_set_battery_last_capacity(int type, int capacity)
{
	...
	if (l_ops && l_ops->set_battery_last_capacity)
		return l_ops->set_battery_last_capacity(capacity);
...
int coul_interface_get_battery_voltage(int type)
{
	...
	if (l_ops && l_ops->get_battery_voltage)
		return l_ops->get_battery_voltage();
...
int coul_interface_get_battery_current(int type)
{
	...
	if (l_ops && l_ops->get_battery_current)
		return l_ops->get_battery_current();
...
int coul_interface_get_battery_avg_current(int type)
{
	...
	if (l_ops && l_ops->get_battery_avg_current)
		return l_ops->get_battery_avg_current();
...
int coul_interface_get_battery_temperature(int type)
{
	...
	if (l_ops && l_ops->get_battery_temperature)
		return l_ops->get_battery_temperature();
...
int coul_interface_get_battery_cycle(int type)
{
	...
	if (l_ops && l_ops->get_battery_cycle)
		return l_ops->get_battery_cycle();
...
int coul_interface_set_vterm_dec(int type, int vterm_dec)
{
	...
	if (l_ops && l_ops->set_vterm_dec)
		return l_ops->set_vterm_dec(vterm_dec);
...
int coul_interface_set_battery_low_voltage(int type, int val)
{
	...
	if (l_ops && l_ops->set_battery_low_voltage)
		return l_ops->set_battery_low_voltage(val);

	return -EPERM;
}

int coul_interface_event_notifier_register(struct notifier_block *nb)
{
	struct coul_interface_dev *l_dev = g_coul_interface_dev;

	if (!nb || !l_dev)
		return NOTIFY_OK;

	return blocking_notifier_chain_register(&l_dev->event_notifier, nb);
}

int coul_interface_event_notifier_unregister(struct notifier_block *nb)
{
	struct coul_interface_dev *l_dev = g_coul_interface_dev;
	...
	return blocking_notifier_chain_unregister(&l_dev->event_notifier, nb);
}

void coul_interface_event_notify(unsigned long event, void *data)
{
	struct coul_interface_dev *l_dev = g_coul_interface_dev;
	...
	blocking_notifier_call_chain(&l_dev->event_notifier, event, data);
}
...
static int __init coul_interface_init(void)
{
	...
	BLOCKING_INIT_NOTIFIER_HEAD(&l_dev->event_notifier);
