...
#define REG_OC_DEBUG(_name, _dmd) \
{ \
	.name = #_name, \
	.dmd = _dmd, \
}

#define MD_REG_OC_DEBUG(_name, _dmd) \
{ \
	.name = #_name, \
	.is_md_reg = true, \
	.dmd = _dmd, \
...
static struct reg_oc_debug_t reg_oc_debug[] = {
	REG_OC_DEBUG(vpu, DSM_PMU_VPU_OVER_LOAD),
	MD_REG_OC_DEBUG(vcore, DSM_PMU_VCORE_OVER_LOAD),
	REG_OC_DEBUG(vgpu11, DSM_PMU_VGPU11_OVER_LOAD),
	MD_REG_OC_DEBUG(vmodem, DSM_PMU_VMODEM_OVER_LOAD),
	REG_OC_DEBUG(vproc1, DSM_PMU_VPROC1_OVER_LOAD),
	REG_OC_DEBUG(vproc2, DSM_PMU_VPROC2_OVER_LOAD),
	REG_OC_DEBUG(vs1, DSM_PMU_VS1_OVER_LOAD),
	REG_OC_DEBUG(vs2, DSM_PMU_VS2_OVER_LOAD),
	MD_REG_OC_DEBUG(vpa, DSM_PMU_VPA_OVER_LOAD),
	MD_REG_OC_DEBUG(vfe28, DSM_PMU_VFE28_OVER_LOAD),
	REG_OC_DEBUG(vxo22, 0),
	MD_REG_OC_DEBUG(vrf18, DSM_PMU_VRF18_OVER_LOAD),
	REG_OC_DEBUG(vrf12, DSM_PMU_VRF12_OVER_LOAD),
	REG_OC_DEBUG(vefuse, DSM_PMU_VEFUSE_OVER_LOAD),
	REG_OC_DEBUG(vcn33_1_bt, DSM_PMU_VCN33_1_OVER_LOAD),
	REG_OC_DEBUG(vcn33_2_bt, DSM_PMU_VCN33_2_OVER_LOAD),
	REG_OC_DEBUG(vcn13, DSM_PMU_VCN13_OVER_LOAD),
	REG_OC_DEBUG(vcn18, DSM_PMU_VCN18_OVER_LOAD),
	REG_OC_DEBUG(va09, DSM_PMU_VA09_OVER_LOAD),
	REG_OC_DEBUG(va12, DSM_PMU_VA12_OVER_LOAD),
	REG_OC_DEBUG(vaux18, DSM_PMU_VAUX18_OVER_LOAD),
	REG_OC_DEBUG(vaud18, DSM_PMU_VAUD18_OVER_LOAD),
	REG_OC_DEBUG(vio18, DSM_PMU_VIO18_OVER_LOAD),
	REG_OC_DEBUG(vsram_proc1, DSM_PMU_VSRAM_PROC1_OVER_LOAD),
	REG_OC_DEBUG(vsram_proc2, DSM_PMU_VSRAM_PROC2_OVER_LOAD),
	REG_OC_DEBUG(vsram_others, DSM_PMU_VSRAM_OTHERS_OVER_LOAD),
	REG_OC_DEBUG(vsram_md, DSM_PMU_VSRAM_MD_OVER_LOAD),
	REG_OC_DEBUG(vemc, DSM_PMU_VEMC_OVER_LOAD),
	REG_OC_DEBUG(vusb, DSM_PMU_VUSB_OVER_LOAD),
	REG_OC_DEBUG(vrfck, 0),
	REG_OC_DEBUG(vbif28, DSM_PMU_VBIF28_OVER_LOAD),
	REG_OC_DEBUG(vio28, DSM_PMU_VIO28_OVER_LOAD),
	REG_OC_DEBUG(vm18, DSM_PMU_VM18_OVER_LOAD),
	REG_OC_DEBUG(vufs, DSM_PMU_VUFS_OVER_LOAD),
};
			       ...
			       void *unused)
{
	...
	if (reg_oc_dbg->dmd) {
		snprintf(tmp_buf, sizeof(tmp_buf) - 1,
			"%s over-load happened\n", reg_oc_dbg->name);
		power_dsm_dmd_report(POWER_DSM_PMU_OCP,
			reg_oc_dbg->dmd, tmp_buf);
	}
