...
static int direct_charge_get_bat_voltage(int *value)
{
	...
	if (l_di->use_higher_vbat && (l_di->cur_stage >= (DOUBLE_SIZE * l_di->stage_size - 2)))
		btb_vol = dc_get_bat_max_btb_voltage_witc_comp(l_di->working_mode, l_di->cur_mode,
			l_di->comp_para.vbat_comp);
	else
		btb_vol = dc_get_bat_btb_voltage_with_comp(l_di->working_mode, l_di->cur_mode,
			l_di->comp_para.vbat_comp);
...
int direct_charge_get_bat_current(int *value)
{
	...
	if (dc_get_bat_current(l_di->working_mode, l_di->cur_mode, value)) {
		direct_charge_fill_eh_buf(l_di->dsm_buff,
			sizeof(l_di->dsm_buff),
			DC_EH_GET_IBAT, NULL);
		direct_charge_set_stop_charging_flag(1);
	...
	dc_get_bat_current_with_comp(value);
...
int direct_charge_init_adapter_and_device(int flag)
{
	...
	if (!strstr(saved_command_line, "androidboot.swtype=factory") && (flag == LVC_MODE))
	...
	if (dc_ic_init(l_di->working_mode, l_di->cur_mode)) {
		snprintf(tmp_buf, sizeof(tmp_buf), "device ls init fail\n");
		goto fail_init;
	}

	if (dc_batinfo_init(l_di->working_mode, l_di->cur_mode)) {
...
extern int huawei_get_vbat_max(void);
...
int direct_charge_open_charging_path(void)
{
	...
	int ibus = 0;
	int soc;
	int vbat_max = huawei_get_vbat_max();
	int max_dc_bat_vol;
	...
	if (dc_ic_enable(l_di->working_mode, l_di->cur_mode, DC_IC_ENABLE)) {
...
static void direct_charge_select_comp_param(struct direct_charge_device *di)
{
	...
	if (di->multi_ic_mode_para.support_multi_ic) {
		main_ic_id = dc_get_ic_id(di->working_mode, CHARGE_IC_MAIN);
		aux_ic_id = dc_get_ic_id(di->working_mode, CHARGE_IC_AUX);
		direct_charge_get_vbat_comp_para(main_ic_id, aux_ic_id, &di->comp_para);
	}
}
...
void direct_charge_select_charging_param(void)
{
	...
	if (l_di->cc_unsafe_sc_enable && !l_di->cc_safe) {
		cur_th_high = cur_th_high > CC_UNSAFE_MAX_CURRENT ? CC_UNSAFE_MAX_CURRENT : cur_th_high;
	}
...
static void direct_charge_change2singlepath(struct direct_charge_device *di)
{
	...
	while (count) {
		if (di->force_single_path_flag)
			break;
		if (dc_get_ic_ibus(di->working_mode, di->cur_mode, &ibus))
			return;
...
static void direct_charge_select_charge_path(struct direct_charge_device *di)
{
	...
	if (ibus >= ((di->multi_ic_ibat_th + di->curr_offset) / di->dc_volt_ratio))
		direct_charge_change2multipath(di);
	else if (ibus <= ((di->multi_ic_ibat_th - di->curr_offset) / di->dc_volt_ratio))
		direct_charge_change2singlepath(di);
}
...
static void direct_charge_update_multi_info(struct direct_charge_device *di)
{
	...
	for (i = 0; i < CHARGE_IC_MAX_NUM; i++) {
		...
		di->curr_info.vbat[i] = dc_get_bat_btb_voltage(di->working_mode, BIT(i));
...
static void direct_charge_update_single_info(struct direct_charge_device *di)
{
	...
	di->curr_info.vbat[0] = dc_get_bat_btb_voltage(di->working_mode, CHARGE_IC_MAIN);
}
...
static void direct_charge_get_ffc_para(struct direct_charge_device *di)
{
	...
	vbat_main = dc_get_bat_btb_voltage_with_comp(CHARGE_IC_MAIN, di->cur_mode, di->comp_para.vbat_comp);
	vbat_aux = dc_get_bat_btb_voltage_with_comp(CHARGE_IC_AUX, di->cur_mode, di->comp_para.vbat_comp);
...
void direct_charge_control_work(struct work_struct *work)
{
	...
	if (mulit_ic_check(di->working_mode, di->cur_mode, &di->multi_ic_check_info) < 0) {
		di->multi_ic_error_cnt++;
		direct_charge_set_stop_charging_flag(1);
		direct_charge_stop_charging();
		return;
...
void direct_charge_start_charging(void)
{
	...
	if (dc_config_ic_watchdog(l_di->working_mode, l_di->cur_mode, WATCHDOG_TIMEOUT))
		return;
...
void cam_stop_charging(void)
{
	...
	if ((scp_adp_mode & SC_MODE) != SC_MODE)
		return;

	ret = dc_ic_enable(l_di->working_mode, CHARGE_IC_MAIN, 0);
	if (ret)
		hwlog_err("ls enable fail\n");

	direct_charge_set_adapter_voltage(5000); /* reset to 5000mv */
	ret = adapter_set_output_mode(ADAPTER_PROTOCOL_SCP, 0); /* reset adapter to normal mode */
	if (ret)
...
void direct_charge_stop_charging(void)
{
	...
	if (dc_ic_enable(l_di->working_mode, l_di->cur_mode, 0))
		hwlog_err("ls enable fail\n");
	...
	if (dc_ic_discharge(l_di->working_mode,  l_di->cur_mode, 1))
	...
	if (dc_ic_discharge(l_di->working_mode, l_di->cur_mode, 0))
	...
	if (dc_batinfo_exit(l_di->working_mode, l_di->cur_mode))
