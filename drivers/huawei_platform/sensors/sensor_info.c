...
#define CP_TSL2591_DEFAULT_PARAM_INDEX 30
#define CP_SYH399_DEFAULT_PARAM_INDEX  38
...
static struct alsps_params param_to_scp[] = {
	 ...
	 */
	{ PRODUCT_AGM3, "", "sy3079", BLACK,
		{ PARAM_CHECK, 248, 900, 4000, 4000, 1120, 1000, 1000, 15184, 25000, 100 } },
	{ PRODUCT_AGM3, "", "sy3079", WHITE,
		{ PARAM_CHECK, 166, 2500, 7000, 7000, 1188, 1000, 1000, 19120, 25000, 100 } },
	 ...
	 * middle_als_data, offset_max, offset_min }
	 */
	{ PRODUCT_AGM3, "", "stk3235", BLACK,
		{ PARAM_CHECK, 3000, 2000, 1002, 1000, 1001, 3864, 25000, 100 } },
	{ PRODUCT_AGM3, "", "stk3235", WHITE,
		{ PARAM_CHECK, 2250, 1800, 600, 600, 600, 5916, 25000, 100 } },
	 ...
	 */
	{ PRODUCT_AGM3, "", "ltr578_l", BLACK,
		{ PARAM_CHECK, 600, 200, 1416, 1416, 1416, 0, 993, 1410, 25000, 100 } },
	{ PRODUCT_AGM3, "", "ltr578_l", WHITE,
		{ PARAM_CHECK, 600, 200, 1200, 1200, 1200, 0, 993, 1410, 25000, 100 } },
...
static void get_als_param(uint16_t *param, uint32_t size)
{
	...
	if (!strcmp(param_to_scp[STK3338_DEFAULT_PARAM_INDEX].als_type, als_type)) {
	...
	} else if (!strcmp(param_to_scp[TSL2591_DEFAULT_PARAM_INDEX].als_type, als_type)) {
		ret = memcpy_s(param, ALS_PARAM_LEN * sizeof(uint16_t),
			param_to_scp[TSL2591_DEFAULT_PARAM_INDEX].params,
			ALS_PARAM_LEN * sizeof(uint16_t) - 1);
		if (ret)
			pr_err("%s : memcpy fail", __func__);
		else
			pr_info("%s : use als_s001_001 default param\n", __func__);
	} else if (!strcmp(param_to_scp[SYH399_DEFAULT_PARAM_INDEX].als_type, als_type)) {
		ret = memcpy_s(param, ALS_PARAM_LEN * sizeof(uint16_t),
			param_to_scp[SYH399_DEFAULT_PARAM_INDEX].params,
			ALS_PARAM_LEN * sizeof(uint16_t) - 1);
		if (ret)
			pr_err("%s : memcpy fail", __func__);
		else
			pr_info("%s : use als_s004_001 default param\n", __func__);
	} else if (!strcmp(param_to_scp[CP_TSL2591_DEFAULT_PARAM_INDEX].als_type, als_type)){
		ret = memcpy_s(param, ALS_PARAM_LEN * sizeof(uint16_t),
		...
		if (ret)
			...
			pr_info("%s : use als_s001_001 default param\n", __func__);
	} else if (!strcmp(param_to_scp[CP_SYH399_DEFAULT_PARAM_INDEX].als_type, als_type)){
		ret = memcpy_s(param, ALS_PARAM_LEN * sizeof(uint16_t),
		...
		if (ret)
			...
			pr_info("%s : use als_s004_001 default param %d\n", __func__, ret);
	} else {
...
static void ps_ldo_regulator_init(void)
{
	...
	pr_info("%s: ++\n", __func__);
	/* Get ldo regulator handler */
	regu_ldo = regulator_get(NULL, "vldo28");
...
static int get_acc_direction(void)
{
	...
	if (direction_node == NULL) {
		...
		ret = of_property_read_u32(direction_node, "acc_direction",
			&direction);
		if (!ret) {
...
static void scp_init_done_work(struct work_struct *work)
{
	...
	if (product_type == PRODUCT_PPA) {
		pr_err("%s : get product type  %d", __func__, product_type);
		ps_ldo_regulator_init();
	}
...
static void get_als_param_work(struct work_struct *work)
{
	...
	ret = memcpy_s(sensor_para->als_para.als_extend_para,
