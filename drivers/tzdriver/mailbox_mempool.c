...
/* get_order(MAILBOX_POOL_SIZE) will compile error, it is the same with digital 10 */
...
static void add_max_order_block(unsigned int idex)
{
	 ...
	 */
	self = &(m_zone.pages[0]);
	list_add_tail(&(self->node), &(m_zone.free_areas[MAILBOX_ORDER_MAX].page_list));
}
...
{
	__free_pages(m_zone.all_pages, MAILBOX_ORDER_MAX);
