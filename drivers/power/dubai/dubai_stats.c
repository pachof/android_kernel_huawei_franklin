...
static const char *hisi_rtc_wakeup_source[] = {"RTC0", "rtc0"};
...
static int dubai_get_all_sensor_time(void __user *argp, size_t count, size_t packets)
{
	...
	length = count * sizeof(struct sensor_time);
...
int dubai_get_swing_data(void __user *argp)
{
	...
	ret = iomcu_dubai_log_fetch(DUBAI_EVENT_SWING, stat->data, sizeof(struct swing_data));
	if (ret < 0) {
...
int dubai_get_tp_duration(void __user *argp)
{
	...
	ret = iomcu_dubai_log_fetch(DUBAI_EVENT_TP, stat->data, sizeof(struct tp_duration));
	if (ret < 0) {
...
static bool is_hisi_rtc_wakeup(const struct dubai_wakeup_entry *entry)
{
	...
	if ((dubai_rtc_timer.pid > 0) && !strcmp(entry->tag, KERNEL_WAKEUP_TAG)) {
...
static bool is_mtk_rtc_wakeup(const struct dubai_wakeup_entry *entry)
{
		...
		if ((dubai_rtc_timer.pid > 0) && !!atomic_read(&rtc_irq_state) && !strcmp(entry->tag, AP_IRQ_TAG)) {
