...
#define VBAT_CURRENT_MAX 3000000
#define VBAT_VOLTAGE_MAX 4400000
#define MTK_BATT_VOLT_COMPENSATION 4000
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
		set_power_supply_health(data->BAT_batt_temp, &(val->intval));
		break;
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		set_low_capacity_hltherm(data->BAT_CAPACITY, &(val->intval));
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		val->intval = data->BAT_batt_vol * 1000 + MTK_BATT_VOLT_COMPENSATION;
		break;
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = gm.tbat_precise;
		set_ui_batt_temp_hltherm(data->BAT_batt_temp, &(val->intval));
		break;
	case POWER_SUPPLY_PROP_RESISTANCE:
		val->intval = fg_cust_data.r_fg_value;
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = charger_manager_get_charge_full_design(gm.pbat_consumer);
		break;
#ifdef CONFIG_HUAWEI_CHARGER
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = VBAT_CURRENT_MAX;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = VBAT_VOLTAGE_MAX;
		break;
	case POWER_SUPPLY_PROP_FCP_STATUS:
 		val->intval = huawei_battery_get_fcp_status();
 		break;
 	case POWER_SUPPLY_PROP_SCP_STATUS:
 		val->intval = huawei_battery_get_scp_status();
...
void battery_update_sync(struct battery_data *bat_data)
{
	struct power_supply *bat_psy = bat_data->psy;
...
static ssize_t show_counter_quse(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	signed int counter_quse;

	/* 1000:soc convert, 10000:aging factor convert */
	counter_quse = ((gm.soc * gm.algo_qmax) / 1000) * gm.aging_factor / 10000;
	bm_info("%s: %d\n", __func__, counter_quse);

	return sprintf(buf, "%d\n", counter_quse);
}

static ssize_t store_counter_quse(struct device *dev,
...
static DEVICE_ATTR(Counter_quse, 0664, show_counter_quse, store_counter_quse);
...
int force_get_tbat_internal(bool update)
{
	...
	bat_temperature_val = set_ntc_compensation_temp(bat_temperature_val,
		fg_current_state, fg_current_temp);
...
static ssize_t show_reset_fuelgauge_daemon(struct device *dev,
	struct device_attribute *attr, char *buf)
{
...
static ssize_t store_reset_fuelgauge_daemon(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static int mt6359_set_current_gain(unsigned int cur_a)
{
	int val;

	val = fg_cust_data.car_tune_value;
	val = ((s64)val * cur_a) / 1000000;

	fg_cust_data.car_tune_value = val;
	return 0;
}

static struct power_bat_cali_ops mt6359_cali_ops = {
	.dev_name = "mt6359",
	.set_current_gain = mt6359_set_current_gain,
};
/*************************************/
...
/*****************************/
	...
	/* update car_tune_value through power cali */
	power_cali_ops_register(&mt6359_cali_ops);
	fg_cust_data.car_tune_value = power_cali_cur_tune_update(fg_cust_data.car_tune_value);
	/* Power supply class */
#if !defined(CONFIG_MTK_DISABLE_GAUGE)
	battery_main.psy =
...
#endif
	/* add for battery remaining capacity */
	ret = device_create_file(&(dev->dev), &dev_attr_Counter_quse);
