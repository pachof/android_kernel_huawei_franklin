...
{
    unsigned int gpioId0 = 0;
    unsigned int gpioId1 = 0;
    unsigned int rowState = 0;
    int rowIndex = 0;
...
{
    unsigned int gpio[MAX_GPIO_NUM];
    unsigned int antBoardid = 0;
    unsigned int temp = 0;

    memset_s(gpio, sizeof(gpio), 0, sizeof(gpio));
    for (unsigned int i = 0; i < g_gpio_count; i++) {
        gpio[i] = (unsigned int)gpio_get_value(pinctrl_table[i].gpio);
        if (gpio[i] < 0) {
            pr_err("%s: get gpio value error, gpio[%u] %u\n", __func__, i, gpio[i]);
            return -1;
        }
        temp |= gpio[i] << i;
    }
    antBoardid = ANTENNA_BOARDID_BASE + temp;
    pr_err("%s: the antBoardid is %u, g_gpio_expect is %d.\n", __func__, antBoardid, g_gpio_expect);
...
static int check_antenna_board_id(void)
{
    ...
    antBoardid = ANTENNA_BOARDID_BASE + (((unsigned int)rowIndex << 4) | ((unsigned int)colIndex));
...
static ssize_t antenna_detect_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    switch(info->name){
        case ANTENNA_BOARDID_GPIO_STATUS: {
            if (g_func_type == 1) {
                antMatchState = get_antenna_board_id();
            } else {
