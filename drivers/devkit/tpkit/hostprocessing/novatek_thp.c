...
#define N9V_CMD_ADDR 0x22D50
#define N9V_CMD_STAMP_ADDR 0x22D77
...
static int gesture_data_read_write(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == N9V_SERIES)
	...
	if (((~r_buf[2] + 1) & 0xFF) != r_buf[3])
		return -EINVAL;
	/* 0x7F is gesture ID, 0x81 is CheckSum data */
...
static int touch_driver_send_fw_cmd(struct thp_device *tdev, uint8_t cmd)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == N9V_SERIES) {
		cmd_addr = N9V_CMD_ADDR;
		stamp_addr = N9V_CMD_STAMP_ADDR;
	}
