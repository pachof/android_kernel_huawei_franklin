	...
	struct cmdq_pkt *handle, struct DISP_GAMMA_LUT_T *user_gamma_lut)
{
	...
	int ret = 0;
	int id;
	struct DISP_GAMMA_LUT_T *gamma_lut = NULL, *old_lut = NULL;
...
static void mtk_gamma_bypass(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle, bool bypass)
{
	DDPINFO("%s bypass=%d\n", __func__, bypass);

	if ((g_gamma_relay_value[index_of_gamma(comp->id)] != 0x1) && (bypass == true)) {
		/* Bypass gamma */
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_GAMMA_CFG, 0x1, 0x1);
		g_gamma_relay_value[index_of_gamma(comp->id)] = 0x1;

	} else if ((g_gamma_relay_value[index_of_gamma(comp->id)] == 0x1) && (bypass == false)) {
		/* Enable gamma */
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_GAMMA_CFG, 0x0, 0x1);
		g_gamma_relay_value[index_of_gamma(comp->id)] = 0x0;
	}
}
...
static int mtk_disp_gamma_probe(struct platform_device *pdev)
{
	...
	struct mtk_disp_gamma *priv = NULL;
