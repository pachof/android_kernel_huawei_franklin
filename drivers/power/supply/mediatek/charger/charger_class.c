...
static bool g_is_otg_en;

bool get_otg_status(void)
{
	return g_is_otg_en;
}
...
int charger_dev_set_dpm(struct charger_device *charger_dev, int vbat_dpm)
{
	if ((charger_dev != NULL) &&
		...
		(charger_dev->ops->set_mivr)) {
		return charger_dev->ops->set_mivr(charger_dev, vbat_dpm);
	}
...
int charger_dev_get_ts_adc(struct charger_device *chg_dev, u32 *ts_volt)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->get_ts_adc)
		return chg_dev->ops->get_ts_adc(chg_dev, ts_volt);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_ts_adc);

int charger_dev_get_usb_id(struct charger_device *chg_dev, u32 *id)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->get_usbid_adc)
		return chg_dev->ops->get_usbid_adc(chg_dev, id);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_usb_id);
...
{
	if ((chg_dev != NULL) && (chg_dev->ops != NULL) &&
		(chg_dev->ops->dump_reg))
		return chg_dev->ops->dump_reg(chg_dev, value, size);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_hw_dump_register);

int charger_dev_get_register_head(struct charger_device *chg_dev, char *head,
	int size)
{
	if ((chg_dev != NULL) && (chg_dev->ops != NULL) &&
		(chg_dev->ops->get_reg_head))
		return chg_dev->ops->get_reg_head(chg_dev, head, size);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_register_head);
...
int charger_dev_enable_force_enable(struct charger_device *chg_dev, bool en)
{
	if (chg_dev && chg_dev->ops && chg_dev->ops->enable_force_sleep)
		return chg_dev->ops->enable_force_sleep(chg_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_enable_force_enable);

int charger_dev_set_watchdog_timer(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_watchdog_timer)
		return chg_dev->ops->set_watchdog_timer(chg_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_watchdog_timer);
...
int charger_dev_enable_otg(struct charger_device *chg_dev, bool en)
{
	if ((chg_dev != NULL) && (chg_dev->ops != NULL) &&
		...
		g_is_otg_en = en ? true : false;
		otg_kick_wdt(chg_dev, en);
...
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->get_charge_state)
		return chg_dev->ops->get_charge_state(chg_dev, pg_state);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_chg_state);
...
int charger_dev_set_vbus_vset(struct charger_device *chg_dev, u32 uV)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_vbus_vset)
		return chg_dev->ops->set_vbus_vset(chg_dev, uV);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_vbus_vset);

int charger_dev_chip_init(struct charger_device *chg_dev,
	struct chip_init_crit *init_crit)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_vbus_vset)
		return chg_dev->ops->chip_init(chg_dev, init_crit);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_chip_init);
...
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->check_input_dpm_state)
		return chg_dev->ops->check_input_dpm_state(chg_dev);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_check_input_dpm_state);

int charger_dev_check_charger_plugged(struct charger_device *chg_dev)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->check_input_dpm_state)
		return chg_dev->ops->check_charger_plugged(chg_dev);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_check_charger_plugged);
