...
#define CFG_SENSOR_PARAMS_TO_SCP_SUPPORT
#define SCP_PARAMS_TO_SCP_SUPPORT

struct sensor_para_t *sensor_para;
...
void *get_sensor_share_mem_addr(enum sh_mem_type_t type)
{
	...
	switch (type) {
	case SHR_MEM_TYPE_ALL:
		return (void *)(sensor_para);
	case SHR_MEM_TYPE_ACC:
		return (void *)(&sensor_para->acc_para);
	case SHR_MEM_TYPE_ALS:
		return (void *)(&sensor_para->als_para);
	case SHR_MEM_TYPE_FINGER:
		return (void *)(&sensor_para->fg_para);
	case SHR_MEM_TYPE_PEDO:
		return (void *)(&sensor_para->extend_step);
	case SHR_MEM_TYPE_MAG:
		return (void *)(&sensor_para->mag_para);
	default:
		break;
	}
	return (void *)(sensor_para);
...
void send_sensor_scp_brightness(int brightness)
{
	...
	if (als_priv == NULL) {
		pr_info("%s: als data err", __func__);
		return;
	}
