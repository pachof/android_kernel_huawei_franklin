...
#define MAX_SCHED_VALUE 50000000000
unsigned long long notrace sched_clock(void)
{
	u64 cyc, res;
	unsigned long seq;
	static unsigned long long sched_clock_pre = 0;
	...
	if ((res - sched_clock_pre > MAX_SCHED_VALUE) && (sched_clock_pre != 0) && (res > sched_clock_pre)) {
		printk("res %llu sched_clock_pre %llu cyc %llu rd epoch_ns %llu epoch_cyc %llu sched_clock_mask %x mul %d shift %d\n",
			res, sched_clock_pre, cyc, rd->epoch_ns, rd->epoch_cyc, rd->sched_clock_mask, rd->mult, rd->shift);
		BUG_ON(1);
	} else if ((sched_clock_pre - res > MAX_SCHED_VALUE) && (sched_clock_pre != 0) && (sched_clock_pre > res)) {
		printk("res %llu sched_clock_pre %llu cyc %llu rd epoch_ns %llu epoch_cyc %llu sched_clock_mask %x mul %d shift %d\n",
			res, sched_clock_pre, cyc, rd->epoch_ns, rd->epoch_cyc, rd->sched_clock_mask, rd->mult, rd->shift);
		BUG_ON(1);
	}
