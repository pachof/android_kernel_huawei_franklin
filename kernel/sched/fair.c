...
#if (defined(CONFIG_VIP_SCHED_OPT) || defined(CONFIG_HW_MTK_RTG_SCHED))
...
#ifdef CONFIG_HW_MTK_RTG_SCHED
	util_min = max_t(unsigned long, util_min, task->cpu_cap_req.min_util);
#endif
		...
		for_each_cpu_and(i, &p->cpus_allowed, sched_group_span(sg)) {
			...
			if (!idle_cpu(i)) {
				...
				if (p->state == TASK_RUNNING &&
					!list_empty(&cpu_rq(i)->vip_thread_list))
					continue;
			}
#endif
			/*
...
static int detach_tasks(struct lb_env *env, struct rq_flags *rf)
{
	...
	while (!list_empty(tasks)) {
		...
		if (energy_aware() &&
			...
			p = get_heaviest_vip_task(
				__pick_first_entity(cfs_rq_of(&p->se)), p, env->dst_cpu);

			boosted = schedtune_task_boost(p) > 0;
			prefer_idle = schedtune_prefer_idle(p) > 0;
			if (!boosted && !prefer_idle &&
			    task_util(p) * 100 < capacity_orig_of(env->src_cpu) * 25)
				goto next;
		}
#endif
