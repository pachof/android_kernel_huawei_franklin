...
static int get_ntc_in_dts(void)
{
	...
	node = of_find_compatible_node(NULL, NULL,
			"mediatek,mtboard-thermistor1");
	if (node) {
		if (of_property_read_u32(node, "PUP_R",
			&g_RAP_pull_up_R) >= 0) {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s PUP_R: %d\n",
				__func__, g_RAP_pull_up_R);
		} else {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s PUP_R read failed\n",
				__func__);
			return -1;
		}
		if (of_property_read_u32(node, "PUP_VOLT",
			...
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s PUP_VOLT: %d\n",
				__func__, g_RAP_pull_up_voltage);
		} else {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s PUP_VOLT read failed\n",
				__func__);
			return -1;
		}
		if (of_property_read_u32(node, "OVER_CRITICAL_L",
			...
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s OVER_CRITICAL_L: %d\n",
				__func__, g_TAP_over_critical_low);
		} else {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s OVER_CRITICAL_L read failed\n",
				__func__);
			return -1;
		}
		if (of_property_read_u32(node, "NTC_TABLE",
			&g_RAP_ntc_table) >= 0) {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s NTC_TABLE: %d\n",
				__func__, g_RAP_ntc_table);
		} else {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s NTC_TABLE read failed\n",
				__func__);
			return -1;
		}
		if (of_property_read_u32(node, "AUXADC_CH",
			&g_RAP_ADC_channel) >= 0) {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s AUXADC_CH: %d\n",
				__func__, g_RAP_ADC_channel);
		} else {
			mtkts_bts_dprintk(
				"[Thermal/TZ/BTS] %s AUXADC_CH read failed\n",
				__func__);
			return -1;
		}
		return 1;
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (g_dt_ntc_status > 0) {
		...
		mtkts_bts_printk(
			"[Thermal/TZ/BTS] %s dts ntc setting applied\n",
			__func__);
		return -EINVAL;
	}
...
static int __init mtkts_bts_init(void)
{
	...
	if (g_dt_ntc_status < 0)
		mtkts_bts_printk("[%s]: dtsi not support\n", __func__);
	else if (g_dt_ntc_status > 0)
		mtkts_bts_printk("[%s]: dtsi setting applied\n", __func__);
	else
		mtkts_bts_printk("[%s]: dtsi not setting\n", __func__);
