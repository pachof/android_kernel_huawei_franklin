	...
	struct device_attribute *attr, char *buf)
{
	...
	case FUELGUAGE_SYSFS_GAUGELOG:
		...
		ret = snprintf(buf, MAX_SIZE, "%-10d %-17s \n", ocv, ctype);
...
static void fuelguage_sysfs_create_group(struct device *dev)
{
	fuelguage_sysfs_init_attrs();
	power_sysfs_create_link_group("hw_power", "coul", "coul_data",
		dev, &fuelguage_sysfs_attr_group);
}

static void fuelguage_sysfs_remove_group(struct device *dev)
{
...
static int fuel_guage_probe(struct platform_device *pdev)
{
	...
	fuelguage_sysfs_create_group(di->dev);
...
static int fuel_guage_remove(struct platform_device *pdev)
{
	...
	fuelguage_sysfs_remove_group(di->dev);
