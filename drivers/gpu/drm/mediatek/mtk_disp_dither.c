			      ...
			      struct cmdq_pkt *handle, bool bypass)
{
	...
	if ((g_dither_relay_value[index_of_dither(comp->id)] != 0x1) && (bypass == true)) {
		/* Bypass Dither */
	...
	} else if ((g_dither_relay_value[index_of_dither(comp->id)] == 0x1) && (bypass == false)) {
		/* Enable Dither */
		g_dither_relay_value[index_of_dither(comp->id)] = 0x0;

		priv->cfg_reg = 0x0 | (priv->cfg_reg & ~0x1);
...
static int mtk_disp_dither_probe(struct platform_device *pdev)
{
	...
	struct mtk_disp_dither *priv = NULL;
