...
#ifdef CONFIG_MTK_SCSI_UFS_DUMP
...
static void ufshcd_err_handler_common(struct ufs_hba *hba)
{
	unsigned long flags;
	u32 err_xfer = 0;
	u32 err_tm = 0;
...
static void ufshcd_err_handler(struct work_struct *work)
{
	...
	ufshcd_err_handler_common(hba);
	return;
}

static void ufshcd_err_handler_mtk(struct kthread_work *work)
{
	struct ufs_hba *hba = NULL;
	hba = container_of(work, struct ufs_hba, eh_threadwork);
	if (!hba) {
		pr_err("%s: invalid para!\n", __func__);
		return;
	}
...
#ifdef CONFIG_HUAWEI_STORAGE_ROFA
        u8 life_time_buf[QUERY_DESC_HEALTH_MAX_SIZE];

        err = ufshcd_read_health_desc(hba, life_time_buf,
                QUERY_DESC_HEALTH_MAX_SIZE);
        if (err) {
                dev_err(hba->dev, "%s: Failed reading health Desc. err = %d\n",
                        __func__, err);
                storage_rochk_record_bootdevice_pre_eol_info(0);
        } else {
                dev_err(hba->dev, "%s: set eol info = %u\n.", __func__,
                        (unsigned int)life_time_buf[2]);
                storage_rochk_record_bootdevice_pre_eol_info(life_time_buf[2]);
        }
#endif
...
#ifdef CONFIG_HUAWEI_STORAGE_ROFA
	storage_rochk_record_bootdevice_type(1);
...
static int ufshcd_ffu_get_device_data(struct ufs_hba *hba)
{
	...
	if (hba->card && hba->ffu_get_device_flag) {
		memset(hba->card, 0, sizeof(struct ufs_dev_desc));
		ret = ufs_get_device_desc(hba, hba->card);
		if (ret) {
			dev_info(hba->dev, "%s: Failed getting device info. err = %d\n",
					__func__, ret);
			return ret;
		}
		ufs_fixup_device_setup(hba, hba->card);
...
static int ufshcd_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	/* Some device may need delay to back noraml state */
	mdelay(1);
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	kthread_init_work(&hba->eh_threadwork, ufshcd_err_handler_mtk);
	...
	mb();
#ifdef CONFIG_HUAWEI_UFS_DSM
	//ufshcd_init_ufs_temp_sys(hba); need to fix
#endif
	/* IRQ registration */
...
int ufshcd_queuecommand_directly(struct ufs_hba *hba,
					struct scsi_cmnd *cmd,
...
start:
	...
	if (needs_flush)
		kthread_flush_work(&hba->eh_threadwork);
	...
	if (err && eh_handle) {
		...
		if (!kthread_queue_work(&hba->eh_worker, &hba->eh_threadwork))
