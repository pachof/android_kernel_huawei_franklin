...
static unsigned int g_aal_ess_level_cmd_id;
static unsigned int g_aal_dre_en_cmd_id;
static unsigned int g_aal_ess_en_cmd_id;
...
#endif
#ifdef CONFIG_LEDS_MTK_PWM
		if(mtk_pwm_leds_get_satus())
			mt_pwm_leds_brightness_set("lcd-backlight", 0);
#endif
#else
		mt_leds_brightness_set("lcd-backlight", 0);
#endif
...
#ifdef CONFIG_LEDS_MTK_PWM
		if(mtk_pwm_leds_get_satus())
			mt_pwm_leds_brightness_set("lcd-backlight", bl_1024);
#endif
#else
		mt_leds_brightness_set("lcd-backlight", bl_1024);
#endif
	...
	void *v)
{
	...
	case 1:
		trans_level = (
			(((1 << (unsigned int)(led_conf->trans_bits)) - 1)
...
{
	unsigned int val = 0;
	unsigned int width = cfg->w, height = cfg->h;
...
#ifdef CONFIG_LEDS_MTK_PWM
	if(mtk_pwm_leds_get_satus())
		mt_pwm_leds_brightness_set("lcd-backlight", backlight_value);
#endif
#else
	mt_leds_brightness_set("lcd-backlight", backlight_value);
#endif
...
static void mtk_aal_bypass(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle, bool bypass)
{
	AALFLOW_LOG(" bypss=%d\n", bypass);

	if ((atomic_read(&g_aal_force_relay) != 1) && (bypass == true)) {
		/* Bypass AAL */
		if (handle)
			mtk_ddp_write_mask(comp, 0x1, DISP_AAL_CFG, 0x1, handle);
		else
			mtk_ddp_write_mask_cpu(comp, 0x1, DISP_AAL_CFG, 0x1);
		atomic_set(&g_aal_force_relay, 1);
	} else if ((atomic_read(&g_aal_force_relay) == 1) && (bypass == false)) {
		/* Enable AAL */
		if (handle)
			mtk_ddp_write_mask(comp, 0x0, DISP_AAL_CFG, 0x1, handle);
		else
			mtk_ddp_write_mask_cpu(comp, 0x0, DISP_AAL_CFG, 0x1);
		atomic_set(&g_aal_force_relay, 0);
	}
}
...
static int mtk_disp_aal_probe(struct platform_device *pdev)
{
	...
	struct mtk_disp_aal *priv = NULL;
	enum mtk_ddp_comp_id comp_id;
	int ret, irq;
#if defined(CONFIG_MTK_DRE30_SUPPORT)
	struct device_node *dre3_dev_node = NULL;
	struct platform_device *dre3_pdev = NULL;
	...
	if(mtk_leds_get_satus()) {
		...
		pr_notice("register mtk disp notifier\n");
	}
#endif
#ifdef CONFIG_LEDS_MTK_PWM
	if(mtk_pwm_leds_get_satus()) {
		mtk_pwm_leds_register_notifier(&leds_init_notifier);
		pr_notice("register mtk pwm notifier\n");
	}
#endif
#else
	mtk_leds_register_notifier(&leds_init_notifier);
...
#ifdef CONFIG_LEDS_MTK_PWM
	if(mtk_pwm_leds_get_satus()) {
		mtk_pwm_leds_unregister_notifier(&leds_init_notifier);
		pr_notice("register mtk pwm notifier\n");
	}
#endif
#else
