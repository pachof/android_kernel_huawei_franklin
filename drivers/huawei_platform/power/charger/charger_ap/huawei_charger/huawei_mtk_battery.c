...
static int get_batt_id_index = 0;
...
static struct ntc_temp_compensation_para_data g_ntc_para[NTC_PARA_LEVEL];
...
#ifdef CONFIG_DIRECT_CHARGER
int huawei_battery_get_scp_status(void)
{
	if (direct_charge_get_super_charging_flag() && huawei_get_ac_online())
		return 1;
	return 0;
}

int huawei_battery_get_fcp_status(void)
{
	int intval;

	if (huawei_get_ac_online() == 0)
		return 0;

	if (fcp_get_stage() >= FCP_STAGE_SUCESS)
		intval = 1; // 0 and 1: fcp status
...
void batt_ntc_parse_para(struct device_node *np)
{
	...
	if (of_property_read_u32(np, "ntc_compensation_is",
		...
		pr_err("get ntc_compensation_is failed\n");
		return;
	}
	...
	for (i = 0; i < array_len; i++) {
		...
		case NTC_PARA_ICHG:
			g_ntc_para[i / NTC_PARA_TOTAL].compensation_ichg = idata;
			break;
		case NTC_PARA_VALUE:
			g_ntc_para[i / NTC_PARA_TOTAL].compensation_value = idata;
			break;
		default:
...
{
	int temp_no_compensation = temp_val;
	...
	if ((g_ntc_compensation_is == 1) &&
		(temp_no_compensation >= COMPENSATION_THRESHOLD)) {
		ichg = abs(cur_temp);
		for (i = 0; i < NTC_PARA_LEVEL; i++) {
			if (ichg >= g_ntc_para[i].compensation_ichg) {
				temp_with_compensation = temp_no_compensation -
					g_ntc_para[i].compensation_value;
				break;
			}
		}
	}

	pr_err("temp_with_compensation=%d temp_no_compensation=%d ichg=%d\n",
		temp_with_compensation, temp_no_compensation, ichg);
	return temp_with_compensation;
}
