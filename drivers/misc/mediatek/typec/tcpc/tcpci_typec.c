...
static inline int typec_set_vbus_cc_short_detection(
	struct tcpc_device *tcpc_dev, bool cc1, bool cc2)
{
#ifdef CONFIG_VBUS_CC_SHORT_DETECTION
	if (tcpc_dev->tcpc_flags & TCPC_FLAGS_VBUS_CC_SHORT_DETECTION)
		return tcpci_set_vbus_cc_short_detection(tcpc_dev, cc1, cc2);
...
static inline void typec_unattached_cc_entry(struct tcpc_device *tcpc_dev)
{
	...
	typec_set_vbus_cc_short_detection(tcpc_dev, false, false);
...
static inline void typec_source_attached_entry(struct tcpc_device *tcpc_dev)
{
	...
	typec_set_vbus_cc_short_detection(tcpc_dev,
					  tcpc_dev->typec_polarity == 0,
					  tcpc_dev->typec_polarity == 1);
	tcpci_report_power_control(tcpc_dev, true);
	...
	struct tcpc_device *tcpc_dev)
{
	...
	typec_set_vbus_cc_short_detection(tcpc_dev, true, true);
	tcpci_report_power_control(tcpc_dev, true);
...
int tcpc_typec_handle_timeout(struct tcpc_device *tcpc_dev, uint32_t timer_id)
{
	...
	case TYPEC_RT_TIMER_SAFE0V_TOUT:
		...
		if (!tcpci_check_vbus_valid_from_ic(tcpc_dev)) {
			...
			vbus = charger_get_vbus();
#ifdef CONFIG_HUAWEI_DSM
			snprintf(tmp_buf, sizeof(tmp_buf),
				"%s:VBUS still Valid, vbus=%d\n", __func__, vbus);
			power_dsm_dmd_report(POWER_DSM_PD,
			DSM_PD_TYPEC_VBUS_VALID, tmp_buf);
#endif
		}
		break;
#endif	/* CONFIG_TYPEC_ATTACHED_SRC_SAFE0V_TIMEOUT */
...
#ifdef CONFIG_VBUS_CC_SHORT_DETECTION
int tcpc_typec_handle_vbus_cc_short(struct tcpc_device *tcpc_dev,
				    bool vbus_cc1_short, bool vbus_cc2_short)
{
	TCPC_INFO("%s short(cc1,cc2)=(%d, %d)\n", __func__, vbus_cc1_short,
		  vbus_cc2_short);
	if ((tcpc_dev->vbus_cc1_short == vbus_cc1_short) &&
	    (tcpc_dev->vbus_cc2_short == vbus_cc2_short)) {
		return 0;
	}
	tcpc_dev->vbus_cc1_short = vbus_cc1_short;
	tcpc_dev->vbus_cc2_short = vbus_cc2_short;
	tcpci_notify_vbus_cc_short(tcpc_dev);
	return 0;
}
#endif /* CONFIG_VBUS_CC_SHORT_DETECTION */
