...
int lcd_kit_updt_fps(struct platform_device *pdev)
{
	...
	if (lcd_is_support_dfr()) {
		...
		disp_info->fps.last_update_fps = pinfo->fps_scence;
		return ret;
	}
...
static void lcd_kit_fps_init(struct hisi_panel_info *pinfo)
{
	if (lcd_is_support_dfr()) {
		...
		disp_info->fps.last_update_fps = LCD_FPS_SCENCE_MAX;
	...
	LCD_KIT_INFO("dfr_support = %d, dynamic_dsc_support = %d\n",
		pinfo->dfr_support, pinfo->dynamic_dsc_support);
}
...
void lcd_kit_pinfo_init(struct device_node *np, struct hisi_panel_info *pinfo)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,mask-delay-time-before-fp",
		&pinfo->mask_delay_time_before_fp, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mask-delay-time-after-fp",
		&pinfo->mask_delay_time_after_fp, 0);
...
void lcd_kit_parse_dt(struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,dsi1-support",
...
static void lcd_kit_fps_dsc_config(struct hisi_panel_info *pinfo)
{
	if (pinfo->ifbc_type != IFBC_TYPE_NONE ||
		disp_info->fps.support != FPS_CONFIG_EN ||
		disp_info->fps.fps_ifbc_type == IFBC_TYPE_NONE)
		return;
	lcd_kit_compress_config(disp_info->fps.fps_ifbc_type, pinfo);
}
