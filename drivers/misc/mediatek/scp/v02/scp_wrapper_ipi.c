			...
			if (irq_status && 1 << scp_mbox_pin_recv[i].pin_index) {
				if (scp_mbox_pin_recv[i].chan_id
					...
					mtk_mbox_read(&scp_mboxdev, mbox,
						    ...
						    &buf, MBOX_SLOT_SIZE * 5);
					pr_info("[SCP] ipi id/type/action/event/reserve = %u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/\n",
						buf.id, buf.info[0],
						buf.info[1], buf.info[2],
						buf.info[3], buf.info[4], buf.info[5], buf.info[6], buf.info[7],
						buf.info[8], buf.info[9], buf.info[10], buf.info[11]);
				} else {
