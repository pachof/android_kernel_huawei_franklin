...
static unsigned int g_disp_ccorr_without_gamma;
...
static int disp_ccorr_write_coef_reg(struct mtk_ddp_comp *comp,
	struct cmdq_pkt *handle, int lock)
{
	struct DRM_DISP_CCORR_COEF_T *ccorr = NULL, *multiply_matrix = NULL;
	int ret = 0;
...
{
	int ret = 0;
	struct DRM_DISP_CCORR_COEF_T *ccorr = NULL, *old_ccorr = NULL;
	int id = index_of_ccorr(comp->id);
	...
	struct cmdq_pkt *handle, int32_t matrix[16], int32_t hint)
{
	int ret = 0;
	int i, j;
	unsigned int ccorr_without_gamma = 0;
...
static void mtk_ccorr_bypass(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle, bool bypass)
{
	DDPINFO("%s bypass=%d\n", __func__, bypass);

	if ((g_ccorr_relay_value[index_of_ccorr(comp->id)] != 0x1) && (bypass == true)) {
		/* Bypass ccorr */
		cmdq_pkt_write(handle, comp->cmdq_base,
				   comp->regs_pa + DISP_REG_CCORR_CFG, 0x1, 0x1);
		g_ccorr_relay_value[index_of_ccorr(comp->id)] = 0x1;

	} else if ((g_ccorr_relay_value[index_of_ccorr(comp->id)] == 0x1) && (bypass == false)) {
		/* Enable ccorr */
		cmdq_pkt_write(handle, comp->cmdq_base,
				   comp->regs_pa + DISP_REG_CCORR_CFG, 0x0, 0x1);
		g_ccorr_relay_value[index_of_ccorr(comp->id)] = 0x0;
	}
}
	...
	struct cmdq_pkt *handle, unsigned int cmd, void *data)
{
	...
	case CLEAR_CCORR:
	{
		mtk_disp_ccorr_clear(comp, handle);
	}
	break;

	case RESTORE_CCORR:
	{
		mtk_disp_ccorr_restore(comp, handle);
	}
	break;
...
static void mtk_ccorr_prepare(struct mtk_ddp_comp *comp)
{
	...
	mtk_ddp_write_mask_cpu(comp, CCORR_BYPASS_SHADOW,
		DISP_REG_CCORR_SHADOW, CCORR_BYPASS_SHADOW);
	mtk_ddp_write_mask_cpu(comp, 0x0, DISP_REG_CCORR_CFG, CCORR_RELAYMODE);
	g_ccorr_relay_value[index_of_ccorr(comp->id)] = 0x0;
	DDPINFO("%s ccorr restore\n", __func__);
#endif
#endif
}
...
static int mtk_disp_ccorr_probe(struct platform_device *pdev)
{
	...
	struct mtk_disp_ccorr *priv = NULL;
...
void mtk_disp_ccorr_clear(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
	DDPINFO("%s +\n",__func__);
	if (default_comp == NULL) {
		DDPPR_ERR("%s default_comp is null!\n",__func__);
		return;
	}
	cmdq_pkt_write(handle, comp->cmdq_base, default_comp->regs_pa + DISP_REG_CCORR_CFG, 0x1, 0x1);
	g_ccorr_relay_value[index_of_ccorr(default_comp->id)]= 0x1;
	DDPINFO("%s -\n",__func__);
}

void mtk_disp_ccorr_restore(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
	DDPINFO("%s +\n",__func__);
	if (default_comp == NULL) {
		DDPPR_ERR("%s default_comp is null!\n",__func__);
	...
	cmdq_pkt_write(handle, comp->cmdq_base, default_comp->regs_pa + DISP_REG_CCORR_CFG, 0x0, 0x1);
	g_ccorr_relay_value[index_of_ccorr(default_comp->id)]= 0x0;
	DDPINFO("%s -\n",__func__);
}

int mtk_drm_ccorr_clear(struct drm_device *dev, void *data,
	struct drm_file *file_priv)
{
	struct mtk_drm_private *private = dev->dev_private;
	struct mtk_ddp_comp *comp = private->ddp_comp[DDP_COMPONENT_CCORR0];
	struct drm_crtc *crtc = private->crtc[0];
	DDPINFO("%s enter\n",__func__);
	return mtk_crtc_user_cmd(crtc, comp, CLEAR_CCORR, data);
}

int mtk_drm_ccorr_restore(struct drm_device *dev, void *data,
	struct drm_file *file_priv)
{
	struct mtk_drm_private *private = dev->dev_private;
	struct mtk_ddp_comp *comp = private->ddp_comp[DDP_COMPONENT_CCORR0];
	struct drm_crtc *crtc = private->crtc[0];
	DDPINFO("%s enter\n",__func__);
	return mtk_crtc_user_cmd(crtc, comp, RESTORE_CCORR, data);
}