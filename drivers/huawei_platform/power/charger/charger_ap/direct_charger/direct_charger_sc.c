	...
	int type, int dev_check_type)
{
	...
	if (ret == 0)
		set_hw_dev_detect_result(dev_check_type);
...
{
	sc_get_devices_info_by_type(di, CHARGE_IC_MAIN, DEV_DETECT_SWITCHCAP);
	if (di->multi_ic_mode_para.support_multi_ic)
		sc_get_devices_info_by_type(di, CHARGE_IC_AUX, DEV_DETECT_SC_AUX);
}
#else
static void sc_get_devices_info(struct direct_charge_device *di)
{
...
static int sc_set_iin_limit(unsigned int val)
{
	...
	hwlog_info("set input current: %u, limit current: %d", val, di->sysfs_iin_thermal);
	return 0;
}
