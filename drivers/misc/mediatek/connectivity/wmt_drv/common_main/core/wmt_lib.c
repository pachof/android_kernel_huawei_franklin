...
{
	return osal_lock_unsleepable_lock(&gDevWmt.power_lock);
}

VOID wmt_lib_power_lock_release(VOID)
{
	osal_unlock_unsleepable_lock(&gDevWmt.power_lock);
}

INT32 wmt_lib_power_lock_trylock(VOID)
{
	return osal_trylock_unsleepable_lock(&gDevWmt.power_lock);
}
...
INT32 wmt_lib_init(VOID)
{
	...
	osal_unsleepable_lock_init(&pDevWmt->power_lock);
...
INT32 wmt_lib_deinit(VOID)
{
	...
	osal_unsleepable_lock_deinit(&pDevWmt->power_lock);
...
extern VOID consys_print_dbg_log(VOID);
...
INT32 wmt_lib_trigger_assert_delay(ENUM_WMTDRV_TYPE_T type, UINT32 reason)
{
	wmt_lib_trigger_assert_keyword_delay(type, reason, NULL);
	return 0;
}
