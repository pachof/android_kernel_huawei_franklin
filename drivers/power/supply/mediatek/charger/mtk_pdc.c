...
#define BAT_VOL_5V                5000
#define BAT_CUR_3A                3000
#define BAT_CUR_1A                1000
...
bool pdc_is_ready(void)
{
	...
	if (direct_charge_get_stage_status() != DC_STAGE_DEFAULT)
		return false;

	if ((type == STANDARD_CHARGER) && (direct_charge_pre_check() == 0))
		return false;
			...
			int *newidx)
{
	...
	/* 100 is BAT 100 PERCENT transformer*/
	if (cap->max_mv[idx] == BAT_VOL_5V && cap->ma[idx] == BAT_CUR_3A)
		pd_max_watt = cap->max_mv[idx] * ((cap->ma[idx] - BAT_CUR_1A) /
			100 * (100 - pd->data.ibus_err) - 100);
	if (cap->max_mv[pd->pd_buck_idx] == BAT_VOL_5V &&
		cap->ma[pd->pd_buck_idx] == BAT_CUR_3A)
		pd_min_watt = cap->max_mv[pd->pd_buck_idx] *
			(cap->ma[pd->pd_buck_idx] - BAT_CUR_1A) / 100 *
			(100 - pd->data.ibus_err) - pd->data.vsys_watt;
