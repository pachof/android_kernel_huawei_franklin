...
static struct mt6360_ldo_info *ldo_info = NULL;
...
int mt6360_ldo5_ndis_en(int en)
{
    struct mt6360_ldo_info *mli = ldo_info;
    int ret;

    dev_info(mli->dev, "LDO5 output discharge : %d\n", en);

    if (en)
        ret = mt6360_ldo_reg_update_bits(mli, MT6360_LDO_LDO5_CTRL2,
                         0x04, 0xff);
    else
        ret = mt6360_ldo_reg_update_bits(mli, MT6360_LDO_LDO5_CTRL2,
                         0x04, 0);
    if (ret < 0) {
        dev_err(mli->dev,
            "%s: Set LDO5 output discharge fail (%d)\n", __func__, ret);
        return ret;
    }
    return 0;
}
EXPORT_SYMBOL_GPL(mt6360_ldo5_ndis_en);
