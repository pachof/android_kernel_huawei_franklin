	...
	unsigned int len)
{
	...
	uint8_t cmd_request_frame[2] = {SENSE_ON_CMD_0, SENSE_ON_CMD_1};
...
exit:
	...
	if (tdev->thp_core->support_vendor_ic_type != STU56A) {
		ret = alix_write(tdev->thp_core->sdev,
			ALIX_FRAME_BUFFER_ADDRESS, cmd_request_frame,
			sizeof(cmd_request_frame));
		if (ret != 0)
			THP_LOG_ERR("%s: Error request another frame\n",
				__func__);
	}
...
static int touch_driver_suspend_ap(struct thp_device *tdev)
{
	...
	cmd[0] = SET_GESTURE_MODE_CMD;
	cmd[1] = GESTURE_MODE_DATA;
	cmd[2] = tdev->thp_core->easy_wakeup_info.sleep_mode ?
		DOUBLE_CLICK_CMD : DEFAULT_VALUE_CMD;
	cmd[5] = g_st_thp_udfp_status ? LONG_PRESS_CMD : DEFAULT_VALUE_CMD;
	if (tdev->thp_core->support_vendor_ic_type == ST_BA80Y) {
		ret = fw_write(sdev, cmd, SUSPEND_CMD_LEN);
		if (ret != 0)
			THP_LOG_ERR("%s:set gesture cmd fail\n", __func__);
	}
