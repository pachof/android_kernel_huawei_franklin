...
static int cam_isp_dsm_enabled(void)
{
	int enabled;
	int ret;
	...
	ret = of_property_read_u32(of_node, "cam-isp-dsm-enabled", &enabled);
	if (ret) {
		LOG_NOTICE("donnot support dsm\n");
		return 0;
	}
	return enabled;
}
...
static void cam_isp_trig_dsm(unsigned int err, enum ISP_IRQ_TYPE_ENUM module)
{
	if (!cam_isp_dsm_enabled()) {
		LOG_NOTICE("donnot support isp dsm\n");
		return;
	}

	LOG_NOTICE("support cam isp dsm\n");

	if (g1stSof[module] || HwP1Done_cnt[module] < CAM_ISP_DSM_SKIP_FRAME_NUM) {
		LOG_NOTICE("the first 3 frames donnot report, ignore\n");
		return;
	}

	/* TG_GRAB_ERR */
	if (err & TG_GBERR_ST) {
		LOG_NOTICE("dsm report ErrStatus:0x%x, HwP1Done_cnt[%d]:%d\n",
			err, module, HwP1Done_cnt[module]);
		(void)camera_dsm_report_info(DSM_CAMERA_SOF_ERR,
			"isp int err status(0x%x), check RFI\n", err);
	}
}
			   ...
			   unsigned int warnTwo, enum ISP_IRQ_TYPE_ENUM module)
{
	...
	if (ErrStatus) {
		...
		case ISP_IRQ_TYPE_INT_CAM_A_ST:
			...
			cam_isp_trig_dsm(ErrStatus, module);
			break;
		case ISP_IRQ_TYPE_INT_CAM_B_ST:
