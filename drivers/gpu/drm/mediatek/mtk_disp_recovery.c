	...
	while (1) {
...
#ifdef CONFIG_LCD_KIT_DRIVER
		if (disp_info->lcd_kit_skip_poweron_esd_check && esd_ctx->first_esd_check) {
			esd_ctx->first_esd_check = false;
			DDPINFO("[ESD]first esd check after crtc enable\n");
			continue;
		}
#endif
		...
		do {
...
#ifdef CONFIG_LCD_KIT_DRIVER
			/* esd recovery need recovery bl */
			if (panel_info->esd_recovery_bl_support) {
				DDPINFO("[ESD] recovery backlight\n");
				switch (panel_info->bl_ic_ctrl_mode) {
					case I2C_ONLY_MODE:
						lcd_kit_bl_ic_set_backlight(esd_recovery_level);
						break;
			...
			if(mtk_leds_get_satus()) {
				mtk_drm_setbacklight(crtc,
					mtk_drm_get_last_backlight(), 0);
				DDPPR_ERR("recovery backlight level 0x%x\n",
					mtk_drm_get_last_backlight());
			}
#endif
			if (disp_info->lcd_kit_skip_poweron_esd_check)
				msleep(disp_info->lcd_kit_again_esd_check_delay);
#endif
		} while (++i < ESD_TRY_CNT);
		...
		if (ret != 0) {
			...
			if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
				/*
				 * esd max number of check registers is 3
				 * 0 is the first register value
				 * 1 is the second register value
				 * 2 is the third register value
				*/
				dsm_client_record(lcd_dclient,
					"lcd esd recovery, read value:0x%x, 0x%x, 0x%x!\n",
...
#ifdef CONFIG_LCD_KIT_DRIVER
	if (disp_info->lcd_kit_skip_poweron_esd_check && enable)
		esd_ctx->first_esd_check = true;
#endif
