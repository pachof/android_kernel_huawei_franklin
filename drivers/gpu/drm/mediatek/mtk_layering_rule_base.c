...
{
	int overlapped;
	struct drm_mtk_layer_config *src_info = NULL, *dst_info = NULL;
	int i;
...
{
	int i;
	struct drm_mtk_layer_config *lc = NULL;
				      ...
				      int disp, int ovl_limit)
{
	...
	struct drm_mtk_layer_config *l_info = NULL;
...
{
	struct hrt_sort_entry *temp = NULL;
	struct drm_mtk_layer_config *layer_info = NULL;
...
static int add_layer_entry(struct drm_mtk_layer_config *l_info, bool sort_by_y,
			   int overlap_w)
{
	struct hrt_sort_entry *begin_t = NULL, *end_t = NULL;
	struct hrt_sort_entry **p_entry = NULL;
...
static int remove_layer_entry(struct drm_mtk_layer_config *layer_info,
			      bool sort_by_y)
{
	struct hrt_sort_entry *temp = NULL, *free_entry = NULL;
...
{
	struct hrt_sort_entry *cur_entry = NULL, *next_entry = NULL;
...
{
	struct hrt_sort_entry *tmp_entry = NULL;
			       ...
			       int debug_mode, struct drm_device *dev)
{
	int ret;
	int overlap_num;
	struct mtk_drm_lyeblob_ids *lyeblob_ids = NULL;
	unsigned int scale_num = 0;
	unsigned int scn_decision_flag = 0;
			      ...
			      struct drm_device *dev)
{
	...
	struct drm_mtk_layer_config *input_config = NULL;
