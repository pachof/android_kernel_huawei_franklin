	...
	char *keyname, char *rv, int rv_len)
{
	...
	if (parser_get_ini_key_value(section, keyname, value, sizeof(value)) < 0) {
...
static int parser_get_ini_phy_data(char *data, int fsize)
{
	...
	while (true) {
		...
		if (equal_sign == 0) {
			...
			if (ini_info[g_ini_items].key_name_len > PARSER_MAX_KEY_NAME_LEN)
				memcpy(ini_info[g_ini_items].key_name, ini_buf, PARSER_MAX_KEY_NAME_LEN);
			else
		...
		if (ini_info[g_ini_items].key_val_len > PARSER_MAX_KEY_VALUE_LEN)
			memcpy(ini_info[g_ini_items].key_value, ini_buf + equal_sign + 1, PARSER_MAX_KEY_VALUE_LEN);
		else
			memcpy(ini_info[g_ini_items].key_value, ini_buf + equal_sign + 1, ini_info[g_ini_items].key_val_len);
...
int ili_mp_test_main(struct ts_rawdata_info *raw_info, u8 *apk, bool lcm_on)
{
	...
	parser_get_ini_key_value("pv5_4 command", "protocol", str, sizeof(str));
