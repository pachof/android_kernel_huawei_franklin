...
static DEFINE_MUTEX(g_rt9759_log_lock);
...
static int rt9759_write_mask(struct rt9759_device_info *di, u8 reg, u8 mask, u8 shift, u8 value)
{
	int ret;
	u8 val = 0;
...
static int rt9759_charge_enable(int enable, void *dev_data)
{
	...
	struct rt9759_device_info *di = (struct rt9759_device_info *)dev_data;
...
static int rt9759_adc_enable(int enable, void *dev_data)
{
	int ret;
	u8 reg = 0;
	u8 value = enable ? 0x1 : 0x0;
...
static int rt9759_value_dump(char *buffer, int size, void *dev_data)
{
	...
	rt9759_get_ibus_ma(&ibus, dev_data);
	rt9759_get_vbus_mv(&vbus, dev_data);
	rt9759_get_ibat_ma(&ibat, dev_data);
	rt9759_get_vusb_mv(&vusb, dev_data);
	rt9759_get_vout_mv(&vout, dev_data);
	rt9759_get_device_temp(&temp, dev_data);

	if (di->ic_role == CHARGE_IC_TYPE_MAIN)
		snprintf(buffer, size,
			"     %-7d%-7d%-7d%-7d%-7d%-7d%-7d  ",
			ibus, vbus, ibat, vusb, vout, rt9759_get_vbat_mv(dev_data), temp);
	else
		snprintf(buffer, size,
			"%-7d%-7d%-7d%-7d%-7d%-7d%-7d  ",
			ibus, vbus, ibat, vusb, vout, rt9759_get_vbat_mv(dev_data), temp);
...
static int rt9759_reg_init(struct rt9759_device_info *di)
{
	...
	ret |= rt9759_config_switching_frequency(di,
		di->switching_frequency);
...
static int rt9759_charge_init(void *dev_data)
{
	...
	di->device_id = rt9759_get_device_id((void *)di);
	if (di->device_id == -1)
		return -1;
...
static void rt9759_fault_handle(struct rt9759_device_info *di, struct nty_data *data)
{
	int val = 0;
	...
	if (ac_protection & RT9759_AC_OVP_FLAG_MASK) {
		...
		val = rt9759_get_vbat_mv((void *)di);
		...
		rt9759_get_ibat_ma(&val, (void *)di);
		...
		rt9759_get_vbus_mv(&val, (void *)di);
		...
		rt9759_get_ibus_ma(&val, (void *)di);
...
static void rt9759_interrupt_work(struct work_struct *work)
{
	...
	ret = rt9759_read_byte(di, RT9759_AC_OVP_REG, &ac_protection);
	ret |= rt9759_read_byte(di, RT9759_BUS_OCP_UCP_REG, &ibus_ucp);
	ret |= rt9759_read_byte(di, RT9759_FLT_FLAG_REG, &fault_flag);
	ret |= rt9759_read_byte(di, RT9759_CONVERTER_STATE_REG, &converter_state);
	if (ret)
...
static struct direct_charge_ic_ops rt9759_aux_sysinfo_ops = {
	.dev_name = "rt9759_aux",
	.ic_init = rt9759_charge_init,
	.ic_exit = rt9759_charge_exit,
	.ic_enable = rt9759_charge_enable,
...
static struct direct_charge_batinfo_ops rt9759_aux_batinfo_ops = {
	.init = rt9759_batinfo_init,
	.exit = rt9759_batinfo_exit,
	.get_bat_btb_voltage = rt9759_get_vbat_mv,
	.get_bat_package_voltage = rt9759_get_vbat_mv,
...
static struct power_log_ops rt9759_aux_log_ops = {
	.dev_name = "rt9759_aux",
	.dump_log_head = rt9759_get_register_head,
	.dump_log_content = rt9759_value_dump,
};
...
{
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		rt9759_sysinfo_ops.dev_data = (void *)di;
		rt9759_batinfo_ops.dev_data = (void *)di;
		rt9759_log_ops.dev_data = (void *)di;
	} else {
		rt9759_aux_sysinfo_ops.dev_data = (void *)di;
		rt9759_aux_batinfo_ops.dev_data = (void *)di;
		rt9759_aux_log_ops.dev_data = (void *)di;
	}
}
...
static int rt9759_ops_register(struct rt9759_device_info *di)
{
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		...
		ret = dc_sc_ops_register(di->ic_role, &rt9759_aux_sysinfo_ops);
		ret |= dc_batinfo_ops_register(SC_MODE, di->ic_role, &rt9759_aux_batinfo_ops);
	}
	if (ret) {
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN)
		...
		ret = power_log_ops_register(&rt9759_aux_log_ops);
	if (ret)
	...
	const struct i2c_device_id *id)
{
	...
	ret = rt9759_get_device_id((void *)di);
	if (ret < 0)
		goto rt9759_fail_0;
