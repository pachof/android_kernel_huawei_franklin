...
static enum mtk_ovl_colorspace mtk_ovl_map_cs(enum mtk_drm_dataspace ds)
{
	...
	switch ((uint)ds & MTK_DRM_DATASPACE_STANDARD_MASK) {
			 ...
			 struct mtk_rect rsz_dst_roi, struct cmdq_pkt *handle)
{
	if (prev == DDP_COMPONENT_RSZ0 ||
		...
		uint lc_x = rsz_dst_roi.x, lc_y = rsz_dst_roi.y;
		uint lc_w = rsz_dst_roi.width, lc_h = rsz_dst_roi.height;
					 ...
					 bool is_ext_layer)
{
	...
	if (is_ext_layer) {
		...
		compr_en =
			DISP_REG_GET_FIELD(REG_FLD(1, (uint)layer + 4),
