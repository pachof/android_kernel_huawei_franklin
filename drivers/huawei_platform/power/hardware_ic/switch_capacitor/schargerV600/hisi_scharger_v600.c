...
};*/ //typec相关，暂不移植，不用v600的PD
int hi6526_set_watchdog_timer_execute(int value);
...
int hi6526_set_buck_enable(int enable, void *dev_data)
{
	...
	(void)hi6526_set_batfet_ctrl(0); // 0:close batfet
	(void)hi6526_set_watchdog_timer_execute(WATCHDOG_TIMER_40_S);
	(void)hi6526_reset_watchdog_timer_execute(NULL);
	return 0;
}
...
int hi6526_set_vbus_vset(struct charger_device *dev, u32 value)
{
	...
	if (hi6526_get_charge_enable() == TRUE) {
		hi6526_set_charge_enable(dev, CHG_DISABLE);
		charger_flag = 1;
	}
	...
	/* resume charge state */
	if (charger_flag == 1)
		hi6526_set_charge_enable(dev, CHG_ENABLE);
...
int hi6526_config_opt_param(int vbus_vol)
{
	...
	ret = hi6526_set_vbus_vset(NULL, vbus_vol);
	if (ret != 0)
...
int hi6526_set_charge_enable_execute(int enable)
{
	struct hi6526_device_info *di = g_hi6526_dev;
...
{
	return hi6526_set_charge_enable_execute(enable);
}

int hi6526_set_otg_current_execute(int value)
{
	u8 reg;
...
static int hi6526_reset_otg_watchdog_timer_execute(int value)
{
	return hi6526_reset_watchdog_timer_execute(NULL);
}
...
int hi6526_set_otg_enable_exeute(int enable)
{
	struct hi6526_device_info *di = g_hi6526_dev;
	...
	if (enable) {
		...
		hi6526_set_charge_enable_execute(CHG_DISABLE);
	} else {
...
int hi6526_set_term_enable(struct charger_device *dev, bool enable)
{
	...
	if (enable) {
		...
		vbatt_mv = battery_get_bat_voltage();
				   ...
				   char *buffer, int size)
{
	...
	ret0 = hi6526_get_vbus_mv(NULL, (unsigned int *)&vbus);
	ret1 = hi6526_get_ibat(&ibat, NULL);
	ret2 = hi6526_get_vusb(&vusb);
	ret3 = hi6526_get_vout(&vout, NULL);
	vbat = hi6526_get_vbat(NULL);
	hi6526_get_ibus_ma(NULL, &ibus);
	ret4 = hi6526_get_chip_temp(&temp, NULL);
...
static int hi6526_dump_register(struct charger_device *dev, char *buffer, int size)
{
	u8 reg_val = 0;
	int i, ret1, offset;
	int ret = 0;
...
static int hi6526_get_register_head(struct charger_device *dev, char *buffer, int size)
{
	int i;
	int ret = 0;
	int str_len = size - 1;
...
/* print the register value in charging process */
static int hi6526_dump_reg_value(struct charger_device *dev, char *reg_value,
	int size)
{
	u8 reg_val = 0;
	int i;
...
/* print the register head in charging process */
static int hi6526_reg_head(struct charger_device *dev, char *reg_head, int size)
{
	const int str_num = HI6526_DBG_VAL_SIZE + 1;
	char buff[BUF_LEN] = { 0 };
	int i;
...
{
	return  hi6526_reg_head(NULL, buffer, size);
}

static int hi6526_dump_reg(char *buffer, int size, void *dev_data)
{
	return hi6526_dump_reg_value(NULL, buffer, size);
}
...
int hi6526_set_watchdog_timer_execute(int value)
{
	...
	scharger_dbg("watch dog timer is %u,the register value is set %u\n",
		     dog_time, val);
	hi6526_reset_watchdog_timer_execute(NULL);
...
struct charger_ops hi6526_ops = {
	...
	.dump_registers = NULL, //hi6526_dump_register,
...
static void hi6526_fcp_scp_ops_register(void)
{
	...
	if (hi6526_is_support_scp() == 0) {
		...
		ret[3] = dc_batinfo_ops_register(SC_MODE, CHARGE_IC_TYPE_MAIN,
			&hi6526_batinfo_ops);
		ret[4] = dc_batinfo_ops_register(LVC_MODE, CHARGE_IC_TYPE_MAIN,
			&hi6526_batinfo_ops);
#else
		ret[0] = 0;
			...
			const struct i2c_device_id *id)
{
	...
	if (ret)
		scharger_err("power log register fail\n");
