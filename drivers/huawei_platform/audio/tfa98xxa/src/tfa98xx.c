...
#define SMARTPA_VENDOR 1
#define SMARTPA_MODEL "tfa98xxa"
...
static void tfa98xx_adsp_send_calib_values(void)
{
	...
	if (tfa98xx_device_count == 2) {
		/*left/right selector*/
		if (tfa98xx_pri->i2c->addr == 0x36){ /*secodary is 0x36*/
			tfa98xx_sec = tfa98xx_pri;
			tfa98xx_pri = list_first_entry(&tfa98xx_device_list, struct tfa98xx, list);
		}
		else {
			tfa98xx_sec = list_first_entry(&tfa98xx_device_list, struct tfa98xx, list);
		}
	}
...
static int tfa98xx_do_calibrate(struct tfa98xx *tfa98xx_pri, struct tfa98xx *tfa98xx_sec)
{
	...
	for (i = 0; i < tfa98xx_device_count; i++){
		...
		dev_info(tfa98xx[0]->dev, "%s: Calibration value: %d.%3d ohm\n", __func__, 
...
static char *box_name[] = {"tfa98xx_NONE.cnt",
						   "tfa98xx_AAC_AAC.cnt",
						   "tfa98xx_AAC_XW.cnt",
						   "tfa98xx_XW_XW.cnt",
						   "tfa98xx_XW_AAC.cnt"};
	...
	unsigned int cmd, void __user *arg)
{
	...
	int cal_impedence_l = 0;
	int cal_impedence_r = 0;
#endif
	...
	switch(cmd) {
		case TFA98XX_SWITCH_FIRMWARE_L:
			...
			/*choose the demand firmware*/
			if (value >= 0 && 
				value < ARRAY_SIZE(box_name)) {
				box_id = value;
				fw_name = box_name[box_id];

				dev_info(tfa98xx_pri->dev, "use firmware %s\n", fw_name);
			}
...
#else
           	mutex_lock(&tfa98xx_pri->dsp_lock);
			tfa98xxCalibration(tfa98xx_pri,&cal_impedence_l);
			pr_err("i2c addr is 0x%x,Calibration impedenc: %d\n", tfa98xx_pri->i2c->addr,cal_impedence_l);
			mutex_unlock(&tfa98xx_pri->dsp_lock);
			if (tfa98xx_device_count == 2){
				mutex_lock(&tfa98xx_sec->dsp_lock);
				tfa98xxCalibration(tfa98xx_sec,&cal_impedence_r);
				pr_err("i2c addr is 0x%x,Calibration impedenc: %d\n", tfa98xx_sec->i2c->addr,cal_impedence_r);
				mutex_unlock(&tfa98xx_sec->dsp_lock);
			}
...
static ssize_t tfa98xx_ioctrl_rpc_read(struct file *file,
					 ...
					 loff_t *ppos)
{
	struct tfa98xx *tfa98xx = list_last_entry(&tfa98xx_device_list, struct tfa98xx, list);
	int ret = 0;
	uint8_t *buffer = NULL;
	enum Tfa98xx_Error error;
					 ...
					 loff_t *ppos)
{
	...
	if (tfa98xx_device_count == 2) {
		if (tfa98xx_sec == NULL || 
			tfa98xx_sec->tfa == NULL) {
			pr_debug("[0x%x] dsp is not available\n", tfa98xx_sec->i2c->addr);
			return -ENODEV;
		}
	}
	...
	buffer = kmalloc(count, GFP_KERNEL);
	if (buffer == NULL) {
		pr_debug("tfa98xx_ioctrl_rpc_read can not allocate memory\n");
		return -ENOMEM;
	}
	...
	if (tfa98xx_pri->tfa->is_probus_device) {
		error = tfa98xx_recv_data_from_dsp(buffer, count/2);
	} else {
		error = dsp_msg_read(tfa98xx_pri->tfa, count/2, buffer);
	}
	...
	if (tfa98xx_device_count == 2) {
		...
		if (tfa98xx_sec->tfa->is_probus_device) {
			error = tfa98xx_recv_data_from_dsp(buffer+count/2, count/2);
		} else {
			error = dsp_msg_read(tfa98xx_sec->tfa, count/2, buffer+count/2);
		}
					 ...
					 size_t count, loff_t *ppos)
{
	...
	msg_file = kmalloc(count + sizeof(TfaFileDsc_t), GFP_KERNEL);
	if ( msg_file == NULL ) {
		pr_debug("tfa98xx_ioctrl_rpc_send can not allocate memory\n");
		return	-ENOMEM;
	}
	msg_file->size = count;
	...
	if (tfa98xx_pri->tfa->is_probus_device) {
		...
		if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
			...
			error = dsp_msg(tfa98xx_pri->tfa, msg_file->size, msg_file->data);
			if (error != Tfa98xx_Error_Ok) {
				pr_debug("[0x%x] dsp_msg error: %d\n", tfa98xx_pri->i2c->addr, error);
				err = -EIO;
			}
	...
	if (tfa98xx_device_count == 2) {
		...
		if (tfa98xx_sec->tfa->is_probus_device) {
			...
			if (err != 0)
				pr_err("[0x%x] dsp_msg error: %d\n", tfa98xx_sec->i2c->addr, err);

			mdelay(3);
			
		} else {
			if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
				error = tfaContWriteFile(tfa98xx_sec->tfa, msg_file, 0, 0); /* int vstep_idx, int vstep_msg_idx both 0 */
				if (error != Tfa98xx_Error_Ok) {
					pr_debug("[0x%x] tfaContWriteFile error: %d\n", tfa98xx_sec->i2c->addr, error);
					err = -EIO;
				}
			} else {
				error = dsp_msg(tfa98xx_sec->tfa, msg_file->size, msg_file->data);
				if (error != Tfa98xx_Error_Ok) {
					pr_debug("[0x%x] dsp_msg error: %d\n", tfa98xx_sec->i2c->addr, error);
					err = -EIO;
				}
	...
	const struct i2c_device_id *id)
{
	...
	if (ret)
        ...
        tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9874");
#endif /*ENABLE_IOCTRL_INTERFACE*/
