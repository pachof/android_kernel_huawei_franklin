 ...
 *
 * huawei charger or battery proxy for mtk
 *
...
int power_proxy_get_filter_sum(void)
{
	if (!g_power_proxy_dev || !g_power_proxy_dev->battery_proxy)
		return huawei_capacity_get_filter_sum(1);

	/* 100: soc base */
	return bat_ui_capacity_get_filter_sum(100);
}

void power_proxy_sync_filter_soc(int rep_soc, int round_soc)
{
	if (!g_power_proxy_dev || !g_power_proxy_dev->battery_proxy) {
	...
	/* 100: soc base */
	bat_ui_capacity_sync_filter(rep_soc, round_soc, 100);
}
...
int power_proxy_set_vterm_dec(int value)
{
	...
	if (value >= vol_max.intval) {
		hwlog_err("vdec=%d,vol_max=%d\n", value, vol_max.intval);
		return -1;
	}

	vol_max_set.intval = vol_max.intval - value;
...
static int power_proxy_get_int_property(const char *name,
...
int power_proxy_get_bat_voltage(int *val)
{
	...
	if (power_proxy_get_int_property("battery",
...
int power_proxy_get_capacity(int *val)
{
	...
	if (power_proxy_get_int_property("assist_battery",
	...
	unsigned long event, void *data)
{
	...
	case CHARGER_NOTIFY_ERROR:
		power_event_notify(POWER_EVENT_NE_SUSPEND_CHARGING, NULL);
		break;
	default:
		break;
	}
