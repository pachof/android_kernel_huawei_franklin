...
static void bq25970_dump_register(void *dev_data)
{
	u8 i;
	int ret;
	u8 val = 0;
...
static int bq25970_get_tsbus_percentage(struct bq25970_device_info *di, long *tsbus_per)
{
	int ret;
	s16 data = 0;
	s16 adc_value;
...
static int bq25970_get_tsbus_ntc_resistor(int adc_channel, long *data, void *dev_data)
{
	int ret;
	long tsbus_per = 0;
	long r_temp;
...
static int bq25970_value_dump(char *buffer, int size, void *dev_data)
{
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN)
		snprintf(buffer, size,
			...
			ibus, vbus, ibat, vusb, vout, bq25970_get_vbat_mv(dev_data), temp);
	else
		snprintf(buffer, size,
			"%-7d%-7d%-7d%-7d%-7d%-7d%-7d  ",
			ibus, vbus, ibat, vusb, vout, bq25970_get_vbat_mv(dev_data), temp);
...
static int bq25970_config_ibat_sns_res(struct bq25970_device_info *di, int data)
{
	...
	return bq25970_write_mask(di, BQ2597X_PULSE_MODE_REG,
		BQ2597X_IBAT_SNS_RES_MASK, BQ2597X_IBAT_SNS_RES_SHIFT, res_config);
}

static int bq25970_chip_init(struct bq25970_device_info *di)
{
	return 0;
}
...
static int bq25970_charge_init(void *dev_data)
{
	...
	di->device_id = bq25970_get_device_id((void *)di);
	if (di->device_id == -1)
		return -1;
...
static void bq25970_fault_handle(struct bq25970_device_info *di, struct nty_data *data)
{
	int val = 0;
	...
	if (ac_protection & BQ2597X_AC_OVP_FLAG_MASK) {
		...
		val = bq25970_get_vbat_mv((void *)di);
		...
		bq25970_get_ibat_ma(&val, (void *)di);
		...
		bq25970_get_vbus_mv(&val, (void *)di);
		...
		bq25970_get_ibus_ma(&val, (void *)di);
...
static void bq25970_interrupt_work(struct work_struct *work)
{
	...
	ret |= bq25970_read_byte(di, BQ2597X_CONVERTER_STATE_REG, &converter_state);
	if (ret)
...
static struct direct_charge_ic_ops bq25970_aux_sysinfo_ops = {
...
static struct direct_charge_batinfo_ops bq25970_aux_batinfo_ops = {
	.init = bq25970_batinfo_init,
	.exit = bq25970_batinfo_exit,
	.get_bat_btb_voltage = bq25970_get_vbat_mv,
	.get_bat_package_voltage = bq25970_get_vbat_mv,
...
static int bq25970_ops_register(struct bq25970_device_info *di)
{
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		...
		ret = dc_sc_ops_register(di->ic_role, &bq25970_aux_sysinfo_ops);
		ret |= dc_batinfo_ops_register(SC_MODE, di->ic_role, &bq25970_aux_batinfo_ops);
	}
	if (ret) {
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		...
		ret = power_tz_ops_register(&bq25970_aux_temp_sensing_ops, "bq25970_aux");
	...
	const struct i2c_device_id *id)
{
	...
	ret = bq25970_get_device_id((void *)di);
	if (ret < 0)
		goto bq25970_fail_0;
