...
#include <huawei_platform/power/huawei_charger_null.h>
...
static enum charger_type fsa9685_get_charger_type(void)
{
	...
	if ((chg_type == CHARGER_UNKNOWN) && is_fcp_charger_type()) {
		chg_type = STANDARD_CHARGER;
...
static int usb_switch_chg_type_det(bool en)
{
	...
	(void)rt8979_psy_online_changed(di, en);
	if (is_rt8979())
...
static void fsa9685_resume_irq(void)
{
	...
	hwlog_info("%s: dcd time out 0x0e=0x%x\n", __func__, fsa9685_read_reg(FSA9685_REG_CONTROL2));
}
...
static int fsa9685_is_water_intruded(void)
{
	u8 usb_id;
	int i;
	enum huawei_charger_type charger_type = huawei_get_charger_type();
