...
#endif
#include <huawei_platform/power/soc_decimal_ne.h>
...
static bool first;
static int g_daemon_ui_soc;

/* ============================================================ */
/* gauge hal interface */
/* ============================================================ */

static int huawei_capacity_pulling_filter(int curr_capacity)
{
	unsigned int index = gm.capacity_filter_count++ % WINDOW_LEN;
...
int huawei_capacity_get_filter_soc(void)
{
	return capacity_sum / WINDOW_LEN;
}

int huawei_capacity_get_daemon_uisoc(void)
{
	return g_daemon_ui_soc;
}

/* exit scp (example : 87.5% to 87%) */
static void huawei_reset_capacity_fifo(int curr_capacity)
{
	unsigned int i = 0;
	...
	bm_err("%s reset capacity_filter to %d\n", __func__, curr_capacity);
}

/* keep ui_soc <= 100% (example: 100.5% to 100%) */
void bound(int *soc)
{
	if (*soc > 100)
		*soc = 100;
...
void huawei_capacity_sync_filter(int rep_soc)
{
	...
	huawei_reset_capacity_fifo(rep_soc);
	bm_err("%s rep_soc:%d gm.ui_soc=%d\n",
		__func__, rep_soc, gm.ui_soc);
	/* step2: capacity changed (100:base) */

	gm.ui_soc = (rep_soc + UI_OFFSET) / 100;
	bm_err("%s aft rep_soc:%d  gm.ui_soc=%d\n",
		__func__, rep_soc, gm.ui_soc);

	if (prev_soc != gm.ui_soc) {
		bound(&gm.ui_soc);
		battery_main.BAT_CAPACITY = gm.ui_soc;
		battery_update_sync(&battery_main);
	}
	prev_soc = gm.ui_soc ;
}

static int huawei_soc_filter_adapter(int daemon_ui_soc)
{
	int delta_soc_dec = 0;
	int filter_soc = capacity_sum / WINDOW_LEN;
	int soc_update = filter_soc;

	bm_err("daemon_ui_soc:%d g_daemon_ui_soc:%d filter_soc:%d\n",
		daemon_ui_soc, g_daemon_ui_soc, filter_soc);
	if (daemon_ui_soc < g_daemon_ui_soc) {
		if (daemon_ui_soc < filter_soc) {
			delta_soc_dec = g_daemon_ui_soc - daemon_ui_soc;
			/* soc_dec no more than 100 (1%) */
			delta_soc_dec = delta_soc_dec > 100 ? 100 : delta_soc_dec;
			soc_update = filter_soc - delta_soc_dec;
			/* 0 : min of soc */
			soc_update = soc_update < 0 ? 0 : soc_update;
		} else {
			soc_update = filter_soc;
		}
		huawei_reset_capacity_fifo(soc_update);
	} else {
		if (battery_main.BAT_STATUS != POWER_SUPPLY_STATUS_CHARGING) {
			soc_update = filter_soc;
			huawei_reset_capacity_fifo(soc_update);
		} else {
			soc_update = huawei_capacity_pulling_filter(daemon_ui_soc);
			/* filter_soc should chase daemon_soc faster */
			if (daemon_ui_soc >= 9900 && filter_soc >= 9800)
				soc_update = huawei_capacity_pulling_filter(daemon_ui_soc);
		}
	}
	g_daemon_ui_soc = daemon_ui_soc;
	return soc_update;
}
...
int get_pl_charger_status(void)
{
	/* pl_charger_status will be true when reboot with charger */
	return gm.hw_status.pl_charger_status;
}
...
void fg_custom_init_from_dts(struct platform_device *dev)
{
	...
	g_batt_id_volt_by_adc = of_property_read_bool(np, "batt_id_volt_by_adc");
	bm_info("batt_id read:%d\n", g_batt_id_volt_by_adc);
	huawei_mtk_battery_dts_parse(np);
	battid_parse_para(np);
	fgauge_get_profile_id();
...
static void huawei_start_uisoc_timer(void)
{
	int ret;
	ktime_t ktime;
	...
	ret = alarm_try_to_cancel(&gm.huawei_soc_timer);
	if (ret < 0)
		return;

	get_monotonic_boottime(&time_now);
	time.tv_sec = 60; /* 60s to start soc timer */
	time.tv_nsec = 0;
	end_time = timespec_add(time_now, time);
	ktime = ktime_set(end_time.tv_sec, end_time.tv_nsec);
	alarm_start(&gm.huawei_soc_timer, ktime);
}

static enum alarmtimer_restart huawei_soc_timer_callback(
	struct alarm *alarm, ktime_t now)
{
	gm.huawei_cb_flag = 1;
	wake_up(&gm.wait_que);
...
static int huawei_check_uisoc_proc(void)
{
	int chg_status = mt_get_charger_status();
	bool chg_tp = mt_get_charger_type();
	bool chg_done = huawei_charge_done();

	bm_err("%s ui_soc:%d ch_dn:%d ch_tp:%d chg_sts:%d daemon_ui_soc:%d\n",
		__func__, gm.ui_soc, chg_done, chg_tp, chg_status, g_daemon_ui_soc);
	/* 100: soc full, 9900:threshold of last soc before full */
	if (gm.ui_soc == 100 || !chg_done || chg_tp == CHARGER_UNKNOWN ||
		chg_status == CHR_ERROR || g_daemon_ui_soc < 9900) {
		bm_err("conditions not satisfy\n");
		return -1;
	}
	return 0;
}

static void huawei_set_uisoc_full(void)
{
	int curr_soc;

	/* 10000: set soc full */
	curr_soc = huawei_capacity_pulling_filter(10000);

	/* 100: convert to percent */
	gm.ui_soc = (curr_soc + UI_OFFSET) / 100;
	bm_err("%s curr_soc:%d gm.ui_soc=%d\n", __func__, curr_soc, gm.ui_soc);
...
static void huawei_set_uisoc_full_proc(void)
{
	huawei_set_uisoc_full();
	if (gm.ui_soc != 100) /* 100: soc full */
		huawei_start_uisoc_timer();
	else
		alarm_cancel(&gm.huawei_soc_timer);
}
...
int battery_update_routine(void *x)
{
	...
	while (1) {
		...
		if (gm.huawei_cb_flag > 0) {
			gm.huawei_cb_flag = 0;
			ret = huawei_check_uisoc_proc();
			if (ret < 0)
				alarm_cancel(&gm.huawei_soc_timer);
			else
				huawei_set_uisoc_full_proc();
		}
...
void fg_daemon_comm_INT_data(char *rcv, char *ret)
{
	...
	case FG_SET_AGING_FACTOR:
		{
			if (is_set_aging_fatcor() == false)
				break;

			gm.aging_factor = prcv->input;
			huawei_set_aging_factor();
		}
		break;
	case FG_SET_QMAX:
		{
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_KERNEL_UISOC:
	{
		...
		if (first == false) {
			first = true;
			g_daemon_ui_soc = daemon_ui_soc;
			huawei_reset_capacity_fifo(daemon_ui_soc);
		} else {
			daemon_ui_soc = huawei_soc_filter_adapter(daemon_ui_soc);
		}

		fg_cust_data.ui_old_soc = daemon_ui_soc;
		old_uisoc = gm.ui_soc;

		huawei_set_ui_soc(daemon_ui_soc);
		if (gm.disableGM30 == true)
			gm.ui_soc = 50;
		else
			/* 100: calculation convert base */
			gm.ui_soc = (daemon_ui_soc + UI_OFFSET) / 100;
		bm_err("[fg_res] FG_DAEMON_CMD_SET_KERNEL_UISOC = %d gm.ui_soc:%d GM3:%d old:%d\n",
			daemon_ui_soc, gm.ui_soc,
			gm.disableGM30, old_uisoc);
...
void mtk_battery_init(struct platform_device *dev)
{
	...
	gm.aging_factor = 10000; /* 10000: mtk aging factor */
	...
	alarm_init(&gm.huawei_soc_timer, ALARM_BOOTTIME,
		huawei_soc_timer_callback);
