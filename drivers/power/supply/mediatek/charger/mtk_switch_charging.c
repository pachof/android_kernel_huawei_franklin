...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	chr_err("force:%d thermal:%d,%d pe4:%d,%d,%dsetting:%d %d type:%d usb_unlimited:%d usbif:%d usbsm:%d aicl:%d atm:%d\n",
...
#ifdef CONFIG_DIRECT_CHARGER
		// if (info->force_disable_dc_path && (direct_charge_in_charging_stage() ==
...
static int mtk_switch_charging_run(struct charger_manager *info)
{
	...
	set_charging_time(swchgalg->total_charging_time);
