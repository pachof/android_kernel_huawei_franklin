...
static void sgm41511h_i2c_err_monitor(void)
{
	...
	if (i2c_err_cnt >= I2C_ERR_MAX_COUNT) {
		...
		power_dsm_dmd_report(POWER_DSM_BATTERY, ERROR_CHARGE_I2C_RW,
			"ERROR_CHARGE_I2C_RW");
	}
}
...
static int sgm41511h_device_check(void)
{
	u8 reg = 0;
	int ret;
	...
	if (((reg & SGM41511H_REG_VPRS_PN_MASK) >>
		SGM41511H_REG_VPRS_PN_SHIFT) == VENDOR_ID) {
...
static int sgm41511h_disable_otg_usb_plugout(struct charger_device *chg_dev)
{
	...
	pr_info("sgm41511h_disable_otg_usb_plugout\n");
...
static int sgm41511h_get_register_head(struct charger_device *chg_dev,
...
static int sgm41511h_hw_dump_register(struct charger_device *chg_dev,
...
static struct charger_ops sgm41511h_ops = {
	...
	.dump_registers = NULL,
	.dump_reg = sgm41511h_hw_dump_register,
	.get_reg_head  = sgm41511h_get_register_head,
...
static void sgm41511h_irq_work(struct work_struct *work)
{
	...
	pr_info("boost_ovp_reg REG%x=0x%x\n", SGM41511H_REG_F, reg);

	if (reg & SGM41511H_REG_F_FAULT_BOOST_MASK)
		pr_info("CHARGE_FAULT_BOOST_OCP happened\n");
	...
	const struct i2c_device_id *id)
{
	...
	if (sgm41511h_device_check() == CHARGE_IC_BAD) {
