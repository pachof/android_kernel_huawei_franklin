...
/* static func, all parameters have been checked or assured already */
static enum oeminfo_region_valid_t get_oeminfo_valid_region(int index,
	unsigned int *age)
{
...
static unsigned int get_oeminfo_offset(int index,
	unsigned int *valid_offset, unsigned int *invalid_offset,
	unsigned int *age)
{
	enum oeminfo_region_valid_t valid_region;
	unsigned int offset = get_oeminfo_base_offset(index);

	if (valid_offset == NULL || invalid_offset == NULL || age == NULL) {
		pr_err("%s para is null\n", __func__);
		return -1;
	}
	if (offset == OEMINFO_OFFSET_ERR)
		return offset;
	valid_region = get_oeminfo_valid_region(index, age);
	if (valid_region == OEMINFO_REGION_ERROR) {
		pr_err("%s: get valid oeminfo region fail\n", __func__);
		return -1;
	}
	if (valid_region == OEMINFO_USING_REGION_VALID) {
		*invalid_offset = offset + OEMINFO_BACKUP_REGION_OFFSET;
		*valid_offset = offset;
		return 0;
	}
	if (valid_region == OEMINFO_BACKUP_REGION_VALID) {
		*invalid_offset = offset;
		*valid_offset = offset + OEMINFO_BACKUP_REGION_OFFSET;
		return 0;
	}
...
static int get_oeminfo(int index, int len, char *data)
{
	...
	if (get_oeminfo_offset(index, &valid_offset,
		&invalid_offset, &age) != 0) {
		pr_err("%s: get_oeminfo_offset fail, index = %d\n",
			__func__, index);
		return -1;
	}
	if (oeminfo_read(PART_OEMINFO, valid_offset,
		OEMINFO_PAGE_SIZE, (unsigned char *)oeminfo_hdr_page_buff)) {
	...
	if ((oeminfo_hdr->magic_number1 == OEMINFO_MAGIC1) &&
		...
		return oeminfo_read(PART_OEMINFO,
			valid_offset + OEMINFO_PAGE_SIZE,
	...
	unsigned int age)
{
	...
	oeminfo_hdr.oeminfo_age = (age + 1) % OEMINFO_MAX_AGE;
...
static int init_oeminfo_para(struct oeminfo_base_info *base, int idx)
{
	int ret;
	unsigned int age;
	...
	base->index = (unsigned int)idx;

	if (get_oeminfo_offset(idx, &base->validOffset,
		&base->invalidOffset, &age) != 0) {
		pr_err("%s: index(%u), GetOeminfoOffset failed\n",
			__func__, (unsigned int)idx);
		return -1;
	}

	ret = get_oeminfo_size(idx);
	if (ret == 0) {
		pr_err("%s: index(%u), GetOeminfoSize failed\n",
			__func__, (unsigned int)idx);
		return -1;
	}
	base->size = (unsigned int)ret;

	return 0;
}

static int erase_oeminfo(int index)
{
	errno_t ret;
	int idx = index;
	unsigned char data[OEMINFO_8K_AREA_SIZE];
	struct oeminfo_base_info para;

	if (index < 0) {
		pr_err("%s: bad parametes, index = %d\n", __func__, index);
		return -1;
	}
	if (!oeminfo_index_adapt(&idx)) {
		pr_err("%s: invalid index: %d\n", __func__, index);
		return -1;
	}
	if (init_oeminfo_para(&para, idx) != 0) {
		pr_err("%s init oeminfo failed\n", __func__);
		return -1;
	}
	ret = memset_s(data, OEMINFO_8K_AREA_SIZE, OEMINFO_BLOCK_ERASE_VALUE,
		OEMINFO_8K_AREA_SIZE);
	if (ret != 0) {
		pr_err("%s init data failed\n", __func__);
		return -1;
	}
	if (oeminfo_write(PART_OEMINFO, para.validOffset, para.size, data)) {
		pr_err("%s: oeminfo write %d fail\n", __func__, index);
		return -1;
	}
	if (oeminfo_write(PART_OEMINFO, para.invalidOffset, para.size, data)) {
		pr_err("%s: oeminfo write %d fail\n", __func__, index);
		return -1;
	}
	return 0;
...
static int set_oeminfo(int index, int len, const char *data)
{
	...
	if (oeminfo_write(PART_OEMINFO, invalid_offset + OEMINFO_PAGE_SIZE,
		len, (unsigned char *)data)) {
		pr_err("%s: oeminfo write %d fail\n", __func__, index);
		return -1;
	}
	ret = write_oeminfo_hdr(index, len, invalid_offset, age);
	if (ret != 0) {
		...
		struct oeminfo_info_user *user_info)
{
	...
	if (user_info->oeminfo_operation == OEMINFO_READ)
		return get_oeminfo(user_info->oeminfo_id,
			user_info->valid_size, user_info->oeminfo_data);

	if (user_info->oeminfo_operation == OEMINFO_WRITE)
		return set_oeminfo(user_info->oeminfo_id,
			user_info->valid_size, user_info->oeminfo_data);
