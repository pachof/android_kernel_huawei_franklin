...
void tcpci_set_cc_mode(int mode)
{
	...
	if (!g_tcpc_dev) {
		pr_info("g_tcpc_dev or tcpc is NULL!\n");
		return;
	}
...
int tcpci_get_cc_mode(void)
{
	...
	if (!g_tcpc_dev) {
		pr_info("g_tcpc_dev tcpc is NULL!\n");
...
int tcpci_get_cc_status(void)
{
	...
	ret = g_tcpc_dev->ops->get_cc_status();
...
int tcpci_set_vbus_cc_short_detection(struct tcpc_device *tcpc, bool cc1,
				      bool cc2)
{
	if (tcpc->ops->set_vbus_cc_short_detection)
		return tcpc->ops->set_vbus_cc_short_detection(tcpc, cc1, cc2);
	return 0;
}

int tcpci_notify_vbus_cc_short(struct tcpc_device *tcpc)
{
	struct tcp_notify tcp_noti;

	tcp_noti.vbus_cc_short.cc1 = tcpc->vbus_cc1_short;
	tcp_noti.vbus_cc_short.cc2 = tcpc->vbus_cc2_short;
	return tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_MISC,
...
int tcpci_notify_typec_state(struct tcpc_device *tcpc)
{
	...
	ret = pd_dpm_handle_pe_event(PD_DPM_PE_EVT_TYPEC_STATE, &tcp_noti.typec_state);
	if (ret < 0)
		pr_err("handle pe event fail\n");
	...
	struct tcpc_device *tcpc, uint8_t event, uint8_t role)
{
	...
	ret = pd_dpm_handle_pe_event(event, &swap_state);
	ret = tcpc_check_notify_time(tcpc, &tcp_noti,
