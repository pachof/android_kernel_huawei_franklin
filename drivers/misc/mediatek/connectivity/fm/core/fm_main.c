...
/* return NULL; */
	...
	if (!fm->eint_wk) {
		...
		fm->eint_wk->init(fm->eint_wk, fm_eint_work_func, (struct work_struct *)fm);
	}
	...
	if (!fm->rst_wk) {
		...
		fm->rst_wk->init(fm->rst_wk, fm_subsys_reset_work_func, (struct work_struct *)fm);
	}
	...
	if (!fm->rds_wk) {
		...
		fm->rds_wk->init(fm->rds_wk, fm_rds_reset_work_func, (struct work_struct *)fm);
	}
	...
	if (!fm->ch_valid_check_wk) {
		...
		fm->ch_valid_check_wk->init(fm->ch_valid_check_wk,
			fm_cqi_check_work_func, (struct work_struct *)fm);
	}
#endif
	...
	if (!fm->fm_tx_power_ctrl_work) {
		...
		fm->fm_tx_power_ctrl_work->init(fm->fm_tx_power_ctrl_work,
						fm_tx_power_ctrl_worker_func, (struct work_struct *)fm);
	}
	...
	if (!fm->fm_tx_desense_wifi_work) {
		...
		fm->fm_tx_desense_wifi_work->init(fm->fm_tx_desense_wifi_work,
						  fm_tx_desense_wifi_worker_func, (struct work_struct *)fm);
	}
	...
	if (!fm->pwroff_wk) {
		...
		fm->pwroff_wk->init(fm->pwroff_wk, fm_pwroff_work_func, (struct work_struct *)fm);
	}
