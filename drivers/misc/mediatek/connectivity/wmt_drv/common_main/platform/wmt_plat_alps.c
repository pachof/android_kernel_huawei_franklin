...
extern SIZE_T  mcu_conn_hif_off_base;

static SIZE_T consys_get_remap_addr(UINT32 addr)
{
	UINT32 base = addr & 0xFFFFF000;
	UINT32 offset = addr & 0xFFF;

	if (base == 0x18007000)
		return conn_reg.mcu_conn_hif_on_base + offset;
	else if (base == 0x18004000)
		return conn_reg.mcu_conn_hif_pdma_base + offset;
	else if (base == 0x180BC000)
		return mcu_conn_hif_off_base + offset;
	else
		return 0;
}

static VOID consys_read_register(UINT32 addr)
{
        SIZE_T reg_base;

	if (mtk_consys_is_connsys_reg(addr) && !wmt_lib_reg_readable()) {
                WMT_INFO_FUNC("[R] 0x%08x (not readable)\n", addr);
		return;
	}

	reg_base = consys_get_remap_addr(addr);
	if (reg_base)
		WMT_INFO_FUNC("[R] 0x%08x = 0x%08x\n", addr, CONSYS_REG_READ(reg_base));
	else
		WMT_INFO_FUNC("[R] %0x08% (remap fail)\n", addr);
}

static VOID consys_write_register(UINT32 addr, UINT32 val, UINT32 mask)
{
        SIZE_T reg_base;

	if (mtk_consys_is_connsys_reg(addr) && !wmt_lib_reg_readable()) {
                WMT_INFO_FUNC("[W] %08x = %08x (not readable)", addr, val);
		return;
	}

	reg_base = consys_get_remap_addr(addr);
        if (reg_base) {
		CONSYS_REG_WRITE_MASK(reg_base, val, mask);
                WMT_INFO_FUNC("[W] %08x = %08x", addr, CONSYS_REG_READ(reg_base));
        } else
		WMT_INFO_FUNC("[W] %0x08% (remap fail)\n", addr);
}
...
struct action_item action_table[] = {
	{0, 0x18007000, 0, 0},
	{0, 0x18007004, 0, 0},
	{0, 0x18007008, 0, 0},
	{0, 0x1800700C, 0, 0},
	...
	{1, 0x18007014, 0x00, 0x7e},
	{0, 0x18007104, 1, 0},
	{1, 0x18007014, 0x02, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x04, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x06, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x08, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x0a, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x0c, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x0e, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x10, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x12, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x14, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x16, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x18, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x1a, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x1c, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x1e, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x20, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x22, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x24, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x26, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x28, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x2a, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x2c, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x2e, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x30, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x32, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x34, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x36, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x38, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x3a, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x3c, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x3e, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x40, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x42, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x44, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x46, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x48, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x4a, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x4c, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x4e, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x50, 0x7e},
	{0, 0x18007104, 0, 0},
	{1, 0x18007014, 0x7e, 0x7e},
	{0, 0x18007104, 0, 0},
	...
	{0, 0x18004168, 0, 0},
	{0, 0x18004138, 0, 0},
	{1, 0x18004124, 0x100, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x101, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x102, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x103, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x104, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x105, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x106, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x107, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x108, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x109, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x10a, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x10b, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x10c, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x10d, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x10e, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x10f, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x110, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x111, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x112, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x18004124, 0x113, 0xfff},
	{0, 0x18004128, 0, 0},
	{1, 0x1800412c, 0x2222, 0xffff},
	{0, 0x18004130, 0, 0},
	{0, 0x18004154, 0, 0},
	{0, 0x18004160, 0, 0},
};

VOID consys_print_dbg_log(VOID)
{
	INT32 i;

	if (wmt_lib_power_lock_aquire()) {
		WMT_INFO_FUNC("fail to get power lock\n");
		return;
	}

	for (i = 0; i < sizeof(action_table)/sizeof(struct action_item); i++) {
		if (action_table[i].action == 0)
			consys_read_register(action_table[i].addr);
		else
			consys_write_register(action_table[i].addr,
					action_table[i].val,
					action_table[i].mask);
	}
