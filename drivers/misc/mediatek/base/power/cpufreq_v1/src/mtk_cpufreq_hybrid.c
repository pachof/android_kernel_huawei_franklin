...
int Ripi_cpu_dvfs_thread(void *data)
{
	...
	/* tag_pr_info("mtk_ipi_register */
	/*IPI_ID_CPU_DVFS pass!!(%d)\n", ret); */

	/* an endless loop in which we are doing our work */
	do {
		/* tag_pr_info("mcupm_ipidev IPI_ID_CPU_DVFS\n"); */
...
int cpuhvfs_set_dvfs(int cluster_id, unsigned int freq)
{
...
#ifdef FIX_FREQ_WORKAROUND
	unsigned int cluster_num = 2;
	struct device_node *cn, *map, *cluster, *d;
	unsigned int max, min;
	unsigned int max_ret, min_ret;
	char name[10];
	int i, cnt = 0;
#endif
	...
	for (i = 0; i < cluster_num ; i++) {
		...
		if (!cluster)
			goto ORIG_FLOW;
		d = of_get_child_by_name(cluster, "doe");
		if (!d)
			goto ORIG_FLOW;
		max_ret = of_property_read_u32(d, "max", &max);
		min_ret = of_property_read_u32(d, "min", &min);
		if (max_ret && min_ret)
			cnt++;
	}
	if (cnt == cluster_num)
...
int cpuhvfs_get_volt(int buck_id)
{
	...
	ret = dvfs_to_mcupm_command(IPI_GET_VOLT, &cdvfs_d);
...
int cpuhvfs_get_freq(int pll_id)
{
	...
	ret = dvfs_to_mcupm_command(IPI_GET_FREQ, &cdvfs_d);
...
static int dvfsp_module_init(void)
{
	...
	if (r)
		tag_pr_notice("fail to register mcupm driver @ %s()\n",
								__func__);

	if (!dvfsp_probe_done) {
		tag_pr_notice("FAILED TO PROBE MCUPM DEVICE\n");
		return -ENODEV;
	}
...
static int cpuhvfs_pre_module_init(void)
{
	...
	r = dvfsp_module_init();
	if (r) {
		tag_pr_notice("FAILED TO INIT DVFS MCUPM (%d)\n", r);
		return r;
	}
