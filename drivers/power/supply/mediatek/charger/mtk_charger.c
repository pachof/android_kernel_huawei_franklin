...
struct charger_manager *huawei_get_pinfo(void)
{
	return pinfo;
}

bool get_kpoc_wakelock_flag(void)
{
	return pinfo->enable_kpoc_wakelock;
}
			...
			struct charger_consumer *consumer, bool en)
{
	...
	if (info->enable_hv_charging == false &&
		...
		set_direct_flag(1); // 1 means ignore error
		cam_stop_charging();
	}
	...
	bool en)
{
	...
	if (info != NULL) {
		charger_dev_enable_eoc(info->chg1_dev, true);
		clear_cur_delay_eoc_count();
		chr_info("%s enable eoc\n", __func__);
		...
		case SLAVE_CHARGER:
			...
			break;
		case OTHER_CHARGER:
			chr_info("%s: OTHER_CHARGER is doing bc12 en:%d\n", __func__, en);
			ret = usbswitch_common_chg_type_det(en);
			if (ret < 0) {
				chr_err("%s: OTHER_CHARGER do bc12 fail\n", __func__);
				return ret;
			}
			return 0;
		default:
...
{
	struct sw_jeita_data *sw_jeita = NULL;
				...
				enum charger_type chr_type)
{
	...
	/* set aicr is 475ma when plug in, follow hisi */
	charger_dev_set_input_current(info->chg1_dev, 475000);
...
static void mtk_battery_notify_VBatTemp_check(struct charger_manager *info)
{
	...
	info->thermal.max_charge_temp = HLTHERM_HIGH_TEMP;
#endif
...
#ifdef CONFIG_HLTHERM_RUNTEST
	if (info->battery_temp >= HLTHERM_HIGH_TEMP) {
...
static void check_direct_charge(void)
{
	if (!pinfo)
		return;

	if (insert_flag == 1 &&  pinfo->enable_hv_charging == false) {
		direct_charge_check(pinfo->enable_hv_charging);
		insert_flag = 0; /* clean first insert */
	} else if (pinfo->enable_hv_charging == true) {
		direct_charge_check(pinfo->enable_hv_charging);
	}
}
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		if (info->charger_thread_polling == true)
			mtk_charger_start_timer(info);
		chr_type = mt_get_charger_type();
		chr_err("close v600 buck\n");
		(void)dc_set_ic_buck_enable(SC_MODE, CHARGE_IC_MAIN, 0);
		chr_err("stage=%d chr_type = %d\n", direct_charge_in_charging_stage(), chr_type);
		if (is_charger_on && direct_charge_in_charging_stage() == DC_NOT_IN_CHARGING_STAGE &&
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "fcp_charger_current", &val) >= 0) {
		info->data.fcp_charger_current = val;
	} else {
		chr_err("use default FCP_CHARGER_CURRENT:%d\n",
			FCP_CHARGER_CURRENT);
		info->data.fcp_charger_current = FCP_CHARGER_CURRENT;
	}
	...
	if (of_property_read_u32(np, "fcp_charger_input_current", &val) >= 0)
		info->data.fcp_charger_input_current = val;
	else {
		chr_err("use default FCP_CHARGER_INPUT_CURRENT:%d\n",
			FCP_CHARGER_INPUT_CURRENT);
		info->data.fcp_charger_input_current = FCP_CHARGER_INPUT_CURRENT;
	}
	...
	info->is_support_pd_chg = !of_property_read_bool(np, "not_support_pd_charge");
	chr_err("support_pd_charge is %d\n", info->is_support_pd_chg);

	info->enable_kpoc_wakelock = of_property_read_bool(np, "enable_kpoc_wakelock");
	chr_err("enable_kpoc_wakelock is %d\n", info->enable_kpoc_wakelock);

	charger_parse_dt(np, info);
...
#ifdef CONFIG_HLTHERM_RUNTEST
	/* 2000000: 2000 mA, set default input current for hltherm test */
	info->chg1_data.input_current_limit = 2000000;
