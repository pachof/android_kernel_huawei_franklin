...
static int8_t nvt_ts_check_chip_ver_trim(void)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		TS_LOG_INFO("ts_check_chip_ver_trim: buf[1]=0x%02X, buf[2]=0x%02X, buf[3]=0x%02X, buf[4]=0x%02X, buf[5]=0x%02X, buf[6]=0x%02X\n",
					     ...
					     uint8_t gesture_id)
{
	...
	switch (gesture_id) {
		case DOUBLE_CLICK_WAKEUP:
			...
			break;
		default:
			TS_LOG_INFO("wakeup_gesture_report: unknown gesture detected!\n");
			return RESULT_ERR;
	}
