...
#ifdef CONFIG_HW_MTK_RTG_SCHED
static int find_rtg_cpu(struct sched_domain *sd,
	struct task_struct *task, struct cpumask *lowest_mask)
{
	...
	do {
		...
		if (!cpumask_intersects(lowest_mask, sched_group_cpus(sg)))
			continue;
		if (cpu_prefer(task) == SCHED_PREFER_BIG) {
...
static int find_lowest_rq(struct task_struct *task)
{
	...
	rcu_read_unlock();
	if (grp) {
		rtg_cpu = find_rtg_cpu(sd, task, lowest_mask);
		if (rtg_cpu != -1)
			return rtg_cpu;
	}
#endif
...
static bool task_fits_max(struct task_struct *p, int cpu)
{
	...
	if (capacity * 1024 > task_util(p) * 1280 &&
		capacity >= boosted_task_util(p))
		return true;
