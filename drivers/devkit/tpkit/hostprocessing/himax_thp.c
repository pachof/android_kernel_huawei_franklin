...
static ssize_t touch_driver_spi_sync(struct thp_device *tdev,
	...
	uint8_t *buf, unsigned int length)
{
	...
	return touch_driver_spi_sync(tdev, &m);
}
...
static int touch_driver_9l_communication_check(
...
static int touch_driver_83112_communication_check(
		struct thp_device *tdev)
{
	uint8_t tmp_addr[HIMAX_NORMAL_ADDR_LEN] = {0};
	uint8_t *tmp_data = g_himax_spi_buf.r_buf_a;
