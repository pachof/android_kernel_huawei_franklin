...
static struct mt6360_chip *g_chip;
...
RT_REG_DECL(MT6360_REG_CC_CTRL2, 1, RT_VOLATILE, {});
...
#ifdef CONFIG_VBUS_CC_SHORT_DETECTION
	if (tcpc->tcpc_flags & TCPC_FLAGS_VBUS_CC_SHORT_DETECTION)
		mask[MT6360_VEND_INT3] |= MT6360_M_VBUS_TO_CC;
#endif /* CONFIG_VBUS_CC_SHORT_DETECTION */
...
#ifdef CONFIG_VBUS_CC_SHORT_DETECTION
static int mt6360_vbus_to_cc_evt_process(struct tcpc_device *tcpc)
{
	int ret;
	u8 status;
	...
	if (ret < 0)
		return ret;
	MT6360_INFO("%s 0x%02X\n", __func__, status);
	if (status & MT6360_M_VBUS_TO_CC1)
		cc1 = true;
	if (status & MT6360_M_VBUS_TO_CC2)
		cc2 = true;
	return tcpc_typec_handle_vbus_cc_short(tcpc, cc1, cc2);
}

static int mt6360_vbus_to_cc2_irq_handler(struct tcpc_device *tcpc)
{
	return mt6360_vbus_to_cc_evt_process(tcpc);
}

static int mt6360_vbus_to_cc1_irq_handler(struct tcpc_device *tcpc)
{
	return mt6360_vbus_to_cc_evt_process(tcpc);
}
#endif /* CONFIG_VBUS_CC_SHORT_DETECTION */
...
#ifdef CONFIG_VBUS_CC_SHORT_DETECTION
	MT6360_IRQ_MAPPING(22, vbus_to_cc1),
	MT6360_IRQ_MAPPING(23, vbus_to_cc2),
#endif /* CONFIG_VBUS_CC_SHORT_DETECTION */
...
#ifdef CONFIG_VBUS_CC_SHORT_DETECTION
static int mt6360_set_vbus_cc_short_detection(struct tcpc_device *tcpc,
					      bool cc1, bool cc2)
{
	int ret = 0;
	u8 cmpen = 0;
	...
	if (tcpc->tcpc_flags & TCPC_FLAGS_VBUS_CC_SHORT_DETECTION) {
		if (cc1)
			cmpen |= MT6360_CMPEN_VBUS_TO_CC1;
		if (cc2)
			cmpen |= MT6360_CMPEN_VBUS_TO_CC2;
		MT6360_INFO("%s (cc1,cc2)=(%d,%d)\n", __func__, cc1, cc2);
		ret = mt6360_i2c_update_bits(tcpc, MT6360_REG_CC_CTRL2,
					     cmpen, MT6360_CMPEN_VBUS_TO_CC);
	}
	return ret;
}
#endif /* CONFIG_VBUS_CC_SHORT_DETECTION */
...
static int mt6360_get_cc_state(void)
{
	int val = 0;
	u8 status = 0;
	int ret;
	if (!g_chip || !g_chip->tcpc)
		return -1;
	ret = mt6360_i2c_read8(g_chip->tcpc, TCPC_V10_REG_CC_STATUS, &status);
	if (ret < 0)
		return ret;
	pr_info("%s:cc_state_REG0x1D = 0x%x\n",__func__,status);
	val = status & CC_STATUS_MASK;
	return val;
}
...
#endif /* CONFIG_WATER_DETECTION */
	.get_cc_status = mt6360_get_cc_state,
#ifdef CONFIG_VBUS_CC_SHORT_DETECTION
	.set_vbus_cc_short_detection = mt6360_set_vbus_cc_short_detection,
#endif /* CONFIG_VBUS_CC_SHORT_DETECTION */
};
...
static int mt6360_tcpcdev_init(struct mt6360_chip *chip, struct device *dev)
{
	...
	chip->tcpc->tcpc_flags |= TCPC_FLAGS_VBUS_CC_SHORT_DETECTION;
	return 0;
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	set_tcpc_dev(chip->tcpc);

	ret = cc_check_ops_register(&cc_check_ops);
	if (ret)
		dev_err(chip->dev, "cc_check_ops register failed\n");
