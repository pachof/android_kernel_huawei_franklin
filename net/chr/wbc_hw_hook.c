...
u32 http_response_code(const char *pstr)
{
	...
	for (idx = 0; idx < CODE_LEN; idx++) {
		...
		if (ch >= '0' && ch <= '9')
			code = code * 10 + (ch - '0'); /* multiply 10 to add ch */
		else
			return 0;
	}
	return code;
...
{
	if ((ip6h == NULL && iph == NULL) || tcph == NULL)
		return;
...
static s64 get_dlen(char *p_http_str, bool is_ipv6_pkt, struct sk_buff *skb,
...
static bool is_http_para_out(s64 dlen, char *p_http_str)
{
	if (p_http_str == NULL)
		return false;
	...
	const struct nf_hook_state *state)
{
	...
	is_ipv6_pkt = (ntohs(skb->protocol) == ETH_P_IPV6);
	dlen = get_dlen(p_http_str, is_ipv6_pkt, skb, ip6h, iph);
	if (dlen < 0 || skb->dev == NULL || skb->dev->name == NULL ||
		skb->sk == NULL)
		return NF_ACCEPT;
