...
{
	int ret = 0;
	struct DISPLAY_PQ_T *pq_index = NULL;
...
static void mtk_color_bypass(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle, bool bypass)
{
	struct mtk_disp_color *color = comp_to_color(comp);
	...
	if (bypass == true) {
		...
		/* Enable color */
		cmdq_pkt_write(handle, comp->cmdq_base,
			       comp->regs_pa + DISP_COLOR_CFG_MAIN, (0 << 7), 0xFF);

		/* disable R2Y/Y2R in Color Wrapper */
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_COLOR_CM1_EN(color), 1, 0x1);
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_COLOR_CM2_EN(color), 0x11, 0x3);
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_COLOR_START(color), 0x1, 0x3);
	}
}
...
static int mtk_disp_color_probe(struct platform_device *pdev)
{
	...
	struct mtk_disp_color *priv = NULL;
