...
#define I2C3_BASE_ADDRE            0x11cb0000
#define I2C3_SECURE_DEFAULT_REG    0x08
...
static void secure_i2c_write(u32 value, u32 offset)
{
	struct arm_smccc_res res;
	arm_smccc_1_1_smc(MTK_SIP_KERNEL_I2C_DEVICE_WRITE_AARCH32,
		value, I2C3_BASE_ADDRE, offset, &res);
}

static void set_secure_slave_addr(u32 index, u32 slave_addr)
{
	struct arm_smccc_res res;
	arm_smccc_1_1_smc(MTK_SIP_KERNEL_I2C_SET_SLAVE_ADDR_AARCH32,
		I2C3_BASE_ADDRE, index, slave_addr, &res);
}

static inline void _i2c_writew(u16 value, struct mt_i2c *i2c, u16 offset)
{
	if (i2c->secure_mode) {
		secure_i2c_write(value, offset);
		return;
	}
...
static int mt_i2c_parse_dt(struct device_node *np, struct mt_i2c *i2c)
{
	...
	ret = of_property_read_u32(np, "secure_mode", &i2c->secure_mode);
	if (ret != 0)
		i2c->secure_mode = 0;

	pr_info("[i2c] i2c secure mode is %u\n", i2c->secure_mode);

	if (i2c->secure_mode == 1) {
		set_secure_slave_addr(0, I2C3_SECURE_DEFAULT_REG);
	}
	return 0;
}
