...
static bool pwrkey_long_press_work_init;
static bool pwrkey_long_press_timer_init;
#endif
...
{
	kpd_print("enter pwrkey_long_press_work_func\n");
...
static void hw_bootfail_long_press(unsigned long pressed)
{
	...
	if (!pwrkey_long_press_timer_init) {
		kpd_print("pwrkey long press timer is not init\n");
		return;
	}
	if (pressed && (!timer_pending(&long_press_pwrkey_timer))) {
		long_press_pwrkey_timer.expires = jiffies + 5 * HZ;
		long_press_pwrkey_timer.data = (unsigned long)pressed;
		long_press_pwrkey_timer.function = pwrkey_long_press;
...
#endif
#ifdef CONFIG_HW_ZEROHUNG
	hung_wp_screen_powerkey_ncb(WP_SCREEN_PWK_RELEASE ^ pressed);
#endif
}
#endif
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	pwrkey_long_press_timer_init = true;
