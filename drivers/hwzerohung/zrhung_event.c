...
int zrhung_send_event(enum zrhung_wp_id id, const char *cmd_buf, const char *msg_buf)
{
	...
	unsigned long flags;
#ifdef CONFIG_HW_FREEZEDETECTOR
	struct hiview_hievent* zrhung_event = hiview_hievent_create(RAW_EVENT_ZRHUNG);
	uint32_t pid = current->pid;
	uint32_t tgid = current->tgid;
	int count = 0;
#endif
	...
	*p = 0;
	...
	hwlog_info("zrhung send event from kernel: wp=%d, ret=%d", evt.wp_id,
		   ret);
#ifdef CONFIG_HW_FREEZEDETECTOR
	// send message from hievent
	hiview_hievent_put_integral(zrhung_event, "WATCHPOINTID", id);
	hiview_hievent_put_integral(zrhung_event, "PID", pid);
	hiview_hievent_put_integral(zrhung_event, "TGID", tgid);
	if (cmd_buf != NULL)
		hiview_hievent_put_string(zrhung_event, "CMD", cmd_buf);
	if (msg_buf != NULL)
		hiview_hievent_put_string(zrhung_event, "MSG", msg_buf);
	count = hiview_hievent_report(zrhung_event);
	if (count > 0) {
		ret = 0;
	} else {
		hwlog_err("hievent report error");
		ret = -EINVAL;
	}
	hiview_hievent_destroy(zrhung_event);
	hwlog_info("zrhung send hievent from kernel: wp=%d, ret=%d Pid = %d tgid = %d", evt.wp_id, ret, pid, tgid);
#endif
	return ret;
}
