...
static int g_simple_pa_vendor = TPA2025D1;
static int g_simple_pa_mode = 0;
static int g_simple_pa_gpio = -1;
...
static const char *const pa_mode_function[] = {
...
static int pa_mode_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	if (g_simple_pa_gpio < 0) {
		ucontrol->value.integer.value[0] = VENDOR_NUM;
		pr_err("%s invalid gpio %d", __func__, g_simple_pa_gpio);
		return 0;
	}

	ucontrol->value.integer.value[0] = g_simple_pa_vendor;
	return 0;
}

void simple_pa_mode_change(int enable)
{
	int i;
	int mode;
	...
	if (enable == 1) { // enable PA
		mode = (g_simple_pa_vendor == AW87318) ?
			g_simple_pa_mode : 1; // TI pa only support mode 1
		for (i = 0; i < mode; i++) {
			gpio_set_value(g_simple_pa_gpio, GPIO_OUT_ZERO);
			udelay(2); // AW87318 recommended pulse delay 2us
			gpio_set_value(g_simple_pa_gpio, GPIO_OUT_ONE);
			udelay(2); // AW87318 recommended pulse delay 2us
		}
	} else {
		gpio_set_value(g_simple_pa_gpio, GPIO_OUT_ZERO);
	}
	msleep(50); // AW87318 recommended delay 50ms,avoid pop
}

static int pa_mode_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s pa mode %ld", __func__, ucontrol->value.integer.value[0]);

	g_simple_pa_mode = ucontrol->value.integer.value[0];

	if (g_simple_pa_mode == 0)
		simple_pa_mode_change(false);
	else
		simple_pa_mode_change(true);

	return 0;
}

static const struct soc_enum dl_pa_mode_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(pa_mode_function), pa_mode_function),
};
...
static const struct snd_kcontrol_new mt6359_snd_controls[] = {
	...
	MT_SOC_ENUM_EXT_ID("Lineout_PGAR_GAIN", dl_pga_enum[0],
			   dl_pga_get, dl_pga_set,
			   AUDIO_ANALOG_VOLUME_LINEOUTR),
	SOC_ENUM_EXT("Ext_Speaker_PaMode", dl_pa_mode_enum[0],
			pa_mode_get, pa_mode_set),
};
...
static void simple_pa_vendor_init(struct platform_device *pdev, int gpio)
{
	...
	if (pinctrl == NULL) {
		pr_err("%s get pinctrl fail", __func__);
		return;
	}
	pin_state = pinctrl_lookup_state(pinctrl, "aud_pa_pin_default");
	if (IS_ERR(pin_state))
	...
	if (gpio_direction_input(gpio) != 0) {
		pr_err("%s: set gpio to input status fail", __func__);
		return;
	}

	if (gpio_get_value(gpio) == 0)
		g_simple_pa_vendor = AW87318;
	else
		g_simple_pa_vendor = TPA2025D1;
}

static void simple_pa_gpio_init(struct platform_device *pdev)
{
	int ret;
	int use_simple_pa;
	int simple_pa_gpio;
	...
	ret = of_property_read_u32(np, "use_simple_pa",
					(u32 *)&use_simple_pa);
	if ((ret != 0) || (use_simple_pa == 0)) {
		pr_info("%s not use simple pa", __func__);
		return;
	}

	simple_pa_gpio = of_get_named_gpio(np, "simple_pa_gpio", 0);
	if (simple_pa_gpio < 0) {
		pr_info("%s of_get_named_gpio fail %d", __func__,
			simple_pa_gpio);
		ret = of_property_read_u32(np, "simple_pa_gpio",
			(u32 *)&simple_pa_gpio);
		if (ret != 0) {
			simple_pa_gpio = -1;
			pr_err("%s get gpio fail", __func__);
			return;
		}
	}

	if (simple_pa_gpio < 0) {
		pr_err("%s invalid gpio %d", __func__, simple_pa_gpio);
		return;
	}

	if (gpio_request(simple_pa_gpio, "simple_pa_enale") != 0) {
		pr_err("%s request gpio %d fail", __func__, simple_pa_gpio);
		return;
	}

	simple_pa_vendor_init(pdev, simple_pa_gpio);

	gpio_direction_output(simple_pa_gpio, GPIO_DIR_OUT);
	gpio_set_value(simple_pa_gpio, GPIO_OUT_ZERO);
	g_simple_pa_gpio = simple_pa_gpio;
}
