...
enum IO_TRACE_ACTION
{
    ...
    BLOCK_BIO_DRAIN_START_TAG,
    BLOCK_BIO_DRAIN_END_TAG,
#endif
    /*lint -restore*/
    IO_TRACE_TAG_END,
};
...
#ifdef CONFIG_IOCACHE
    {BLOCK_BIO_DRAIN_START_TAG, "iocache_drain_s"},
    {BLOCK_BIO_DRAIN_END_TAG, "iocache_drain_e"},
#endif
};
/*lint -restore*/
...
static struct attribute *io_trace_attrs[] = {
    ...
    //&g_kobjAttrPid.attr,//this attr is just for debug (&g_kobjAttrPid.attr,)
#ifdef CONFIG_SCSI_UFS_HI1861_VCMD
    &g_kobjAttrFsr.attr,
#endif
...
static void scsi_dispatch_cmd_done(void *ignore, struct scsi_cmnd *cmd)
{
    ...
    if ((host->hostt == NULL) || (host->hostt->name == NULL))
        return;

    scsi_name = host->hostt->name;
    if (strcmp(scsi_name, UFSHCD) != 0)
        return;
...
#ifdef CONFIG_IOCACHE
static void iocache_drain_last_trans_enter(void *ignore,
    unsigned long trans_total, unsigned long tot_uppages)
{
    if(io_trace_this->enable)
        io_trace_global_log(BLOCK_BIO_DRAIN_START_TAG,
            0, 0, 0, 0, 0, IO_TRACE_WRITE, NULL, 0, NULL);

    return;
}

static void iocache_drain_last_trans_end(void *ignore,
    unsigned int submitted)
{
    if(io_trace_this->enable)
        io_trace_global_log(BLOCK_BIO_DRAIN_END_TAG,
            0, 0, 0, 0, 0, IO_TRACE_WRITE, NULL, 0, NULL);
...
static void iotrace_register_tracepoints(void)
{
    int ret;
#ifdef CONFIG_IOCACHE
    ret = register_trace_iocache_drain_last_trans_enter(
        iocache_drain_last_trans_enter, NULL);
    WARN_ON(ret);
    ret = register_trace_iocache_drain_last_trans_end(
        iocache_drain_last_trans_end, NULL);
    WARN_ON(ret);
#endif
    /*vfs*/
