...
{
	rt9426_reg_write_word(chip->i2c, RT9426_REG_CNTL, 0x0041);
}
...
static unsigned int rt9426_get_avg_current(struct rt9426_chip *chip)
{
	...
	chip->avg_curr = rt9426_reg_read_word(chip->i2c,
		 RT9426_REG_AI);
	if (chip->avg_curr < 0)
		return -EIO;
...
{
	chip->rm = rt9426_reg_read_word(chip->i2c,
		RT9426_REG_RM);
...
static void set_low_volt_smooth(struct rt9426_chip *chip)
{
	...
	if (regval == RT9426_UNSEAL_FAIL) {
		dev_info(chip->dev, "%s: Unseal failed. Unable to do lv smooth!\n" ,__func__);
	} else {
...
{
	int vbat = 0, temp = 0, soc = 0, curr = 0;

	vbat = rt9426_reg_read_word(chip->i2c, RT9426_REG_VBAT);
	temp = rt9426_get_temp(chip);
	soc = rt9426_fg_get_soc(chip);
	curr = rt9426_get_current(chip);
...
{
	struct rt9426_chip *di = (struct rt9426_chip *)dev_data;
...
{
	struct rt9426_chip *di = (struct rt9426_chip *)dev_data;
	struct rt9426_fg_display_data g_dis_data;
...
static irqreturn_t rt9426_irq_handler(int irqno, void *param)
{
	...
	if (irq_val & 0x02)
		coul_interface_event_notify(COUL_EVENT_LOW_VOL, NULL);
	...
	const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	long int param[RT9426_MAX_PARAMS];
...
/* Add for calibration */
static int rt9426_get_calibration_para(struct rt9426_chip *chip, u8 *curr_offs, u8 *curr_gain, u8 *volt_gain)
{
	return 0;
}
...
static void rt9426_apply_calibration_para(struct rt9426_chip *chip, u8 curr_offs, u8 curr_gain, u8 volt_gain)
{
	if (rt9426_unseal_wi_retry(chip) == RT9426_UNSEAL_PASS){
...
static int rt9426_request_charging_inhibit(bool needInhibit)   //Disable power path
{
	if(needInhibit){
	...
	const char *buf, size_t count)
{
	...
	case UNSEAL:
		...
		if ((ret & 0x0001) == 0) {
			/* Unseal RT9426 */
			rt9426_reg_write_word(chip->i2c, RT9426_REG_CNTL, (RT9426_Unseal_Key & 0xffff)); /* get low 2 bytes*/
			rt9426_reg_write_word(chip->i2c, RT9426_REG_CNTL, (RT9426_Unseal_Key >> 16)); /* get high 2 bytes*/
			rt9426_reg_write_word(chip->i2c, RT9426_REG_DUMMY, 0x0000); /* reset */
	...
	case CALIB_RSENSE:
		if (!chip->calib_unlock)
			return -EACCES;
		/*
		if (kstrtoint(buf, 0, &x) || x < 0 || x > 3)
			return -EINVAL;
		rt9426_apply_sense_resistor(chip, x);
		*/
...
static int rt9426_apply_pdata(struct rt9426_chip *chip)
{
	...
	u8 target_curr_offs = 0, target_curr_gain = 0, target_volt_gain = 0;
	int volt_now = 0, fd_vth_now = 0, fd_threshold = 0;
	int retry_times_all = 0, fc_target1 = 0, fc_target2 = 0, fd_target1 = 0;
	int dc;
	...
	retry_times = 30; /* sometimes need retry */
	for (i = 0 ; i < retry_times ; i++) {
		regval = rt9426_reg_read_word(chip->i2c, RT9426_REG_FLAG2);
		if (regval < 0)
			return -1;
		if (regval & RT9426_RDY_MASK) {
			...
			if (!(regval & RT9426_RI_MASK)) {
				...
				if ((regval & 0x7FFF) == RT9426_DRIVER_VER) { /* mask high bits */
					if (dc == chip->pdata->design_capacity) {
						dev_info(chip->dev, "RI=0, bypass initial phase\n");
						goto out_apply;
					} else {
						dev_info(chip->dev, "designed fcc not matched\n");
						break;
					}
				} else {
...
INIT_START:
	...
	/* set OCV Table */
	if (chip->pdata->ocv_table[0].data[0] == 0x13) { /* ocv table head */
		retry_times = 3; /* sometimes need retry */
		dev_notice(chip->dev, "Write NEW OCV Table\n");
		while (retry_times) {
			for (i = 0; i < (RT9426_OCV_ROW_SIZE - 1); i++) {
				rt9426_reg_write_word(chip->i2c, RT9426_REG_EXTREGCNTL, 0xCB00 + i); /* ocv reg: 0xCB00 + i */
				rt9426_reg_write_word(chip->i2c, RT9426_REG_EXTREGCNTL, 0xCB00 + i); /* ocv reg: 0xCB00 + i */
				for (j = 0; j < RT9426_OCV_COL_SIZE; j++) {
					rt9426_reg_write_word(chip->i2c, 0x40 + j * 2,
						chip->pdata->ocv_table[i].data[j]);
					usleep_range(1000, 1100); /* delay 1ms */
				}
			}
			rt9426_reg_write_word(chip->i2c, RT9426_REG_EXTREGCNTL, 0xCB09); /* ocv reg: 0xCB00 + 9 */
			rt9426_reg_write_word(chip->i2c, RT9426_REG_EXTREGCNTL, 0xCB09); /* ocv reg: 0xCB00 + 9 */
			for (i = 0; i < 5; i++) { /* write ocv data */
				rt9426_reg_write_word(chip->i2c, 0x40 + i * 2, /* ocv offset reg*/
	...
	/* set fc_vth + fc_ith */
	fc_target1 = (chip->pdata->fc_vth) | (chip->pdata->fc_ith << 8);
	rt9426_write_page_cmd(chip, RT9426_PAGE_5);
	...
	if (ret == fc_target1) {
		dev_info(chip->dev, "%s: fc_vth & fc_ith matched.\n", __func__);
	} else {
	...
	if (ret == fc_target2) {
		dev_info(chip->dev, "%s: fc_sth matched.\n", __func__);
	} else {
	...
	ret = rt9426_reg_read_word(chip->i2c, RT9426_REG_SWINDOW6);
	if (ret == fd_target1) {
		dev_info(chip->dev, "%s: fd_vth matched.\n", __func__);
	} else {
	...
	if (chip->ocv_checksum_ic != chip->ocv_checksum_dtsi) {
		if(rt9426_get_current(chip)>0){
			...
			dev_info(chip->dev, "RT9426 VBAT = %d\n",volt_now);
			dev_info(chip->dev, "RT9426 FD_VTH = %d\n",fd_vth_now);
			dev_info(chip->dev, "RT9426 FD Threshold = %d\n",fd_threshold);
...
static int rt_parse_dt(struct device *dev, struct rt9426_platform_data *pdata)
{
	...
	int sizes[RT9426_SOC_OFFSET_SIZE + 1] = {0}, j, ret, i;
	...
	ret = of_property_read_u32_array(np, "rt,dtsi_version", pdata->dtsi_version, RT9426_DTSI_VER_SIZE);
	if (ret < 0)
	...
	sizes[0] = sizes[1] = 0;
	ret = of_property_read_u32_array(np, "rt,soc_offset_size", sizes, RT9426_SOC_OFFSET_SIZE);
	if (ret < 0)
	...
	if (pdata->soc_offset.soc_offset_data) {
		...
		if (!offset_params) {
			hwlog_err("devm_kzalloc fail\n");
			return -1;
		}
		of_property_read_u32_array(np, "rt,soc_offset_data",
			(u32 *)offset_params, sizes[0] * sizes[1] * (RT9426_SOC_OFFSET_SIZE + 1));
	...
	if (pdata->extreg_update.extreg_update_data) {
		...
		for (j = 0; j < pdata->extreg_size;  ++j) {
			pdata->extreg_update.extreg_update_data[j].extreg_page = extreg_params[j].edata[0];
			pdata->extreg_update.extreg_update_data[j].extreg_addr = extreg_params[j].edata[1];
			pdata->extreg_update.extreg_update_data[j].extreg_data = extreg_params[j].edata[2];
		}
	...
	if (ret < 0) {
		...
		pdata->otc_tth = RT9426_OTC_TTH_DEFAULT_VAL;
	}
	...
	if (ret < 0) {
		...
		pdata->otd_dchg_ith = RT9426_OTD_DCHG_ITH_DEFAULT_VAL;
	}
	...
	if (ret < 0) {
		...
		pdata->uv_ov_threshold = RT9426_UV_OV_DEFAULT_VAL;
	}

	ret = of_property_read_u32(np, "rt,fg_us_threshold",
		&pdata->us_threshold);
	if (ret < 0) {
		dev_notice(dev, "no us th property, use default 0\n");
		pdata->us_threshold = 0;
	}
	...
	if (ret < 0) {
		...
		pdata->design_capacity = RT9426_DESIG_CAP_VAL;
	}

	ret = of_property_read_u32(np, "rt,fcc", &pdata->fcc);
	if (ret < 0) {
		dev_notice(dev, "no FCC property, use defaut 2000\n");
		pdata->fcc = RT9426_DESIG_FCC_VAL;
	}
	...
	if (ret < 0) {
		...
		for (j = 0; j < RT9426_OCV_ROW_SIZE; j++)
			for (i = 0; i < RT9426_OCV_COL_SIZE; i++)
...
static int rt9266_irq_register(struct rt9426_chip *chip)
{
	int ret;
...
static int rt9426_is_ready(void)
{
	if (!g_rt9426_chip)
		return 0;
...
static int rt9426_read_battery_temperature(void)
{
	...
	temp = rt9426_get_display_data(g_rt9426_chip,
		RT9426_DISPLAY_TEMP);
	bat_curr = rt9426_get_display_data(g_rt9426_chip,
		RT9426_DISPLAY_IBAT);

	return set_ntc_compensation_temp(temp, true, bat_curr);
}

static int rt9426_is_battery_exist(void)
{
	int temp;
	...
	temp = rt9426_read_battery_temperature();
	if ((temp <= RT9426_TEMP_ABR_LOW) ||
		(temp >= RT9426_TEMP_ABR_HIGH))
		return 0;

	return 1;
}

static int rt9426_read_battery_soc(void)
{
	if (!g_rt9426_chip)
		return 0;

	return rt9426_get_display_data(g_rt9426_chip,
		RT9426_DISPLAY_SOC);
}

static int rt9426_read_battery_vol(void)
{
	if (!g_rt9426_chip)
		return 0;
...
static int rt9426_read_battery_current(void)
{
	if (!g_rt9426_chip)
		return 0;
...
static int rt9426_read_battery_avg_current(void)
{
	if (!g_rt9426_chip)
		return 0;
...
static int rt9426_read_battery_fcc(void)
{
	if (!g_rt9426_chip)
		return 0;
...
static int rt9426_read_battery_cycle(void)
{
	if (!g_rt9426_chip)
		return 0;

	return rt9426_get_cyccnt(g_rt9426_chip);
}

static int rt9426_read_battery_rm(void)
{
	if (!g_rt9426_chip)
		return 0;

	return rt9426_get_display_data(g_rt9426_chip, RT9426_DISPLAY_RM);
}

static int rt9426_set_battery_low_voltage(int val)
{
	int uv_set;
	int reg_val;
	u16 wr_val;
	...
	rt9426_read_page_cmd(g_rt9426_chip, RT9426_PAGE_3);
	reg_val = rt9426_reg_read_word(g_rt9426_chip->i2c, RT9426_REG_SWINDOW5);
	if (reg_val < 0)
		return -1;
	wr_val = ((unsigned int)uv_set << RT9426_BYTE_BITS) & RT9426_HIGH_BYTE_MASK;
	wr_val |= (unsigned int)reg_val & RT9426_LOW_BYTE_MASK;

	dev_notice(g_rt9426_chip->dev, "uv_set=0x%x, reg_val=0x%x, wr_val=0x%x\n",
		uv_set, reg_val, wr_val);
	rt9426_write_page_cmd(g_rt9426_chip, RT9426_PAGE_3);
	return rt9426_reg_write_word(g_rt9426_chip->i2c,
		RT9426_REG_SWINDOW5, wr_val);
}

static int rt9426_set_last_capacity(int capacity)
{
	if ((capacity > RT9426_FULL_CAPCACITY) || (capacity < 0)|| !g_rt9426_chip)
		return 0;

	return rt9426_reg_write_word(g_rt9426_chip->i2c,
		RT9426_EXTEND_REG, capacity);
}

static int rt9426_get_last_capacity(void)
{
	int last_cap;
	int cap;
	...
	last_cap = rt9426_reg_read_word(g_rt9426_chip->i2c, RT9426_EXTEND_REG);
	cap = rt9426_read_battery_soc();

	dev_notice(g_rt9426_chip->dev, "%s read cap=%d, last_cap=%d\n",
	...
	/* reset last capacity */
	rt9426_reg_write_word(g_rt9426_chip->i2c, RT9426_EXTEND_REG, 0);
...
struct rt9426_calibration_para{
	int cur_gain;
	int cur_offset;
	int vol_gain;
};

static int rt9426_get_calibration_curr(int *val)
{
	if (!val || !g_rt9426_chip)
		return -1;

	rt9426_enter_calibration_mode(g_rt9426_chip);
	*val = rt9426_get_curr_by_conversion(g_rt9426_chip);
	rt9426_exit_calibration_mode(g_rt9426_chip);
	dev_notice(g_rt9426_chip->dev, "cali cur %d\n", *val);
	return 0;
}

static int rt9426_get_calibration_vol(int *val)
{
	if (!val || !g_rt9426_chip)
		return -1;

	rt9426_enter_calibration_mode(g_rt9426_chip);
	*val = rt9426_get_volt_by_conversion(g_rt9426_chip);
	rt9426_exit_calibration_mode(g_rt9426_chip);
	dev_notice(g_rt9426_chip->dev, "cali vol %d\n", *val);
	return 0;
}

static int rt9426_set_current_gain(unsigned int val)
{
	if ( !g_rt9426_chip)
		return -1;
	val = 0x80 + (((s64)(val) * 512) / 1000000 - 512);
	g_rt9426_chip->c_gain = val;
	rt9426_apply_calibration_para(g_rt9426_chip, g_rt9426_chip->c_offset,
		g_rt9426_chip->c_gain, g_rt9426_chip->v_gain);
	dev_notice(g_rt9426_chip->dev, "cur gain %d\n", val);
	return 0;
}

static int rt9426_set_voltage_gain(unsigned int val)
{
	if ( !g_rt9426_chip)
		return -1;
	val = 0x80 + (((s64)(val) * 512) / 1000000 - 512);
	g_rt9426_chip->v_gain = val;
	rt9426_apply_calibration_para(g_rt9426_chip, g_rt9426_chip->c_offset,
		g_rt9426_chip->c_gain, g_rt9426_chip->v_gain);
	dev_notice(g_rt9426_chip->dev, "voltage gain %d\n", val);
	return 0;
}

static struct power_bat_cali_ops rt9426_cali_ops = {
	.dev_name = "rt9426_ti",
...
#define RT9426_CALI_MIN_A 800000
#define RT9426_CALI_MAX_A 1300000

static int rt9426_calibration_para_invalid(struct power_cali_para* p)
{
	return ((p->para[POWER_CALI_PARA_CUR_A] <RT9426_CALI_MIN_A) ||
		(p->para[POWER_CALI_PARA_CUR_A] > RT9426_CALI_MAX_A) ||
		(p->para[POWER_CALI_PARA_VOL_A] <RT9426_CALI_MIN_A) ||
		(p->para[POWER_CALI_PARA_VOL_A] > RT9426_CALI_MAX_A));
}
static void rt9426_init_calibration_para(struct rt9426_chip *chip)
{
	struct power_cali_para* l_para = NULL;
	int val;
	...
	l_para = power_cali_get_para(POWER_CALI_TYPE_OTHERF_FG);
	if (!l_para)
		goto update;
	if (rt9426_calibration_para_invalid(l_para)) {
		l_para = power_cali_get_para(POWER_CALI_TYPE_LOCAL_FG);
		if (!l_para)
			goto update;
		if (rt9426_calibration_para_invalid(l_para))
			goto update;
	...
	val = l_para->para[POWER_CALI_PARA_CUR_A];
	chip->c_gain = 0x80 + (((s64)(val) * 512) / 1000000 - 512);

	val = l_para->para[POWER_CALI_PARA_VOL_A];
	chip->v_gain = 0x80 + (((s64)(val) * 512) / 1000000 - 512);

	dev_notice(chip->dev, "c_gain %d, v_gain %d\n",
		chip->c_gain, chip->v_gain);
update:

	rt9426_apply_calibration_para(chip, 0x80, chip->c_gain, chip->v_gain);
}
