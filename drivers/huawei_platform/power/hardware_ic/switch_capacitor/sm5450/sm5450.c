...
static int sm5450_scp_adapter_reg_read_block(u8 reg, u8 *val, u8 num)
{
	...
	if (num) {
		p += data_len;
		reg += data_len;
		ret = sm5450_scp_adapter_reg_read_block(reg, p, num);
		if (ret) {
			/* Manual Init */
			sm5450_write_mask(di, SM5450_SCP_CTL_REG, SM5450_SCP_CTL_SNDCMD_MASK,
				SM5450_SCP_CTL_SNDCMD_SHIFT, SM5450_SCP_CTL_SNDCMD_RESET);
...
static int sm5450_fcp_master_reset(void)
{
	struct sm5450_device_info *di = g_sm5450_scp_dev;
	int ret;
...
static int sm5450_fcp_adapter_reset(void)
{
	int ret;
	struct sm5450_device_info *di = g_sm5450_scp_dev;
...
static int sm5450_fcp_read_switch_status(void)
{
	return 0;
}

static int sm5450_is_fcp_charger_type(void)
{
	u8 reg_val = 0;
	int ret;
	struct sm5450_device_info *di = g_sm5450_scp_dev;
...
static int sm5450_fcp_stop_charge_config(void)
{
	struct sm5450_device_info *di = g_sm5450_scp_dev;
...
static int sm5450_scp_chip_reset(void)
{
	int ret;

	ret = sm5450_fcp_master_reset();
	if (ret) {
		hwlog_err("sm5450_fcp_master_reset fail\n");
		return -1;
	}
...
static int sm5450_scp_detect_adapter(void)
{
	struct sm5450_device_info *di = g_sm5450_scp_dev;
...
static int sm5450_fcp_detect_adapter(void)
{
	struct sm5450_device_info *di = g_sm5450_scp_dev;
...
static int sm5450_pre_init(void)
{
	int ret;
	struct sm5450_device_info *di = g_sm5450_scp_dev;
...
static int sm5450_scp_adapter_reset(void)
{
	return sm5450_fcp_adapter_reset();
}
...
static int sm5450_dump_reg_value(struct charger_device *dev,
	char *reg_value, int size, void *dev_data)
{
	u8 reg_val = 0;
	int i;
	int ret = 0;
...
static int sm5450_reg_head(struct charger_device *dev, char *reg_head,
	int size, void *dev_data)
{
	int i;
	int tmp;
	int len = 0;
...
int sm5450_register_head(char *buffer, int size, void *dev_data)
{
	...
	return sm5450_reg_head(NULL, buffer, size, di);
}
...
int sm5450_dump_reg(char *buffer, int size, void *dev_data)
{
	...
	return sm5450_dump_reg_value(NULL, buffer, size, di);
}

static struct direct_charge_ic_ops sm5450_lvc_ops = {
	.dev_name = "sm5450",
...
static struct direct_charge_ic_ops sm5450_sc_ops = {
	.dev_name = "sm5450",
	.ic_init = sm5450_charge_init,
	.ic_exit = sm5450_charge_exit,
...
static struct direct_charge_batinfo_ops sm5450_batinfo_ops = {
...
static struct direct_charge_ic_ops sm5450_aux_lvc_ops = {
	.dev_name = "sm5450_aux",
	.ic_init = sm5450_charge_init,
	.ic_exit = sm5450_charge_exit,
	.ic_enable = sm5450_lvc_charge_enable,
...
static struct direct_charge_ic_ops sm5450_aux_sc_ops = {
	.dev_name = "sm5450_aux",
	.ic_init = sm5450_charge_init,
	.ic_exit = sm5450_charge_exit,
	.ic_enable = sm5450_sc_charge_enable,
...
static struct direct_charge_batinfo_ops sm5450_aux_batinfo_ops = {
	.init = sm5450_batinfo_init,
	.exit = sm5450_batinfo_exit,
	.get_bat_btb_voltage = sm5450_get_vbat_mv,
	.get_bat_package_voltage = sm5450_get_vbat_mv,
...
static int sm5450_ops_register(struct sm5450_device_info *di)
{
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		...
		ret = dc_lvc_ops_register(CHARGE_IC_TYPE_MAIN, &sm5450_lvc_ops);
		ret += dc_sc_ops_register(CHARGE_IC_TYPE_MAIN, &sm5450_sc_ops);
		...
		mutex_lock(&g_sm5450_log_lock);
		ret = dc_lvc_ops_register(CHARGE_IC_TYPE_AUX, &sm5450_aux_lvc_ops);
		ret += dc_sc_ops_register(CHARGE_IC_TYPE_AUX, &sm5450_aux_sc_ops);
