...
void hi6526_dc_ucp_delay_work(struct work_struct *work)
{
	...
	if (ibus_abnormal_cnt >= IBUS_ABNORMAL_CNT) {
		...
		(void)hi6526_lvc_enable(0, NULL);
		(void)hi6526_sc_enable(0, NULL);
		return;
	}
...
int hi6526_switch_to_buck_mode(void *dev_data)
{
	...
	ret = hi6526_set_charge_enable(dev, CHG_DISABLE);
	if (ret != 0)
	...
	ret = hi6526_set_term_enable(dev, CHG_TERM_DIS);
	if (ret != 0)
		scharger_err("%s hi6526_set_term_enable err!\n", __func__);
	ret = hi6526_set_input_current(dev, CHG_ILIMIT_475);
	if (ret != 0)
		scharger_err("%s hi6526_set_input_current err!\n", __func__);
	ret = hi6526_set_charge_current(dev, CHG_FAST_ICHG_500MA);
	if (ret != 0)
		scharger_err("%s hi6526_set_input_current err!\n", __func__);
	ret = hi6526_set_terminal_voltage(dev, CHG_FAST_VCHG_4400);
	if (ret != 0)
		scharger_err("%s hi6526_set_input_current err!\n", __func__);
	(void)hi6526_set_terminal_current(dev, CHG_TERM_ICHG_150MA);
	(void)hi6526_set_watchdog_timer(dev, WATCHDOG_TIMER_40_S);
#endif
	return 0;
}
...
int hi6526_batinfo_get_ibus_ma(int *vbus_mv, void *dev_data)
{
	...
	hi6526_get_ibus_ma(dev, (u32 *)vbus_mv);
	return 0;
}
