...
#include <huawei_platform/power/wireless/wireless_power_supply.h>
#include <huawei_platform/power/wireless/wireless_dc_error_handle.h>
...
static int wldc_get_bat_current(void)
{
	...
	ret = dc_get_bat_current(SC_MODE, di->ic_data.cur_type, &bat_curr);
	if (ret) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		wldc_fill_eh_buf(di->wldc_err_dsm_buff,
	...
	dc_get_bat_current_with_comp(&bat_curr);
...
static void wldc_select_comp_param(struct wldc_dev_info *di)
{
	...
	if (di->ic_mode_para.support_multi_ic) {
		main_ic_id = dc_get_ic_id(SC_MODE, CHARGE_IC_AUX);
		aux_ic_id = dc_get_ic_id(SC_MODE, CHARGE_IC_AUX);
...
static bool can_vbatt_do_wldc_charge(struct wldc_dev_info *di, int dc_mode)
{
	int vbatt_min;
	int vbatt_max;
	int vbatt = battery_get_bat_voltage();
...
{
	return charge_enable_hz(false);
}

int wldc_cut_off_normal_charge_path(void)
{
	return charge_enable_hz(true);
}
...
open_fail:
	...
	if (soc >= 95) { /* maybe normal, when capacity>=95 */
...
static bool wldc_is_volt_param_match(struct wldc_dev_info *di, int id)
{
	...
	batt_brand = huawei_get_battery_type();
	huawei_battery_temp(BAT_TEMP_MIXED, &tbatt);
...
static void wldc_notify_start_charging_evt(struct wldc_dev_info *di)
{
	...
	dc_type = (u8)di->mode_para[di->cur_dc_mode].init_para.dc_type;
...
set_vout:
	if ((data.rx_vmax > 0) && (di->rx_vout_set > data.rx_vmax))
		di->rx_vout_set = data.rx_vmax;
...
static int wldc_multi_ic_check(struct wldc_dev_info *di)
{
	...
	rx_ratio = di->mode_para[di->cur_dc_mode].init_para.rx_ratio;
	if (mulit_ic_check(SC_MODE, di->ic_data.cur_type, &di->ic_check_info) < 0) {
