	...
	struct drm_fb_helper *helper)
{
	...
	const struct drm_connector_helper_funcs *helper_private = NULL;
	int ret = 0;
...
#ifdef CONFIG_LCD_KIT_DRIVER
static int lcdkit_register_sysfs(struct fb_info *fb_info)
{
	...
	if (fb_info->dev == NULL) {
		printk(KERN_ERR "fb_info dev is NULL\n");
		return ret;
	}
	...
	if (lcd_ops->lcd_kit_support)
		if (lcd_ops->lcd_kit_support())
			ret = lcd_ops->create_sysfs(&fb_info->dev->kobj);
...
#ifdef CONFIG_LCD_KIT_DRIVER
	ret = lcdkit_register_sysfs(helper->fbdev);
	if (ret < 0)
		printk(KERN_ERR "lcd kit create sysfs fail\n");
#endif
	DDPMSG("%s-\n", __func__);
