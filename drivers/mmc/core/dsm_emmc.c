...
#define MMC_RSP_R1_ERROR_MASK (R1_OUT_OF_RANGE | R1_ADDRESS_ERROR | \
		...
		R1_CARD_ECC_FAILED | R1_CC_ERROR | \
		R1_ERROR | R1_CID_CSD_OVERWRITE | \
		R1_WP_ERASE_SKIP | R1_ERASE_RESET | R1_SWITCH_ERROR)

#define MMC_RSP_R1_ERROR_NON_CRC_MASK (MMC_RSP_R1_ERROR_MASK & \
		(~(R1_COM_CRC_ERROR | R1_ILLEGAL_COMMAND)))
	...
	if (dsm_emmc_process_log(code, err_msg)) {
		...
		if (NULL != card_dev) {
			if (sizeof(struct mmc_cid) < buff_size) {
				ret = snprintf(dsm_log_buff, buff_size, "0x%04x,", card_dev->cid.manfid);
				if (ret < 0) {
					pr_err("%s:%d ++\n", __func__, __LINE__);
					ret = 0;
				}
				...
				ret = snprintf(dsm_log_buff, buff_size, "%s,", card_dev->cid.prod_name);
				if (ret < 0) {
					pr_err("%s:%d ++\n", __func__, __LINE__);
					ret = 0;
				}
				...
				ret = snprintf(dsm_log_buff, buff_size, /* unsafe_function_ignore: snprintf */
					"%02x%02x%02x%02x%02x%02x%02x%02x,",
					card_dev->ext_csd.fwrev[0],
					card_dev->ext_csd.fwrev[1],
					card_dev->ext_csd.fwrev[2],
					card_dev->ext_csd.fwrev[3],
					card_dev->ext_csd.fwrev[4],
					card_dev->ext_csd.fwrev[5],
					card_dev->ext_csd.fwrev[6],
					card_dev->ext_csd.fwrev[7]);
				...
				ret = snprintf(dsm_log_buff, buff_size, "%02x,", (card_dev->raw_cid[3]>>8)&0xff);
				if (ret < 0) {
					pr_err("%s:%d ++\n", __func__, __LINE__);
					ret = 0;
				}
				...
				for (j = 0; j < strlen(g_emmc_dsm_log.emmc_dsm_log); j++) {
					g_emmc_dsm_log.emmc_dsm_log[j] -= 1;
				}
		...
		if (card->ext_csd.rev >= 7) {
			...
			if (card->ext_csd.device_life_time_est_typ_a >= DEVICE_LIFE_TRIGGER_LEVEL ||
				card->ext_csd.device_life_time_est_typ_b >= DEVICE_LIFE_TRIGGER_LEVEL) {
					pr_err("%s:eMMC life time has problem,"
					"device_life_time_est_typ_a[268]:%d,"
					"device_life_time_est_typ_b{269]:%d\n",
					__func__, card->ext_csd.device_life_time_est_typ_a,
...
inline int mmc_dsm_request_response_error_filter(struct mmc_request *mrq)
{
	return (((mrq)->cmd->opcode == MMC_SEND_TUNING_BLOCK_HS200) ||
			(((mrq)->cmd->opcode == MMC_SWITCH) &&
			((mrq)->cmd->resp[0] & (R1_COM_CRC_ERROR |
			R1_ILLEGAL_COMMAND)) && (((mrq)->cmd->resp[0] &
			MMC_RSP_R1_ERROR_NON_CRC_MASK) == 0)));
}

void mmc_dsm_request_response_error_check(struct mmc_host *host,
			struct mmc_request *mrq)
{
	...
	if (mrq->cmd && ((mmc_resp_type(mrq->cmd) == MMC_RSP_R1) ||
		...
		if (mrq->cmd->resp[0] & (MMC_RSP_R1_ERROR_MASK |
			R1_EXCEPTION_EVENT)) {
			if (host->index == 0) {
				if ((mrq->cmd->resp[0] &
				MMC_RSP_R1_ERROR_MASK) &&
				!mmc_dsm_request_response_error_filter(mrq)) {
					struct mmc_card *card = NULL;
		...
		if (mrq->done == mmc_wait_data_done) {
			...
			if (ext_csd[EXT_CSD_EXP_EVENTS_STATUS] &
				...
				DSM_EMMC_LOG(
					...
					"DYNCAP_NEEDED [58]: %d, the device may"
					"degrade in performance and eventually"
...
EXPORT_SYMBOL(mmc_dsm_request_response_error_check);
