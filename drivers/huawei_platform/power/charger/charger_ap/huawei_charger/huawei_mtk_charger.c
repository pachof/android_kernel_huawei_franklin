...
static bool check_batt_not_exist(void)
{
...
static bool check_batt_volt_overhigh(void)
{
	return check_batt_volt(true);
}

static bool check_batt_volt_overlow(void)
{
	return check_batt_volt(false);
}

static bool check_batt_not_terminate(void)
{
	int vbat;
...
static bool check_charge_curr_overhigh(void)
{
...
static bool check_discharge_curr_overhigh(void)
{
	static int curr_over_cnt;
...
static bool check_vbus_volt_overhigh(void)
{
	enum charger_type chg_type;
	int vbus_volt;
...
static bool check_batt_high_temp(void)
{
	return check_batt_temp(true);
}

static bool check_batt_low_temp(void)
{
	return check_batt_temp(false);
}

static bool check_batt_temp_out_range(void)
{
	int batt_temp = battery_get_bat_temperature();
...
static bool check_batt_high_temp_warm(void)
{
...
void dsm_report_once(enum power_dsm_type type, int err_no, char *buf)
{
	int ret;
...
void bat_check_error_func(void)
{
	...
	for (i = 0; i < array_len; ++i) {
		...
		if ((g_batt_dsm_array[i].notify_enable == false) ||
			(g_batt_dsm_array[i].check_error() == false))
			continue;
		if (g_batt_dsm_array[i].dump)
			g_batt_dsm_array[i].dump(buf, DMS_REPORT_BUF_LEN);
		/* print the final buffer */
		ret = power_dsm_dmd_report(POWER_DSM_BATTERY,
			g_batt_dsm_array[i].error_no, buf);
		if (!ret) {
			g_batt_dsm_array[i].notify_enable = false;
			continue;
		}
	}
}
