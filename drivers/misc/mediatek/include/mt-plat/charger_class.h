...
struct ico_input {
	enum huawei_charger_type charger_type;
...
struct charger_ops {
	/* add from hisi platform's charge_device_ops */
	int (*chip_init)(struct charger_device *dev, struct chip_init_crit *init_crit);
	int (*set_adc_conv_rate)(struct charger_device *dev, int rate_mode);
	int (*dev_check)(struct charger_device *dev);
	void (*set_input_current_thermal)(struct charger_device *dev, int val1, int val2);
	void (*set_charge_current_thermal)(struct charger_device *dev, int val1, int val2);
	/* int (*set_term_enable)(struct charger_device *dev, int enable);enable_termination */
	int (*get_charge_state)(struct charger_device *dev, unsigned int *state);
	/* int (*get_register_head)(struct charger_device *dev, char *reg_head, int size);get_reg_head */
	int (*set_watchdog_timer)(struct charger_device *dev, int value);
	int (*set_batfet_disable)(struct charger_device *dev, int disable);
	int (*check_charger_plugged)(struct charger_device *dev);
	int (*check_input_dpm_state)(struct charger_device *dev);
	int (*check_input_vdpm_state)(struct charger_device *dev);
	int (*check_input_idpm_state)(struct charger_device *dev);
	int (*set_covn_start)(struct charger_device *dev, int enable);
	int (*stop_charge_config)(struct charger_device *dev);
	int (*set_otg_switch_mode_enable)(struct charger_device *dev, int enable);
	int (*get_vbat_sys)(struct charger_device *dev);
	int (*set_vbus_vset)(struct charger_device *dev, u32 vset);
	int (*set_uvp_ovp)(struct charger_device *dev);
	int (*set_force_term_enable)(struct charger_device *dev, int enable);
	int (*get_charger_state)(struct charger_device *dev);
	int (*soft_vbatt_ovp_protect)(struct charger_device *dev);
	int (*rboost_buck_limit)(struct charger_device *dev);
	int (*get_iin_set)(struct charger_device *dev);
	int (*set_boost_voltage)(struct charger_device *dev, int voltage);
	int (*get_dieid)(struct charger_device *dev, char *dieid, unsigned int len);
	int (*get_vbat)(struct charger_device *dev);
	int (*get_terminal_voltage)(struct charger_device *dev);
	int (*get_vusb)(struct charger_device *dev, int *value);
	int (*set_pretrim_code)(struct charger_device *dev, int val);
	int (*get_dieid_for_nv)(struct charger_device *dev, u8 *dieid, unsigned int len);
	...
	int (*dump_reg)(struct charger_device *dev, char *reg, int size);
	int (*get_reg_head)(struct charger_device *dev, char *head, int size);
	...
	int (*get_tchg_adc)(struct charger_device *dev, int *tchg_min,
		int *tchg_max);
	int (*get_ts_adc)(struct charger_device *dev, u32 *ts_volt_);
	int (*get_usbid_adc)(struct charger_device *chg_dev, u32 *id);
	...
	int (*enable_force_sleep)(struct charger_device *dev, bool en);
...
extern int charger_dev_get_eoc_current(
	struct charger_device *charger_dev, u32 *uA);
extern int charger_dev_enable_eoc(struct charger_device *charger_dev, bool en);
...
extern int charger_dev_enable_hz(
	struct charger_device *charger_dev, bool en);
extern int charger_dev_enable_force_enable(
	struct charger_device *chg_dev, bool en);
extern int charger_dev_set_watchdog_timer(
	struct charger_device *chg_dev, bool en);
...
extern int charger_dev_get_temperature(
	struct charger_device *charger_dev, int *tchg_min,
		int *tchg_max);
extern int charger_dev_get_ts_adc(
	struct charger_device *charger_dev, u32 *ts_volt);
extern int charger_dev_get_usb_id(struct charger_device *chg_dev, u32 *id);
...
int charger_dev_hw_dump_register(struct charger_device *chg_dev, char *value,
	int size);
int charger_dev_get_register_head(struct charger_device *chg_dev, char *head,
	int size);
extern int charger_dev_set_vbus_vset(struct charger_device *chg_dev, u32 uV);
int charger_dev_chip_init(struct charger_device *chg_dev, struct chip_init_crit *init_crit);
int charger_dev_get_chg_state(struct charger_device *chg_dev, u32 *pg_state);
int charger_dev_check_input_dpm_state(struct charger_device *chg_dev);
int charger_dev_check_charger_plugged(struct charger_device *chg_dev);
