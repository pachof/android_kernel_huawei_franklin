...
struct chg_type_info {
	...
	/* power off charge wakelock */
	struct wakeup_source pwroff_chg_wakelock;
};
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	if (mtk_chg->chg_type == FCP_CHARGER) {
		pr_info("%s: fcp type return\n", __func__);
		return 0;
	}
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			/* Workaround! */
			/* not suspend, within two minutes after startup and kpoc */
			if (cti->tcpc_kpoc && get_kpoc_wakelock_flag() &&
				!cti->pwroff_chg_wakelock.active) {
				pr_info("pwroff_chg_wakelock wake\n");
				__pm_wakeup_event(&cti->pwroff_chg_wakelock,
					120000);
			}
...
static int mt_charger_probe(struct platform_device *pdev)
{
	...
	wakeup_source_init(&cti->pwroff_chg_wakelock,
		"power off charge wakelock");
...
static int mt_charger_resume(struct device *dev)
{
	...
	if ((mt_charger != NULL) &&
		((mt_charger->chg_type == STANDARD_HOST) ||
		(mt_charger->chg_type == CHARGING_HOST)))
