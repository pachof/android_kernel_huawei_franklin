...
int msg30xx_dbbus_set_default(void)
{
    ...
    for(i = 0; i < ARRAY_SIZE(tmc); i++) {
        rc = mstar_iic_write_data(SLAVE_I2C_ID_DBBUS, (u8 *)(&tmc[i].data), 1);
        if (rc < 0) {
...
int mp_new_flow_main(int item)
{
    ...
    for(i = 0; i < ARRAY_SIZE(tmc); i++) {
        ret = mstar_iic_write_data(SLAVE_I2C_ID_DBBUS, (u8 *)(&tmc[i].data), 1);
        if(ret < 0) {
...
out:
    ...
    mstar_iic_write_data(SLAVE_I2C_ID_DBBUS, (u8 *)(&tmc[2].data), 1);
...
void mp_send_data2kit(struct ts_rawdata_info *info)
{
    ...
    info->hybrid_buff[hybrid_buff_index++] = drv_num;
    info->hybrid_buff[hybrid_buff_index++] = sense_num;
