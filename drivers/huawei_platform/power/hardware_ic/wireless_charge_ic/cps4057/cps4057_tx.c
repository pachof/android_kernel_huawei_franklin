...
static int cps4057_tx_set_pt_full_bridge(bool flag)
{
	...
	if (flag)
		wr = CPS7181_TX_FUNC_EN;
	else
		wr = CPS7181_TX_FUNC_DIS;
...
static int cps4057_tx_set_fod_coef(u32 pl_th, u8 pl_cnt)
{
	...
	ret = cps4057_write_word(CPS4057_TX_PLOSS_TH_ADDR, (u16)pl_th);
	/* tx ploss fod debounce count 0:no debounce */
	ret += cps4057_write_byte(CPS4057_TX_PLOSS_CNT_ADDR, pl_cnt);
...
static int cps4057_tx_get_ept_type(u16 *ept)
{
	...
	ret = cps4057_read_byte(CPS4057_TX_EPT_SRC_ADDR, &ept_value);
	if (ret) {
