...
void __put_task_struct(struct task_struct *tsk)
{
	...
	raw_spin_lock(&tsk->cpu_cap_req.lock);
	list_del_init(&tsk->cpu_cap_req.req_entry);
	raw_spin_unlock(&tsk->cpu_cap_req.lock);
#endif
	cgroup_free(tsk);
...
#endif
#if (defined(CONFIG_HW_VIP_THREAD) || defined(CONFIG_HW_MTK_RTG_SCHED))
