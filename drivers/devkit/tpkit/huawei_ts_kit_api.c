...
void ts_algo_calibrate(struct ts_cmd_node* in_cmd, struct ts_cmd_node* out_cmd)
{
	...
	TS_LOG_DEBUG("algo order: %u, algo_size: %d\n", order, algo_size);

	for (i = 0; (i < algo_size) && (order & BIT_MASK(i)); i++) {
		TS_LOG_DEBUG("algo index: %u is setted\n", i);
		list_for_each_entry(algo,
			&g_ts_kit_platform_data.chip_data->algo_head, node) {
			/* found the right algo func */
			if (algo->algo_index == i) {
				TS_LOG_DEBUG("algo: %s called\n",
					algo->algo_name);
				algo->chip_algo_func(g_ts_kit_platform_data.chip_data,
...
out:
	memcpy(&out_cmd->cmd_param.pub_params.report_info, in_finger,
		sizeof(out_cmd->cmd_param.pub_params.report_info));
...
void ts_palm_report(struct ts_cmd_node* in_cmd, struct ts_cmd_node* out_cmd){
	...
	TS_LOG_DEBUG("palm_button is %u\n", button);
	input_report_key(input_dev, button, 1);
	input_sync(input_dev);
	input_report_key(input_dev, button, 0);
	input_sync (input_dev);
