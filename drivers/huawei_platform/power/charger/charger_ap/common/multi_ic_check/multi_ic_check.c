	...
	struct multi_ic_check_para *info, const char *buf, int buf_size)
{
	...
	(void)huawei_battery_temp(BAT_TEMP_MIXED, &tbat);
	...
	snprintf(tmp_buf, sizeof(tmp_buf),
		...
		dc_get_bat_btb_voltage(working_mode, CHARGE_IC_MAIN),
		dc_get_bat_btb_voltage(working_mode, CHARGE_IC_AUX),
		tbat, 0, mt_get_charger_type());
...
static void multi_ic_check_vbat(int working_mode, struct multi_ic_check_para *info)
{
	...
	vbat_main = dc_get_bat_btb_voltage(working_mode, CHARGE_IC_MAIN);
	vbat_aux = dc_get_bat_btb_voltage(working_mode, CHARGE_IC_AUX);
...
int mulit_ic_check(int working_mode, int mode, struct multi_ic_check_para *info)
{
	u32 cur_time = current_kernel_time().tv_sec;
	int vbat_main;
	int vbat_aux;
...
static void multi_check_btb_result(struct multi_ic_check_mode_para *para)
{
	u32 volt_result = 0;
	...
	if (volt_result == 0)
		return;
	if (volt_result & MAIN_BAT_BTB_ERR) {
		...
		hwlog_info("main btb vol check fail, can not direct charge\n");
	}
	if (volt_result & AUX_BAT_BTB_ERR) {
		para->ic_error_cnt[CHARGE_IC_TYPE_AUX] = MULTI_IC_CHECK_ERR_CNT_MAX;
		hwlog_info("aux btb vol check fail\n");
	}
}
