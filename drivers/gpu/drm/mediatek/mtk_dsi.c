...
static int mtk_dsi_poweron(struct mtk_dsi *dsi)
{
	...
	mipi_tx_rate = (unsigned long)(data_rate * 1000000);
...
static irqreturn_t mtk_dsi_irq_status(int irq, void *dev_id)
{
	...
	if (status) {
		...
		if (status & (1UL << 7)) {
			if (mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
				DDPPR_ERR("[IRQ] %s: TE timeout\n",
					mtk_dump_comp_str(&dsi->ddp_comp));
				if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
					dsm_client_record(lcd_dclient, "TE timeout\n");
					dsm_client_notify(lcd_dclient,
						DSM_LCD_TE_TIME_OUT_ERROR_NO);
				}
			}
		}
#endif
...
#ifdef CONFIG_LCD_KIT_DRIVER
	struct lcd_kit_common_info *lcd_common_info = NULL;
#endif
...
#ifdef CONFIG_LCD_KIT_DRIVER
	lcd_common_info = lcd_kit_get_common_info();
	if (lcd_common_info)
		lcd_common_info->new_doze_state = new_doze_state;
#endif
	...
	int force_lcm_update)
{
	...
	mtk_crtc->have_frame_update = false;
...
int mtk_dsi_esd_cmp(struct mtk_ddp_comp *comp, void *handle, const void *slot)
{
	int i = 0, ret = 0;
	u32 tmp0 = 0, tmp1 = 0, chk_val = 0;
	struct mtk_dsi *dsi = container_of(comp, struct mtk_dsi, ddp_comp);
	struct esd_check_item *lcm_esd_tb = NULL;
	struct mtk_panel_params *params = NULL;
...
#if defined (CONFIG_HUAWEI_DSM) && defined (CONFIG_LCD_KIT_DRIVER)
			lcd_record.esd_read_value[i] = chk_val & 0xFF;
#endif
			ret = -1;
			break;
		}
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	struct drm_display_mode *adjust_mode = NULL;
...
static void mtk_dsi_cmdq(struct mtk_dsi *dsi, const struct mipi_dsi_msg *msg)
{
	...
	u8 config = 0, cmdq_size = 0, cmdq_off = 0, type = msg->type;
	u32 reg_val = 0, cmdq_mask = 0, i = 0;
	unsigned long goto_addr = 0;
				...
				unsigned int para_size)
{
	...
	const char *tx_buf = NULL;
	u8 config = 0, cmdq_off = 0, type = 0;
	u8 cmdq_size = 0, total_cmdq_size = 0;
	u8 start_off = 0;
	u32 reg_val = 0, cmdq_val = 0;
	u32 cmdq_mask = 0, i = 0, j = 0;
	...
	if (fps_chg_index & DYNFPS_DSI_MIPI_CLK) {
		...
		if (!need_send_cmd) {
			...
			if (cmdq_pkt_flush_threaded(handle,
				...
				if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
					dsm_client_record(lcd_dclient,
						"vfp: failed to change fps\n");
					dsm_client_notify(lcd_dclient,
					DSM_LCD_CHANGE_FPS_ERROR_NO);
				}
			}
#else
			if (cmdq_pkt_flush_threaded(handle,
...
{
	unsigned int compress_rate = 0, bpp = 0, bpc = 0;
	struct mtk_panel_ext *ext = dsi->ext;
