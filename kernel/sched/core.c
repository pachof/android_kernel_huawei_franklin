...
#ifdef CONFIG_HW_MTK_RTG_SCHED
int set_task_min_util_tick(struct related_thread_group *grp,
	struct task_struct *p, unsigned int new_util)
{
	...
	old_util = p->cpu_cap_req.min_util;
	p->cpu_cap_req.min_util = new_util;
	...
	if (task_on_rq_queued(p) && new_util != old_util) {
		...
		if (old_util == 0) {
			if (likely(list_empty(&p->cpu_cap_req.req_entry)))
				list_add(&p->cpu_cap_req.req_entry, &rq->min_util_req);
		}

		if (new_util == 0) {
			if (likely(!list_empty(&p->cpu_cap_req.req_entry)))
				list_del_init(&p->cpu_cap_req.req_entry);
		}
	...
	need_update_freq = group_should_update_freq(grp, cpu, RTG_FREQ_NORMAL_UPDATE);
...
int set_task_min_util(struct task_struct *p, unsigned int new_util)
{
	...
	int cpu;
	bool should_update_freq = false;
	bool need_update_grp_freq = false;
	unsigned long flag;
...
{
	struct cpu_cap_request *req;
	bool need_update_grp_freq;
	unsigned long flag;

	req = &p->cpu_cap_req;
	if (unlikely(req->min_util)) {
		write_lock_irqsave(&rq->util_lock, flag);
		if (unlikely(!list_empty(&req->req_entry))) {
			write_unlock_irqrestore(&rq->util_lock, flag);
			pr_warn("error when add req %d\n", p->pid);
			return;
		}

		list_add(&req->req_entry, &rq->min_util_req);
		write_unlock_irqrestore(&rq->util_lock, flag);

		if (req->min_util > capacity_curr_of(cpu_of(rq)))
			sugov_mark_util_change(cpu_of(rq), RQ_SET_MIN_UTIL);
	}
}
...
static void del_freq_request(struct task_struct *p)
{
	...
	int cur_min_util = get_min_util(rq);
	unsigned long flag;
...
{
	struct cpu_cap_request *req = NULL;
	unsigned int ret = 0;
	int loop_max = 10;
	unsigned long flag;
	...
	list_for_each_entry(req, &rq->min_util_req, req_entry) {
		if (req->min_util > ret)
			ret = req->min_util;
...
#endif
#ifdef CONFIG_HW_MTK_RTG_SCHED
	p->restore_params.trans_type = -1;
	p->rtg_depth = 0;
	raw_spin_lock_init(&p->cpu_cap_req.lock);
	p->cpu_cap_req.min_util = 0;
	INIT_LIST_HEAD(&p->cpu_cap_req.req_entry);
#endif
}
...
#ifdef CONFIG_VIP_SCHED_OPT
	clear_tsk_thread_flag(prev, TIF_WAKE_SYNC);
#endif
...
#endif
#ifdef CONFIG_VIP_SCHED_OPT
		rq->vip_preempt_type = vip_preempt_classify(rq, prev, next);
#endif
		rq->nr_switches++;
		rq->curr = next;
		/*
