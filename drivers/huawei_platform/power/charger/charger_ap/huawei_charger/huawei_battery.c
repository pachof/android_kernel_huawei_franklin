...
struct huawei_battery_info *g_info = NULL;
...
static int hw_get_reg_head(struct charger_consumer *consumer, int idx,
	char *reg_head)
{
	struct charger_device *pdev = NULL;
	...
	charger_dev_get_register_head(pdev, reg_head, DUMP_REG_LEN);
	return 0;
}
...
static int hw_dump_reg(struct charger_consumer *consumer, int idx,
	char *reg)
{
	struct charger_device *pdev = NULL;
	...
	charger_dev_hw_dump_register(pdev, reg, DUMP_REG_LEN);
	return 0;
}
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_REGISTER_HEAD:
		rc = hw_get_reg_head(g_pconsumer, MAIN_CHARGER,
			(char *)val->strval);
		break;
	case POWER_SUPPLY_PROP_DUMP_REGISTER:
		rc = hw_dump_reg(g_pconsumer, MAIN_CHARGER,
			(char *)val->strval);
		break;
	default:
		return -EINVAL;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		rc = get_prop_from_psy(info->batt_psy,
...
int huawei_battery_capacity(void)
{
	...
	if (!g_info || !g_info->batt_psy)
		return cap;
	rc = get_prop_from_psy(g_info->batt_psy,
		POWER_SUPPLY_PROP_CAPACITY, &val);
	if (rc < 0)
		return cap;
	cap = val.intval;
...
int huawei_get_ac_online(void)
{
	...
	if (!g_info || !g_info->ac_psy)
		return online;
	rc = get_prop_from_psy(g_info->ac_psy,
			POWER_SUPPLY_PROP_ONLINE, &val);
	if (rc < 0)
		return online;
	online = val.intval;
