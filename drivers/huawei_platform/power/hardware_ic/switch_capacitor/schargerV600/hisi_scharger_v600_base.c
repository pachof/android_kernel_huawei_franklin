...
int hi6526_get_chip_temp(int *temp, void *dev_data)
{
	...
	if (di->chg_mode == SOH_MODE) {
		*temp = di->chip_temp;
		return 0;
	} else if (di->chg_mode == LVC || di->chg_mode == SC) {
...
{
	return hi6526_get_vbat_execute(NULL);
}
...
int hi6526_reset_watchdog_timer_execute(void *dev_data)
{
	...
	if (ibus_abnormal_cnt >= IBUS_ABNORMAL_CNT) {
		...
		hi6526_lvc_enable(0, NULL);
		hi6526_sc_enable(0, NULL);
		return 0;
	}
...
static int hi6526_fcp_adapter_vol_check(int adapter_vol_mv)
{
	...
	for (i = 0; i < FCP_ADAPTER_VOL_CHECK_TIMEOUT; i++) {
		ret = hi6526_get_vbus_mv(NULL, (unsigned int *)&adc_vol);
		if (ret)
			continue;
...
int hi6526_global_reset(void)
{
	int ret0;
	int ret1;

	ret0 = hi6526_write(SOFT_RST_CTL_REG, CHG_RST_CTRL_VAL);
	ret1 = hi6526_write(SOFT_RST_CTL_REG, CHG_RST_CTRL_VAL_DEFAULT);
	if (ret0 || ret1)
...
int hi6526_scp_master_init(void)
{
	...
	ret1 = hi6526_set_buck_enable(CHG_DISABLE, NULL);
	...
	ret7 = hi6526_set_otg_enable(NULL, OTG_DISABLE);
...
int hi6526_chip_init(struct charger_device *dev, struct chip_init_crit *init_crit)
{
	int i;
	...
	ret[13] = hi6526_set_watchdog_timer(dev, WATCHDOG_TIMER_40_S);
	...
	ret[20] = hi6526_set_otg_enable(dev, OTG_DISABLE);
