...
void StateMachineUnattached(struct Port *port)
{
	...
	if (port->Registers.Status.I_VBUSOK) {
		if (port->Registers.Status.VBUSOK) {
			pr_info("FUSB:IVBUS VBUSOK start vbus only timer\n");
			port->PEIdle = FALSE;
			TimerStart(&port->VbusOnlyTimer, tVbusOnly);
		} else {
			if (g_is_last_vbus_ok)
				g_is_last_vbus_ok = false;

			pr_info("FUSB:VBUS NOT OK stop vbus only timer\n");
			port->PEIdle = TRUE;
			TimerDisable(&port->VbusOnlyTimer);
			notify_observers(NORP_SRC_UNATTACHED, port->I2cAddr, 0);
			return;
		}
	}

	if (TimerExpired(&port->VbusOnlyTimer)) {
		if (!g_is_last_vbus_ok)
			g_is_last_vbus_ok = true;

		pr_info("FUSB:cc timer expired stop vbus only timer\n");
		port->PEIdle = TRUE;
		TimerDisable(&port->VbusOnlyTimer);
		notify_observers(NORP_SRC_ATTACHED, port->I2cAddr, 0);
	}

	if (g_is_last_vbus_ok) {
		pr_info("g_is_last_vbus_ok return\n");
		return;
	}
...
void StateMachineAudioAccessory(struct Port *port)
{
	...
	if (port->Registers.Status.VBUSOK) {
		if (!g_is_last_vbus_ok) {
			pr_info("FUSB AudioAccessory vbus only attached\n");
			notify_observers(NORP_SRC_ATTACHED, port->I2cAddr, 0);
			g_is_last_vbus_ok = true;
		}
	} else {
		if (g_is_last_vbus_ok) {
			pr_info("FUSB AudioAccessory vbus only detached\n");
			notify_observers(NORP_SRC_UNATTACHED, port->I2cAddr, 0);
			g_is_last_vbus_ok = false;
		}
	}
...
{
    return isVBUSOverVoltage(port, VBUS_MV_VSAFE5V_DISC);
}
